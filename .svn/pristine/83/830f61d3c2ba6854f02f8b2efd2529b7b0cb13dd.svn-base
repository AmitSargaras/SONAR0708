...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
    ...
    <tbody>
        ...
        <tr class="odd">
            ...
            %>
            <td class="<%=UIUtil.isAmountUnChanged(stagingAmt,actualAmt,true)?"fieldname":"fieldnamediff"%>"><bean:message key="label.advancepayment.claim.amount"/></td>
            <td><integro:amount param="all" amount="<%=obAdvanceDetail.getAmount()%>"/>&nbsp;</td>
        </tr>
        <tr class="even">
            <%
        ...
        <tr class="odd">
            <%
                stagingAmt = (obAdvance == null) ? null : obAdvance.getActualAdvanceAmount();
                actualAmt = (obActualAdvance == null) ? null : obActualAdvance.getActualAdvanceAmount();
            %>
            <td class="<%=UIUtil.isAmountUnChanged(stagingAmt,actualAmt,true)?"fieldname":"fieldnamediff"%>"><bean:message key="label.advancepayment.actual.advanced.amount"/></td>
            <td><integro:amount param="all" amount="<%=obAdvanceDetail.getActualAdvanceAmount()%>" />&nbsp;</td>
        </tr>
        <tr class="odd">
