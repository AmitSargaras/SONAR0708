...
<%
	...
	ICountryTrxValue countryTrxValue = (ICountryTrxValue)session.getAttribute("com.integrosys.cms.ui.geography.country.CountryAction.ICountryTrxValue");
	...
	if (sr != null) {
		...
		pageContext.setAttribute("countryList",
				searchList);
	}
...
<%@page import="com.integrosys.cms.ui.geography.country.CountryForm"%><html>
...
function cancelPage(){
    document.forms[0].action="Country.do?event=checker_view_list_country";
    
    document.forms[0].submit();
}
function approvePage() {
	
    document.forms[0].action="Country.do?event=<%=submitEvent%>";
    document.forms[0].submit();
}
function rejectPage() {
    document.forms[0].action="Country.do?event=<%=rejectEvent%>";
    document.forms[0].submit();
}
function cancelPage(){
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tbody>
												<logic:present name="countryList">
													<logic:iterate id="ob" name="countryList"
													offset="<%=String.valueOf(startIndex)%>"
                               						length="<%=String.valueOf(10)%>"
														type="com.integrosys.cms.app.geography.country.bus.OBCountry"
														scope="page">
														<%
															String rowClass = "";
																			sno++;
														...
														<tr class="<%=rowClass%>">
															...
															<td><integro:empty-if-null value="<%=ob.getCountryCode()%>" />&nbsp;
															</td>
															<td><integro:empty-if-null value="<%=ob.getCountryName()%>" />
															</td>																														
														</tr>
													</logic:iterate>
													<%
												...
												<logic:notPresent name="countryList">
													<tr class="odd">
														<td colspan="10">
															<bean:message key="label.global.not.found" />
														</td>
														...
														<table width="100%" border="0" align="center"
															...
															<tbody>
																...
																<tr>
																	...
																	<td class="even"><%=countryTrxValue.getUserInfo()%>&nbsp;
																	</td>
																</tr>
																<tr class="odd">
																	<td class="fieldname">
																	...
																	<td><%=countryTrxValue.getRemarks() != null ? countryTrxValue.getRemarks() : ""%>&nbsp;
																	</td>
																</tr>
															</tbody>
														</table>
															...
															<tr>
																...
																<%
																	String pageIndexUrl = null;
																			...
																			pageIndexUrl = "Country.do?event=checker_process_create_insert&TrxId="+transId+"&startIndex=";
																%>
