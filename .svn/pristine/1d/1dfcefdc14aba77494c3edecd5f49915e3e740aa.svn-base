...
function searchPage(){
document.forms[0].action="ValuationAgency.do?event=maker_list_valuation&go=Y";
...
function uploadValuationAgency() {
	document.forms[0].action="ValuationAgency.do?event=maker_prepare_upload_valuation_agency";
	    document.forms[0].submit();
	}
//-->
</script>
...
<tr>
   <td valign="top">
                    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
                      ...
                      <td width="30%" >
                      <html:text property="valuationAgencyCodeSearch" value="<%=code%>" maxlength="10"/>
                      <html:errors property="valuationAgencyCodeError" />
                      </td>
                      <td  width="30%"  class="fieldname">Valuation Agency Name : </td>
                      <td width="30%" >
                      <html:text property="valuationAgencyNameSearch" value="<%=name%>"/>
                        <html:errors property="valuationAgencyNameError" />
                      </td>
                      </tr>
                      <td colspan="4"><center><input class=btnNormal style="margin-left: 60px;height:30px; WIDTH: 70px;"  type="button" value="Go" onclick='searchPage()'></center></td>
                    ...
                    <td  valign="bottom" align="right">
											<input type="button" name="Submit" value="Add New" class="btnNormal" onclick="createValuationAgency()"/>
                        ...
                        %>
                              ...
                              <%if(ob.getCityTown()==null){ %>   
                                   ...
                                   <%}else{ %>
                                    ...
                                    <%! String status=null;
                                         %>
                         <%if(ob.getStatus()!=null && ob.getStatus()!="")
                         {
                       ...
                       %> 
                                  ...
                                  <td> <select name="menu1" onchange="MM_jumpMenu('self',this,0)" style="width:110">
                                     ...
                                     <%}else{ %>
                                      ...
                                      <option value='ValuationAgency.do?event=maker_view_valuationAgency&branchCode=<%=ob.getId()%>&startIndex=<%= startIndex %>'>View</option>
								<%if(ob.getStatus().trim().equalsIgnoreCase("ACTIVE"))
                            {%>
                                      <option value='ValuationAgency.do?event=maker_edit_valuationAgency_read&branchCode=<%=ob.getId()%>&startIndex=<%= startIndex %>'>Edit</option>
                                 
                             <%}else{} %>             
                         <%if(ob.getStatus().trim().equalsIgnoreCase("ACTIVE"))
                       {%>    
                                 
                                       <option value='ValuationAgency.do?event=maker_disable_valuationAgency_read&branchCode=<%=ob.getId()%>&startIndex=<%= startIndex %>'><%=status %></option>
                             
                       <%
                             
                       }else{ %> 
                       
                           <option value='ValuationAgency.do?event=maker_enable_valuationAgency_read&branchCode=<%=ob.getId()%>&startIndex=<%= startIndex %>'><%=status %></option>
                                 <%
