...
SearchResult sr = (SearchResult)request.getAttribute("productMasterList");

if(sr!=null){
	 List productMasterList = new ArrayList(sr.getResultList());
		pageContext.setAttribute("productMasterList",productMasterList);
	}
...
function createProductMaster() {
 	...
 	document.forms[0].action = "productMaster.do?event=maker_prepare_create_product_master&startIndex=<%=startIndex%>";
	...
	function searchProductMaster()
	{
        ...
        {
	        document.forms[0].action            = "productMaster.do";
	        document.forms[0].event.value       = "maker_list_product_master";
	        document.forms[0].submit();
        }
        else
        {
	        if(searchType !='' && searchText!='')
		    {	        	
		        document.forms[0].action            = "productMaster.do?type="+searchType +"&"+ "text="+searchText;		        
		        document.forms[0].event.value       = "maker_list_product_master";		              
				document.forms[0].submit();
	        }	       
	    }      
			...
			<table width="95%" border="0" align="center" cellpadding="0"
				...
				<thead>
                   ...
                   <td><label>Search :&nbsp;</label>
	           		...
	           		&nbsp;<img alt="Search" src="./img/search.gif" onclick="javascript:searchProductMaster()">
                   </td>
                   <td>&nbsp;</td>
                   <td  valign="bottom" align="right">						
						<input type="button" name="Submit" value="Add New" class="btnNormal" onclick="createProductMaster()" />
				   </td>    
               </tr>
               <tr>
				...
				<tbody>
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tbody>
								<logic:present name="productMasterList">
                    <logic:iterate id="ob" name="productMasterList"  
                    length="10" 
                    offset="<%=String.valueOf(startIndex)%>"
                    indexId="counter"
                     type="com.integrosys.cms.app.productMaster.bus.OBProductMaster" scope="page">
                        <%
                            String rowClass="";
                            sno++;
                            if(sno%2!=0){
                        ...
                        %>
                                <tr class="<%=rowClass%>">
                                  <td class="index"><%=counter.intValue()+1%></td>
                                    ...
                                    <td><center><%=ob.getProductCode()%>&nbsp;</center></td>                                  
                                    <td><center><%=ob.getProductName()%>&nbsp;</center></td> 
              ...
              <%}else{ %>Disable<%} %>&nbsp;</td>
                                  <td> <select name="menu1" onchange="MM_jumpMenu('self',this,0)" style="width:110">
                                      ...
                                      <%if((event.equals("checker_product_master"))||event.equals("checker_list_pagination")){%>
                                      <option value='productMaster.do?event=checker_view_product_master&productCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
                                       <%}else{ %>
                                      
                                      <option value='productMaster.do?event=maker_view_product_master&productCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
								
                                      <option value='productMaster.do?event=maker_edit_product_master_read&productCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Edit</option>
                 ...
                 <logic:notPresent name="productMasterList">
                                <tr class="odd">
                                	<td colspan="10">
                                		<bean:message key="label.global.not.found"/>                                
                                	</td>
					        ...
					        <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
					            <tr>
					                <td valign="middle">
					                    <integro:pageindex
					                            pageIndex="<%=new PageIndex(startIndex, 10, sr.getResultList().size())%>"
					                            url="productMaster.do?event=list_pagination&startIndex="/>
					                </td>
					            </tr>
					        </table>
					    </td>
