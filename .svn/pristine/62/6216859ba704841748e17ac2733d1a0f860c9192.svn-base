...
<%
    ...
    String isDPDueDateEditable=request.getParameter("isDPDueDateEditable");
    if(isDPDueDateEditable==null){
    	isDPDueDateEditable="";
    }

     List items=(List) request.getAttribute("itemhistory");
    if(items!=null && items.size()>1){
    	isDPDueDateEditable="N";
    }
...
function updateRecurrent() {
	document.forms[0].event.value="update_recurrent_dp_recipt_item";
	document.forms[0].docEndDate.value=document.forms[0].dueDate.value;
...
/* function setDueDate(cal, date) {
	...
	if (gracePeriod != null && gracePeriod != '') {
		gracePeriod = parseFloat(gracePeriod.split(' ').join(''));
	}
	else {
		gracePeriod = 0;
	}
...
<html:hidden property="docEndDate" />
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  <thead>
    <tr>
      <td><h3>Update DP Receipt<%-- <bean:message key="title.rec.doc.receipt.process"/> --%></h3></td>
    </tr>
    <tr>
      <td><hr /></td>
    </tr>
          ...
          <tbody>
            ...
            <tr class="even">
              ...
              <td>
                ...
                <html:hidden property="dateReceived"/> 
                <html:hidden property="deferredDate"/> 
                <html:hidden property="dateWaived"/> 
              </td>
            </tr>
            <tr class="odd">
