...
document.forms[0].action="DocFacilityMaster.do?event=view_template_item&go=s&custTypeTrxID="+trxID;
document.forms[0].submit();
}
...
function createEdit(subType,subTypeDesc,trxID){
	...
	document.forms[0].action="DocFacilityMaster.do?event=edit_template_item&go=s&secTrxID="+trxID;
	document.forms[0].submit();
	}
	}
...
var list2=document.getElementById('secType').value;
...
isNew = true;
isEditAllowed = true;
subType = currentTagTokens[ 0];
subTypeDesc =currentTagTokens[ 0];
trxID = null;
viewOnly= false;
...
function rightAction2(collateralId,viewOnly){

//alert(viewOnly);	
	//document.getElementById('collateralId2').value=collateralId;
...
//alert(subTypeDesc);
document.getElementById('secType').value=subType;
	if(isNew=='true'){
	
         if(viewOnly=='false'){
     	     createEdit(subType,subTypeDesc,trxID);
         }else{
             alert('This Facility master template  is not yet maintained');
         }
    }else{
...
<input type="hidden" name="secType" id="secType" />
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td style="padding-left:5"><h3> Facility Doc Template</h3>
      <hr></td>
  </tr>
  <tr>
    <td><form name="selectcol" method="post" action="">
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
          <tbody>
          <tr class="even">
              <td width="22%" class="fieldname">Facility Name</td>
              <td width="78%">
              <logic:present name="facSubTypeList">
				<select id="subTypeList" onchange="javascript:rightAction2(this,'<%=viewOnly%>')">
	                <option value="pleaseselect" >Please Select </option>
	                <logic:present name="facSubTypeList">
	   				   <logic:iterate id="OB" name="facSubTypeList"  type="com.integrosys.cms.app.chktemplate.bus.CollateralSubTypeSearchResultItem" > 
...
<!--              -->
<!--			  <html:select property="secType" onchange="rightAction()" >-->
<!--                <html:option value="pleaseSelect" >Please Select </html:option>-->
<!--				<html:options name="facTypeValues" labelName="facTypeLabels"/>-->
<!--			  </html:select>-->
              </td>
            </tr> 
...
<!--	   				   <logic:iterate id="OB" name="colSubTypeList"  type="com.integrosys.cms.app.chktemplate.bus.CollateralSubTypeSearchResultItem" > -->
	   				    <%-- <option value="<%=(OB.isNew()+"*"+OB.isEditAllowed()+"*"+OB.getCollateralSubTypeCode()+"*"+OB.getCollateralSubTypeDesc()+"*"+OB.getTrxID())%>"  ><%= OB.getCollateralSubTypeDesc()%></option> --%>
<!--	   				   </logic:iterate>-->
<!--					</logic:present>-->
<!--				  </select>-->
<!--			  </logic:present>-->
