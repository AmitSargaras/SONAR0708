...
String bankingMethodVals = (String) session
.getAttribute("com.integrosys.cms.ui.manualinput.customer.ManualInputCustomerAction.bankingMethodVals");
System.out.println("bankingMethodVals 11111111111111111111111111"+bankingMethodVals);

String bankingMethodVals2 = request.getParameter("bankingMethodVals");
System.out.println("bankingMethodVals 22222222222222222222222222"+bankingMethodVals2);
...
if (!(ifscBranchList == null)) {
	...
	/*	if("O".equals(ob.getBankType())){
			...
			soretedMap.put(ob.getBranchName().toUpperCase()+"+"+ob.getIfscCode(),ob);
		System.out.println("ifscBranchList Branch name: "+ob.getBranchName().toUpperCase()+" Ifsc code : "+ob.getIfscCode()+"  List : BankName "+ob.getBankName()+", BranchNameAddress : "+ob.getBranchNameAddress()+" NODAL : "+ob.getNodal()+" LEAD : "+ob.getLead());	
	}
...
if (!(otherBankList == null)) {
	for (int i = 0; otherBankList.size() > i; i++) {
		OBBankingMethod ob1 = (OBBankingMethod) otherBankList.get(i);
		System.out.println("sorted Map data inside otherBankList loop is : bank type : "+ob1.getBankType());
		if(ob1.getBankType().equalsIgnoreCase("s")){
	    System.out.println(" systembank name "+ob1.getSystembank().getSystemBankName()+" systembank address : "+ob1.getSystembank().getAddress());
		}
		else if(ob1.getBankType().equalsIgnoreCase("o")){
		System.out.println(" Otherbank name "+ob1.getOtherbranch().getOtherBranchName()+" otherbank address : "+ob1.getOtherbranch().getOtherBranchName());
		}
		}
...
<%@page import="java.math.BigDecimal"%><head>
		...
		<table width="95%" border="0" align="center" cellpadding="0"
			...
			<tr id="financialdetails">
							...
							<tbody>
                        		...
                        		<logic:present name="ifscBranchList">
									<logic:iterate id="ob" name="ifscBranchList"
										...
										<td>
											<center>
											<%System.out.println("value.getLeadValue()====>"+value.getLeadValue());
											if(value.getLeadValue()!=null && !value.getLeadValue().equals(""))
											{
			                                   	String[] array = value.getLeadValue().split(",");
		                                       	String bankId = array[1];
                ...
                <!-- @@@@@@@@@@@@@@@@@@@@  IFSC CODE @@@@@@@@@@@@@@@@@@@@@@@@@  --> 
						...
						<%}else if(value.getBankingMethod().equals("")  ||value.getBankingMethod().contains("SOLE")
								  || value.getBankingMethod().contains("SOLE-SOLE")) {%>
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
