...
IValuationAgencyTrxValue valuationAgencyTrx = (IValuationAgencyTrxValue)session.getAttribute("com.integrosys.cms.ui.valuationAgency.ValuationAgencyAction.IValuationAgencyTrxValue");
	...
	String rejectEvent;
	if (event.equals("checker_view_valuationAgency")) {
		rejectEvent = "checker_list_valuation";
	} else {
		rejectEvent = "maker_list_valuation";
	}
...
function cancelPage(){
    document.forms[0].action="ValuationAgency.do?event=<%=rejectEvent%>&go=N&startIndex=<%=startIndex%>";
