...
DROP PROCEDURE SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH@
...
BEGIN
	...
	INSERT INTO CMS_BATCH_CIF_FUSION (CMS_BATCH_CIF_FUSION_ID, OLD_LE_ID, NEW_LE_ID, SOURCE_ID, TIME_RECEIVED, PROCESSED_IND)
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR CMS_BATCH_CIF_FUSION_SEQ), 2)) as BIGINT),
			OLD_CIF_ID, NEW_CIF_ID, 'ARBS', CURRENT_TIMESTAMP, 'N'
	   FROM si_temp_arbs_cu003 t
	  WHERE is_valid = 'Y');
	  
	CALL "SI_LOG_PROC_TIME"('SI_RUN_TEMP_ARBS_CU003', 'ARBS', 2);
	commit;
...
BEGIN
	...
	-- si_temp_arbs_co001
	-- @refer si_host_col_code_map (deprecated)
	-- @refer common_code_category_entry
	--
	--------------------------------------------
    /*
	...
	*/
	UPDATE si_temp_arbs_co001 a
	   SET (cms_security_type_id, cms_security_subtype_id, asset_type_cat) =
	(SELECT a1.ref_entry_code AS security_type_id, a2.ref_entry_code AS security_sub_type_id,
            CASE a1.ref_entry_code
                WHEN 'AB101'
                    THEN 'PLANT_EQUIP'
                WHEN 'AB102'
                    THEN 'VEHICLE_TYPE'
                WHEN 'AB103'
                    THEN 'AB_OTHERS_TYPE'
            END
        FROM common_code_category_entry a1 INNER JOIN common_code_category_entry a2 ON a1.entry_code = a2.entry_code AND a1.entry_code = a.source_security_sub_type
        AND a1.category_code = 'SOURCE_SEC_TYPE' AND a2.category_code = 'SOURCE_CMS_SEC_TYPE_MAP');
	...
	-------------------------------------
		...
		UPDATE CMS_ASSET_PLANT_EQUIP A SET (PRICE_LIST,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH) =
	(SELECT PRICE_LIST,
		DOWN_PAYMENT_TRADE_IN,
		DOWN_PAYMENT_CASH FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO001 T
	...
	WHERE EXISTS (SELECT '1' FROM CMS_SECURITY, SI_TEMP_ARBS_CO001
								WHERE A.CMS_COLLATERAL_ID = CMS_COLLATERAL_ID
									...
									AND SOURCE_ID = 'ARBS'
									AND IS_VALID = 'Y'
									AND SECURITY_SUB_TYPE_ID IN ('AB101'));

	UPDATE CMS_ASSET_VEHICLE A SET (CHASSIS_NUMBER,
		INS_INDICATOR,
		AUTO_DR_COLLATERAL_FEE,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH,
		FREIGHT_CHARGES,
		PRICE_LIST,
	...
	(SELECT CHASSIS_NUMBER,
		...
		ENGINE_NUMBER,
		HORSE_POWER,
		AMT_COLLECTED_FROM_SALES FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO001 T
		WHERE SEC.CMS_COLLATERAL_ID = T.CMS_ACT_COL_ID
    	AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID)
	WHERE EXISTS (SELECT '1' FROM CMS_SECURITY, SI_TEMP_ARBS_CO001
								WHERE A.CMS_COLLATERAL_ID = CMS_COLLATERAL_ID
									...
									AND SOURCE_ID = 'ARBS'
									AND IS_VALID = 'Y'
									AND SECURITY_SUB_TYPE_ID IN ('AB102'));
	...
	UPDATE CMS_ASSET A SET (ASSET_DESCRIPTION,
		...
		MODEL_NO,
		--RESIDUAL_ASSET_LIFE,
		--RESIDUAL_ASSET_LIFE_UOM,
		ASSET_VALUE,
		--DOC_PERFECT_AGE, /*derived, refering ABCMS-235*/
		SALES_PROCEED,
		REG_NO,
		REG_FEE,
		BRAND,
	...
	(SELECT ASSET_DESC,
		...
		PUBLIC_TRANSPORT,
		--AMT_COLLECTED_FROM_SALES,
		T.ASSET_TYPE_DESC FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO001 T
	  WHERE SEC.CMS_COLLATERAL_ID = T.CMS_ACT_COL_ID
    	AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID)
	  WHERE EXISTS (SELECT '1' FROM CMS_SECURITY, SI_TEMP_ARBS_CO001
	    			 			WHERE A.CMS_COLLATERAL_ID = CMS_COLLATERAL_ID
	...
	INSERT INTO CMS_ASSET_PLANT_EQUIP (CMS_COLLATERAL_ID,
	    PRICE_LIST,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH)
	...
	(SELECT CMS_COLLATERAL_ID,
		...
		--AMT_COLLECTED_FROM_SALES,
		T.ASSET_TYPE_DESC,
		'N' FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO001 T
	  WHERE SEC.SCI_SECURITY_DTL_ID = T.SECURITY_ID
	    AND SOURCE_ID = 'ARBS'
	    AND CMS_ACT_COL_ID IS NULL
			AND IS_VALID = 'Y'
...
--							ASSET_TYPE_HOST_REF) =
--	(SELECT asset_desc, asset_type_code, purchase_price, --purchase_price_currency, 
--		purchase_date, reg_date,
...
--	(SELECT cms_collateral_id, asset_desc, asset_type_code, purchase_price, --purchase_price_currency, 
--			purchase_date, reg_date,
--			manufacture_year, model_no, 
--			--residual_asset_life, residual_asset_life_unit, 
--			asset_value, 
...
--		AND security_sub_type_id IN ('AB101', 'AB102', 'AB103'));
	...
	UPDATE CMS_STAGE_ASSET_PLANT_EQUIP A SET (PRICE_LIST,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH) =
	(SELECT PRICE_LIST,
		DOWN_PAYMENT_TRADE_IN,
		DOWN_PAYMENT_CASH FROM CMS_STAGE_SECURITY SEC, SI_TEMP_ARBS_CO001 T
		WHERE SEC.CMS_COLLATERAL_ID = T.CMS_STG_COL_ID
    	AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID
    	AND SEC.SECURITY_SUB_TYPE_ID IN ('AB101'))
	...
	UPDATE CMS_STAGE_ASSET_VEHICLE A SET (CHASSIS_NUMBER,
		INS_INDICATOR,
		AUTO_DR_COLLATERAL_FEE,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH,
	...
	(SELECT CHASSIS_NUMBER,
		...
		ENGINE_NUMBER,
		HORSE_POWER,
		AMT_COLLECTED_FROM_SALES FROM CMS_STAGE_SECURITY SEC, SI_TEMP_ARBS_CO001 T
		WHERE SEC.CMS_COLLATERAL_ID = T.CMS_STG_COL_ID
    	AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID
    	AND SEC.SECURITY_SUB_TYPE_ID IN ('AB102'))
	WHERE EXISTS (SELECT '1' FROM CMS_STAGE_SECURITY, SI_TEMP_ARBS_CO001
								WHERE A.CMS_COLLATERAL_ID = CMS_COLLATERAL_ID
									AND CMS_COLLATERAL_ID = CMS_STG_COL_ID
									AND SOURCE_ID = 'ARBS'
	...
	(SELECT ASSET_DESC,
		...
		PUBLIC_TRANSPORT,
		--AMT_COLLECTED_FROM_SALES,
		T.ASSET_TYPE_DESC FROM CMS_STAGE_SECURITY SEC, SI_TEMP_ARBS_CO001 T
	  WHERE SEC.CMS_COLLATERAL_ID = T.CMS_STG_COL_ID
    	AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID
    	AND SEC.SECURITY_SUB_TYPE_ID IN ('AB103'))
	  WHERE EXISTS (SELECT '1' FROM CMS_STAGE_SECURITY, SI_TEMP_ARBS_CO001
	    			 			WHERE A.CMS_COLLATERAL_ID = CMS_COLLATERAL_ID
										AND CMS_COLLATERAL_ID = CMS_STG_COL_ID
										AND SOURCE_ID = 'ARBS'
	...
	INSERT INTO CMS_STAGE_ASSET_PLANT_EQUIP (CMS_COLLATERAL_ID,
	    PRICE_LIST,
		DOWNPAYMENT_TRADEIN,
		DOWNPAYMENT_CASH)
	(SELECT CMS_COLLATERAL_ID,
	...
	(SELECT CMS_COLLATERAL_ID,
		...
		--AMT_COLLECTED_FROM_SALES,
		T.ASSET_TYPE_DESC,
		'N' FROM CMS_STAGE_SECURITY SEC, SI_TEMP_ARBS_CO001 T
	  WHERE SEC.SCI_SECURITY_DTL_ID = T.SECURITY_ID
	    AND SOURCE_ID = 'ARBS'
	    AND CMS_STG_COL_ID IS NULL
			AND IS_VALID = 'Y');
...
--								  public_transport, amt_collected_from_sales,
--									ASSET_TYPE_HOST_REF) =
--	(SELECT asset_desc, asset_type_code, purchase_price, 
--	--purchase_price_currency, 
--	purchase_date, reg_date,
--			manufacture_year, model_no, 
--			--residual_asset_life, residual_asset_life_unit, 
...
BEGIN
	...
	*/
	...
	-------------------------------------
		...
		UPDATE CMS_ASSET_GOLD A SET (--ASSET_DESCRIPTION,
		--TYPE_OF_ASSET,
		--PURCHASE_PRICE,
		--PURCHASE_CURRENCY,
		--SALES_PROCEED,
	...
	(SELECT --ASSET_DESC,
		...
		AUCTION_DATE,
		AUCTION_PRICE,
		AUCTIONEER FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO002 T
		--CASH_VALUE_FOR_FORCE_SALE FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO002 T
	  WHERE SEC.SCI_SECURITY_DTL_ID = T.SECURITY_ID
	    AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID)
	...
	INSERT INTO CMS_ASSET_GOLD (CMS_COLLATERAL_ID,
		--ASSET_DESCRIPTION,
		--TYPE_OF_ASSET,
		--PURCHASE_PRICE,
		--PURCHASE_CURRENCY,
...
--	(SELECT cms_collateral_id, asset_desc, asset_type_code, purchase_price, --purchase_price_currency, 
--			sales_proceed,gold_grade, purchase_receipt_no, gold_unit_price, gold_unit_price_uom, gold_weight, gold_weight_uom,
...
--			AND IS_VALID = 'Y');
	...
	UPDATE CMS_STAGE_ASSET_GOLD A SET (--ASSET_DESCRIPTION,
		--TYPE_OF_ASSET,
		--PURCHASE_PRICE,
		--PURCHASE_CURRENCY,
		--SALES_PROCEED,
	...
	(SELECT --ASSET_DESC,
		...
		AUCTION_DATE,
		AUCTION_PRICE,
		AUCTIONEER FROM CMS_STAGE_SECURITY SEC, SI_TEMP_ARBS_CO002 T
		--CASH_VALUE_FOR_FORCE_SALE FROM CMS_SECURITY SEC, SI_TEMP_ARBS_CO002 T
	  WHERE SEC.SCI_SECURITY_DTL_ID = T.SECURITY_ID
	    AND A.CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID)
	...
	INSERT INTO CMS_STAGE_ASSET_GOLD (CMS_COLLATERAL_ID,
		--ASSET_DESCRIPTION,
		--TYPE_OF_ASSET,
		--PURCHASE_PRICE,
		--PURCHASE_CURRENCY,
	...
	(SELECT CMS_COLLATERAL_ID,
	  ...
	  WHERE SEC.SCI_SECURITY_DTL_ID = T.SECURITY_ID
			AND SOURCE_ID = 'ARBS'
	    AND NOT EXISTS (SELECT '1' FROM CMS_STAGE_ASSET_GOLD
	    				 				WHERE CMS_COLLATERAL_ID = SEC.CMS_COLLATERAL_ID)
											AND IS_VALID = 'Y');
...
BEGIN
  ...
  WHERE EXISTS (SELECT '1' FROM si_temp_arbs_co009
	...
	UPDATE cms_property a SET(
		description_asset, sale_purchase_value, title_type, title_number, lot_no, PROPERTY_ADDRESS, PROPERTY_ADDRESS_2, PROPERTY_ADDRESS_3,
	...
	(SELECT 
		asset_desc_code, sale_purchase_agreement_value, title_type, title_number, lot_no, substr(lot_loc, 1, 40), substr(lot_loc, 41, 40), substr(lot_loc, 81, 40),
...
-- 		when 'N' then '3' end),
	...
	INSERT INTO cms_property (
		...
		lot_no, PROPERTY_ADDRESS, PROPERTY_ADDRESS_2, PROPERTY_ADDRESS_3, postcode, tenure, tenure_unit, quit_rent_paid, abandoned_project,
	...
	(SELECT 
		...
		lot_no, substr(lot_loc, 1, 40), substr(lot_loc, 41, 40), substr(lot_loc, 81, 40), post_code, tenure, tenure_unit, quit_rent_amt_paid, abandoned_project,
...
-- 		when 'N' then '3' end),
  ...
  WHERE EXISTS (SELECT '1' FROM si_temp_arbs_co009
	...
	(SELECT 
		...
		lot_no, substr(lot_loc, 1, 40), substr(lot_loc, 41, 40), substr(lot_loc, 81, 40), post_code, tenure, tenure_unit, quit_rent_amt_paid,
...
-- 		when 'N' then '3' end),
	...
	(SELECT 
		...
		title_number, lot_no, substr(lot_loc, 1, 40), substr(lot_loc, 41, 40), substr(lot_loc, 81, 40), post_code, tenure, tenure_unit, quit_rent_amt_paid,
		abandoned_project, sale_purchase_date, master_title, master_title_number,
		property_usage, property_completed_flag, 
-- 		(case property_completed_flag when
-- 		'Y' then '1'
...
BEGIN
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR valuation_seq), 2)) as BIGINT),
			cms_collateral_id, valuer_code, valuation_date, valuation_currency, t.cmv, t.fsv, t.reserve_price,
			t.reserve_price_date, t.appraisal_name,
			(CASE 
	...
	INSERT INTO cms_valuation (valuation_id, cms_collateral_id, valuer, valuation_date, valuation_currency, cmv, fsv, 
  	...
  	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR valuation_seq), 2)) as BIGINT),
		  	...
		  	t.fsv, t.reserve_price, t.reserve_price_date, t.appraisal_name, 'B', CURRENT_TIMESTAMP, 'S', 'ARBS'
	   FROM si_temp_arbs_co015 t, cms_security sec
	  WHERE NOT EXISTS (SELECT '1' FROM cms_valuation
	  					 WHERE cms_collateral_id = sec.cms_collateral_id
    ...
    (SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR valuation_seq), 2)) as BIGINT),
			...
			t.reserve_price, t.reserve_price_date, t.appraisal_name, 'A', CURRENT_TIMESTAMP, 'S', 'ARBS'
	   FROM si_temp_arbs_co015 t, cms_security sec
	  WHERE NOT EXISTS (SELECT '1' FROM cms_valuation
	  					 WHERE cms_collateral_id = sec.cms_collateral_id
...
BEGIN
	...
	(SELECT pledgor_legal_name, UPDATE_STATUS_IND, current_timestamp,
		--(select LMP_INC_NUM_TEXT from sci_le_main_profile
		 (select LMP_ID_NUMBER from sci_le_main_profile
...
CREATE PROCEDURE SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH
	LANGUAGE SQL
BEGIN

    -- < 500 except 399 - conventional
    
    CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 1);
   
    UPDATE cms_stage_bank_entity_param set (ENTITY_TYPE) =
	(SELECT
		'2'	
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_code < '500' and entry_code <> '399'
	  and BRANCH_CODE = ENTRY_CODE 
	  and EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE)
	  and active_status = '1')
	  where BRANCH_CODE < '500' and BRANCH_CODE <> '399'; 
	  
	CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 2);
  
	INSERT INTO cms_stage_bank_entity_param (
		BANK_ENTITY_ID,
		GROUP_ID,
		ENTITY_TYPE,
		BRANCH_CODE,
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
		1,
		'2',
		ENTRY_CODE,
		'ACTIVE',
		0,
		CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT)
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_source = 'ARBS' and entry_code < '500' and entry_code <> '399' and active_status = '1' 
	  and NOT EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE) order by entry_code);
	  					 
	 CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 3);
	  					 
	   UPDATE cms_bank_entity_param set (ENTITY_TYPE) =
	(SELECT
		'2'	
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_code < '500' and entry_code <> '399'
	  and BRANCH_CODE = ENTRY_CODE 
	  and EXISTS (SELECT '1' FROM cms_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE)
	  and active_status = '1')
	  where BRANCH_CODE < '500' and BRANCH_CODE <> '399'; 
	  					 		 
	  INSERT INTO cms_bank_entity_param (
		BANK_ENTITY_ID,
		GROUP_ID,
		ENTITY_TYPE,
		BRANCH_CODE,
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
	   ...
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_source = 'ARBS' and entry_code < '500' and entry_code <> '399' and active_status = '1' 
	  and NOT EXISTS (SELECT '1' FROM cms_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE) order by entry_code);
	  	
	  					 	
	   -- > 500 except 899 - islamic
	   
	   CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 4);
	   
	    UPDATE cms_stage_bank_entity_param set (ENTITY_TYPE) =
	(SELECT
		'3'	
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_code > '500' and entry_code <> '899' 
	  and BRANCH_CODE = ENTRY_CODE 
	  and EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE)
	  	and active_status = '1')
		where BRANCH_CODE > '500' and BRANCH_CODE <> '899';
		
	   CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 5);
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
		...
		CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT)
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_source = 'ARBS' and entry_code > '500' and entry_code <> '899' and active_status = '1'  
	  and NOT EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE) order by entry_code);
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
	  ...
	  CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 6);
	   
	    UPDATE cms_stage_bank_entity_param set (ENTITY_TYPE) =
	(SELECT
		'4'	
	   FROM common_code_category_entry
	  WHERE category_code = '40' and (entry_code = '399' or entry_code = '899')
	  and BRANCH_CODE = ENTRY_CODE 
	  and EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE)
	  and active_status = '1')
	  where (BRANCH_CODE = '399' or BRANCH_CODE = '899'); 					 
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
		...
		CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT)
	   FROM common_code_category_entry
	  WHERE category_code = '40' and entry_source = 'ARBS' and (entry_code = '399' or entry_code = '899') and active_status = '1' 
	  and NOT EXISTS (SELECT '1' FROM cms_stage_bank_entity_param
	  					 WHERE BRANCH_CODE = ENTRY_CODE) order by entry_code);		
	  					 
	 CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 7);		 
	...
	(SELECT
	  ...
	  CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 8);	
	...
	(SELECT CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(NEXT VALUE FOR cms_stg_bank_entity_seq), 2)) as BIGINT),
	 ...
	 CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 9);
	
	
	  UPDATE cms_stage_bank_entity_param SET STATUS = 'DELETED' where 
		EXISTS (SELECT '1' FROM common_code_category_entry
	  					 WHERE BRANCH_CODE = ENTRY_CODE and ACTIVE_STATUS = '0'); 
	  					 
	  UPDATE cms_bank_entity_param SET STATUS = 'DELETED' where 
		EXISTS (SELECT '1' FROM common_code_category_entry
	  					 WHERE BRANCH_CODE = ENTRY_CODE and ACTIVE_STATUS = '0'); 
	  					 
	  CALL "SI_LOG_PROC_TIME"('SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH', 'ARBS', 0);
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_count FROM si_temp_arbs_co018;
	...
	call SI_AUTO_UPDATE_VALUATION;
	
	call SI_AUTO_UPDATE_ARBS_BANK_ENTITY_BRANCH;
