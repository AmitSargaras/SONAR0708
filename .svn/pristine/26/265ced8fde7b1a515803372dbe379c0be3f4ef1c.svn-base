...
List tagDetailList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.secreceipt.SecurityReceiptAction.tagDetailList");
...
function showListing() {
	var catg= '<%=category%>';
	<%-- if('<%=event%>'== 'retrieveImageGallaryFacilityDoc')
	{
	...
	} retrieveImageTagUntagSecReceipt --%>
	
	if('<%=event%>'== 'retrieveImageTagUntagSecReceipt')
	{
	 document.forms[0].action="SecurityReceipt.do?event=imagetaguntaglist&category="+catg;
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveIndImageTagUntagSecReceiptPath' || '<%=event%>' == 'retrieveImageTagUntagSecReceipt'){
			 document.forms[0].action="SecurityReceipt.do?event=retrieveImageTagUntagSecReceipt&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category="+categ;
				document.forms[0].submit();
			}
...
<%if(event.equals("retrieveIndImageTagUntagSecReceiptPath")){%>
