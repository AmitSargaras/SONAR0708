...
<%@page import="java.text.DateFormat"%><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
...
String custLimitProfileNo=(String)request.getAttribute("custLimitProfileID");   
request.setAttribute("custLimitProfileID",custLimitProfileNo);
String rowClass = "";
...
List tageedImageList = (List) request.getAttribute("tageedImageList");
 //request.setAttribute("tageedImageList",tageedImageList);
 session.setAttribute("com.integrosys.cms.ui.imageTag.ImageTagAction.tageedImageList",tageedImageList);
	String customerName = request.getParameter("customerName");
	int rowIdx = 0;
	int sno = 0;
	
	//Added by Uma Khot: Start: Phase 3 CR:tag scanned images of Annexure II
	String	docType=(String)session.getAttribute("com.integrosys.cms.ui.imageTag.ImageTagAction.docType");
...
//Added by Uma Khot: Start: Phase 3 CR:tag scanned images of Annexure II
var docType= '<%=docType%>';
if(docType == '<%=IImageTagConstants.EXCHANGE_OF_INFO%>'){
...
function closePage() {
    document.forms[0].action="ImageTag.do?event=untag_customer_search";
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="ImageTagMap.do?event=retrieveUntagImageGallary&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
...
<html:hidden name="ImageTagMapForm" property="category" />
<html:hidden name="ImageTagMapForm" property="bank" />								
<table width="100%" >
<tr>
...
<a href="#"
onclick="getImageGallary('<%=obImageUploadAdd.getImageFilePath()%>','<%=obImageUploadAdd.getImgFileName()%>','<%=obImageUploadAdd.getStatus()%>','<%=counter1-1%>')"><%=obImageUploadAdd.getImgFileName()%>
</a>
<%}else{ %>
...
<%} %>
                                    								...
                                    								categoryCode="<%=IImageTagConstants.IMG_UPLOAD_CATEGORY %>" display="true" descWithCode="false" />
																		...
																		<!--Added by Uma Khot: Start: Phase 3 CR:tag scanned images of Annexure II -->
																		<td width="5%">&nbsp; <integro:empty-if-null
        ...
        <table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
            ...
            <tbody>
                ...
                <tr class="odd">
					...
					<integro:common-code-single  entryCode="<%=imageTagMapForm.getDocType()%>"
...
<logic:equal name="ImageTagMapForm" property="docType" value="<%=IImageTagConstants.SECURITY_DOC%>">
                ...
                <tr class="odd">
					...
					<td width="25%" class="even" colspan="4">
					<bean:write name="ImageTagMapForm" property="securityIdLabel"/>&nbsp;
...
<p align="center">
<a href="javascript:previousPage();" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image3311','','img/prev2.gif',1)" >
	<img src="img/prev1.gif" name="Image3311" border="0" id="Image3311" />
</a>
<%if (tageedImageList != null && tageedImageList.size()>0) { %>
