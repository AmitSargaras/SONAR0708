...
<%
    ...
    if(docsHeldMap.get("SECURITY")==null){
		System.out.print("========docsHeldMap===================null");
	}
    List docList = (List) docsHeldMap.get(ICMSConstant.CHECKLIST_SECURITY);
    ...
    ICollateral collateral = docHeld.getCollateral();
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
  <tr class="odd"> 
    ...
    <td><integro:empty-if-null value="<%=cust.getCustomerName()%>"/>&nbsp;</td>
  </tr>
  <tr class="even">
    <td class="fieldname"><bean:message key="label.customer.id"/></td>
    <td><integro:empty-if-null value="<%=cust.getCMSLegalEntity().getLEReference()%>"/>&nbsp;</td>
  </tr>
  <tr class="odd"> 
    <td class="fieldname"><bean:message key="label.customer.subprofile.id"/></td>
    <td><integro:empty-if-null value="<%=cust.getCustomerReference()%>"/>&nbsp;</td>
  </tr>
</table>
<br>
-->
<table  width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblinfo" style="margin-top:0">
        <thead> 
        <tr> 
          <td width="40%">Customer Name</td>
          <td width="40%"><bean:message key="label.customer.id"/></td>
          <td width="20%"><bean:message key="label.customer.subprofile.id"/></td>
        </tr>
        </thead> 
        <%
        ArrayList coCustomerList=docHeld.getCoCustomerList();
        String backColor = "even";
        for(int i=0;coCustomerList!=null&&i<coCustomerList.size();i++){ 
	      ISecurityCustomer aCustomer=(OBSecurityCustomer)coCustomerList.get(i);
        ...
        <td><integro:empty-if-null value="<%=String.valueOf(aCustomer.getLspID())%>"/></td>
        </tr>      
        <%}%>
</table>
<BR>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
  <tr class="odd"> 
    <td width="24%" class="fieldname">Security ID</td>
    <td><integro:empty-if-null value="<%=String.valueOf(collateral.getSCISecurityID())%>"/>&nbsp;</td>
  </tr>
  <tr class="even"> 
    <td class="fieldname">Security Type</td>
    <td><integro:empty-if-null value="<%=collateral.getCollateralSubType().getTypeName()%>"/> / 
    <integro:empty-if-null value="<%=collateral.getCollateralSubType().getSubTypeName()%>"/></td>
  </tr>
  <%
  	Locale locale = (Locale)session.getAttribute(ICommonEventConstant.GLOBAL_SCOPE+"."+com.integrosys.base.uiinfra.common.Constants.GLOBAL_LOCALE_KEY);
  	String[] colDetailsList = CheckListUtil.getCollateralParticulars(collateral, locale);
  		for (int i = 0; i < colDetailsList.length; i++) {  	
	  		if (colDetailsList[i] != null) {
  %>
    <tr class="odd">
  <%
	  		if (i == 0) {
  %>
  	<td rowspan="<%=colDetailsList.length%>" class="fieldname">Particulars of Security</td>
  <% } %>  
  	<td><integro:empty-if-null value="<%=colDetailsList[i]%>"/>&nbsp;</td>	
  </tr>
  <% } } %>  
  <%
  	String[] chargeDetailsList = CheckListUtil.getChargeParticulars(collateral, locale);  	
  	for (int i = 0; i < chargeDetailsList.length; i++) {
	  	if (chargeDetailsList[i] != null) {
  %>
  <tr class="even">
  <%
	  	if (i == 0) {
  %>    
  	<td rowspan="<%=chargeDetailsList.length%>" class="fieldname">Particulars of Charge</td>
  <% } %>
  	<td><integro:empty-if-null value="<%=chargeDetailsList[i]%>"/>&nbsp;</td>  
  </tr>
  <% } } %>  
</table>
<br>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  ...
  <tr> 
    <td> 
      <h3> Security Documents - <%=docHeld.getSubCategory()%></h3>
    </td>
  </tr>
  <tr> 
    <td>
...
<%
	if (DocumentCheckListAction.EVENT_LIST_SECURITIES.equals(event)) { 
%>
<table width="75" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    ...
    <td valign="top" ><a href="#" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image2','','img/print2.gif',1)" onclick="MM_openBrWindow('DocumentCheckList.do?event=print_securities_list&checklist_id=<%=checkListIDStr%>&collateral_id=<%=collateral.getCollateralID()%>','','scrollbars=yes,toolbar=yes,menubar=yes,resizable=yes,width=760,height=450,left=5,top=5')" ><img src="img/print1.gif" name="Image2" border="0" id="Image2" /></a></td>
  </tr>
  <tr> 
    <td valign="top" >&nbsp;</td>
  </tr>
