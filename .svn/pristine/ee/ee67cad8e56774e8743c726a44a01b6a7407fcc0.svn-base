...
String event = request.getParameter("event");
    ...
    ArrayList docIds = (ArrayList)session.getAttribute("com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction.docNos");
...
<%
String actionStr1 = "com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction";
...
formObj = document.forms["OtherReceiptForm"];
    ...
    if(temp=="") {
        return false;
	      ...
	      (selObj.options[selObj.selectedIndex].value == 'ALLOW_PERM_UPLIFT' ||
	       ...
	       alert("The document must de-link linkage to Other before deleting/perm-uplifting");
...
function searchDoc(){
	...
	document.forms[0].action      	= "OtherReceipt.do?list="+list+"&searchType="+searchType+"&search="+search;
 	document.forms[0].event.value 	= "search_other_doc";
 	document.forms[0].submit();
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  <thead>
    <tr>
      <td> <h3>Update Other Doc Receipt</h3>
...
<!--			      <td width="763">-->
<!--			         <html:radio property="valuerFirmPanelFlag"  name="OtherReceiptForm" value="Y" onclick="getlag()" />Yes &nbsp;-->
<!--	                 <html:radio property="valuerFirmPanelFlag"  name="OtherReceiptForm" value="N" onclick="getlag()" /> No &nbsp;-->
<!--					<html:radio property="valuerFirmPanelFlag"  name="OtherReceiptForm" value="-" onclick="getlag()" /> N/A-->
...
<!--		 </td>-->
<!--	  </tr>-->
<html:hidden property="valuerFirmPanelFlag"  name="OtherReceiptForm" value="-"  />
	  <tr>
	  	<td>&nbsp;</td>
	  </tr> 
...
<!--			      <td width="763">-->
<!--			         <html:radio property="insurerFirmPanelFlag"  name="OtherReceiptForm" value="Y" onclick="getlag()" />Yes &nbsp;-->
<!--	                 <html:radio property="insurerFirmPanelFlag"  name="OtherReceiptForm" value="N" onclick="getlag()" /> No &nbsp;-->
<!--					<html:radio property="insurerFirmPanelFlag"  name="OtherReceiptForm" value="-" onclick="getlag()" /> N/A-->
...
<!--		 </td>-->
<!--	  </tr>-->
<html:hidden property="insurerFirmPanelFlag"  name="OtherReceiptForm" value="-" />
	  <tr>
	  	<td>&nbsp;</td>
	  </tr> 
<!--	<tr>-->
...
--%>
             ...
             <td>&nbsp;<%= docStatus%></td>
              ...
              <td>
				<%
				if(OB.getItemStatus()!=null &&OB.getDeferCount()!=null){
				if(OB.getItemStatus().equals("PENDING_DEFER") && !OB.getDeferCount().equals("1") ){
				if(OB.getDeferExtendedDate()!=null){%>
				<integro:date object="<%=OB.getDeferExtendedDate()%>" />
				<%} else {%> <center>-</center> <%}
				}else if(OB.getItemDesc().contains(ICMSConstant.INSURANCE_POLICY2) && OB.getItemStatus().equals("PENDING_DEFER")) {
					if(OB.getDeferExtendedDate()!=null){%> <integro:date object="<%=OB.getDeferExtendedDate()%>" /><%} else {%> <center>-</center> <%}}else{
					if(OB.getExpectedReturnDate()!=null){%>
					<integro:date object="<%=OB.getExpectedReturnDate()%>" />
					<%} else {%> <center>-</center> <%}}}else if(null==OB.getDeferCount() && OB.getItemDesc().contains(ICMSConstant.INSURANCE_POLICY2) && "DEFERRED".equals(OB.getItemStatus())){ 
						if(OB.getExpectedReturnDate()!=null){%>
						<integro:date object="<%=OB.getExpectedReturnDate()%>" />
...
<%-- show view only for share doc                    --%>
				...
				<% if(!( null!=OB.getItemDesc() && OB.getItemDesc().contains(ICMSConstant.INSURANCE_POLICY2))) {%>
				...
				<%}if(OB.getItemStatus().equalsIgnoreCase("PENDING_RECEIVED")){%>
				<option value="VIEW_AWAITING" >VIEW</option>
				<option value="RECEIVE" >UPDATE</option>
...
<% if(row==0){%>
            <tr class="odd">
              <td colspan="15">There are no others linked</td>
            </tr>
<% }%>
          </tbody>
        </table></td>
