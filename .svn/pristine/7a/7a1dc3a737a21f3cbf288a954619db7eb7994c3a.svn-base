...
OBDiscrepency updateDisc=(OBDiscrepency)request.getAttribute("discrepencyObj");
String discId=(String)request.getAttribute("discId");
...
if(updateDisc!=null){
	if(updateDisc.getFacilityList()!=null){
	 discrepencyFacilityLists=updateDisc.getFacilityList();
	}
}
...
<head>
    ...
    <script language="JavaScript" type="text/JavaScript">
        ...
        function saveForm() {
            ...
            document.forms["DiscrepencyForm"].action = "Discrepency.do?event=maker_conform_update_temp_discrepency&searchstatus=<%=searchstatus%>&discType=<%=discType%>&startIndex=<%=startIndex%>&startIndexInn=<%=startIndexInn%>&discId=<%=discId%>";
        ...
        function cancelForm() {
        	...
        	document.forms["DiscrepencyForm"].action = "Discrepency.do?event=prepare_create_discrepency_return&searchstatus=<%=searchstatus%>&discType=<%=discType %>&startIndex=<%=startIndex%>&startIndexInn=<%=startIndexInn%>";
    	    document.forms["DiscrepencyForm"].submit();
    ...
    <tbody>
        <tr>
            <td>	<table width="100%" border="0" cellspacing="0" cellpadding="0"
                        ...
                        <td class="even"><%=aForm.getDiscrepencyType() %></td>
                       
                    </tr>
                      <% if( aForm.getDiscrepencyType().equals("Facility") ) { %>
		                        		<tr class="even" >
										...
										<%if("prepare_maker_edit_reject_discrepency".equals(event)){
										if(aForm.getCritical().equals("Yes")){ %>
			                            ...
			                            <input type="radio" name="critical" id="radio4" value="No" ></td>
	                          				<%}else if(aForm.getCritical().equals("No")){ %>
	                          				<td class="even" width="20%" >Yes
