...
String list=(String)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.session.list");
if(list==null)
	list="";
String searchType=(String)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.session.searchType");
if(searchType==null)
	searchType="";
String search=(String)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.session.search");
if(search==null)
	search="";
    ...
    ICheckList checkList = (ICheckList)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.checkList");
	ArrayList deferCreditApproverList = (ArrayList)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.deferCreditApproverList");
    String checkListid=String.valueOf(checkList.getCheckListID());
    ArrayList allCreditApproverList = (ArrayList)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.allCreditApproverList");
    ...
    if(flag.equals("")){
   	 flag= (String)session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.flag");
   }
   if(flag==null){
	   flag="";
   }
    ...
    if(!toTrack || "cancel_checklist_item".equals(event)||"search_cam_doc_checker_close".equals(event)){
 	   searchEvent="search_cam_doc_checker_close";
    }
    if("close_checklist_item".equals(event)){
    	searchEvent="search_cam_doc_checker_close";
    }
    if("edit_staging_checklist_item".equals(event)){
    	searchEvent="search_cam_doc_maker_close";
    }
    if(isUserCpcMaker && "view_return".equals(event) && !toTrack && !previous.equals("view2")){
    	searchEvent="search_cam_doc_checker_close";
    }
    if(!isUserCpcMaker && !toTrack ){
    	searchEvent="checker_search_cam_doc";
    }
    if(isUserCpcMaker && ("view2".equals(event)||"search_cam_doc_view".equals(event)||"view_back".equals(event)||previous.equals("view")) && !toTrack){
    	searchEvent="search_cam_doc_view";
    }
...
function goView(itemRef, checkListID){
    formObj = document.forms["CAMReceiptForm"];
...
function goView1(itemRef){
    formObj = document.forms["CAMReceiptForm"];
    formObj.event.value="view_receipt_img";
    formObj.itemRef.value = itemRef;
    formObj.submit();
}
...
function viewImage2(itemRef,ind) {
    formObj = document.forms["CAMReceiptForm"];
    formObj.event.value="view_image_page2";
    formObj.itemRef.value = itemRef;
    document.forms[0].index.value=ind;
    formObj.submit();
}
...
//	var search=document.getElementById("pramod").value;		
//	document.forms[0].action      	= "CAMReceipt.do?list="+list+"&searchType="+searchType+"&search="+search+"&checkListID="+id;
 //	document.forms[0].event.value 	= "checker_search_cam_doc";
 //	document.forms[0].submit();
//}
function searchDoc(){
	var list = document.getElementById("list").value;
	...
	document.forms[0].action      	= "CAMReceipt.do?list="+list+"&searchType="+searchType+"&search="+search;
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  <thead>
    <tr>
      <td> <h3><%=("Cancel".equals(flag)?"Close":flag)%> CAM Doc Receipt</h3>
...
<!--		    </tr>-->
		     ...
		     <tr class="odd">
                ...
                <td><%= String.valueOf(checkListTrxVal.getCheckList().getCheckListID() )%>
                </td>
                </tr>
<tr class="even">
...
--%>
              ...
              <%}else if("view_return".equals(event)|| "close_checklist_item".equals(event)||"cancel_checklist_item".equals(event)||"search_cam_doc_maker_close".equals(event)||"search_cam_doc_checker_close".equals(event)){
            	    %>
              
               <a href="#" onClick="viewImageToDo('<%=OB.getCheckListItemRef()%>','<%=row-1 %>')">View Images</a>
              <%}else if("view2".equals(event)||"view_back".equals(event)||"search_cam_doc_view".equals(event)){             	  
              %>
              
                <a href="#" onClick="viewImage2('<%=OB.getCheckListItemRef()%>','<%=row-1 %>')">View Images</a>
              ...
              <td>
              <%-- if(!OB.getItemStatus().equals(ICMSConstant.STATE_ITEM_AWAITING)) { --%>
              <%  if ("view2".equals(request.getParameter("event"))) {
                    String checkListID = request.getParameter("checkListID");                   
              %>
                <a href="#" onClick="goView('<%=OB.getCheckListItemRef()%>', '<%=checkListID%>')">View</a>
              <% }else if("search_cam_doc_view".equals(event)||"view_back".equals(event)){ %>
...
<% if(row==0){%>
            <tr class="odd">
              <td colspan="16">There are no cam linked</td>
            </tr>
<% }%>
          </tbody>
        </table></td>
...
<table width="150" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <%if((!toTrack && ("Close".equals(flag)||"Edit".equals(flag)))&& !checkListTrxVal.getStatus().equalsIgnoreCase("DRAFT")&& !("search_cam_doc_view".equals(event)||"view2".equals(event)||"view_back".equals(event))) {System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 111");
    ...
    <%}else if("search_cam_doc_view".equals(event)||"view2".equals(event)||"view_back".equals(event)){%>
    
    <td><a href="CAMReceipt.do?event=list" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image4','','img/return2.gif',1)"><img src="img/return1.gif" name="Image4" width="70" height="20" border="0" id="Image4" /></a></a></td>
