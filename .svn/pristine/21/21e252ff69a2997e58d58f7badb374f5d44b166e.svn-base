...
<%
	System.out.println(">>>> Entity Limit track.jsp");
    ...
    boolean forClose = event.equals(EntityLimitListAction.EVENT_READ_MAKER_CLOSE) ||
    					event.equals(EntityLimitListAction.EVENT_LIST_MAKER_CLOSE);

	boolean forRead = EntityLimitListAction.EVENT_TOTRACK.equals(event) ||
						EntityLimitListAction.EVENT_LIST_READ.equals(event);
	
    // Get the entity limit trx value from session.
    ...
    if (entityLimitTrxValue == null)
    	entityLimitTrxValue = new OBEntityLimitTrxValue();
    ...
    int offset = ((Integer)session.getAttribute(
            "com.integrosys.cms.ui.creditriskparam.entitylimit.EntityLimitAction.offset")).intValue();
    int length = ((Integer)session.getAttribute(
            "com.integrosys.cms.ui.creditriskparam.entitylimit.EntityLimitAction.length")).intValue();
...
function goPageIndex(targetOffset) {
	<% if (forClose) { %>
		document.forms[0].event.value = "<%=EntityLimitListAction.EVENT_LIST_MAKER_CLOSE%>"
	<% } else if (forRead) { %>
		document.forms[0].event.value = "<%=EntityLimitListAction.EVENT_LIST_READ%>"
	<% } else { %>
		document.forms[0].event.value = "<%=EntityLimitListAction.EVENT_LIST_VIEW%>"
	<% } %>
...
function editForm()
{
	document.forms[0].event.value = "<%=EntityLimitListAction.EVENT_READ_MAKER_EDIT%>";
	document.forms[0].submit();
...
<thead>
	<tr>
      <td height="34" width="70%"><h3><bean:message key="title.crp.entitylimit"/></h3></td>
	...
	<thead>
	       <tr>
	           ...
	           <td width="20%"><bean:message key="label.crp.limit.last.review.date"/></td>
	       </tr>
	</thead> 
	<tbody>
		<%
		...
		<%
			...
			//System.out.println("entityLimit : " + entityLimit);
	...
	<table id="tblExchange" width="100%" border="0" cellpadding="0" cellspacing="0" class="tblinfo" style="margin-top:0">
		...
		<tr class="odd">
          <td colspan="2" class="fieldname"><bean:message key="label.last.date.update"/></td>
