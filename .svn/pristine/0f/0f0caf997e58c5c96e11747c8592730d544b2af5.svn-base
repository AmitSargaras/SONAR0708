...
<%@ page import="com.integrosys.cms.ui.collateral.others.RankList"%>
	  ...
	  <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInput">
		  <thead>
			<tr>
			  ...
			  <td width="9%"><bean:message key="label.security.rank.sec"/></td>
			  <td width="16%"><bean:message key="label.security.prior.charge.amount"/></td>
			  <td width="25%"><bean:message key="label.security.charge.prior.charge"/></td>
			  <td width="17%"><bean:message key="label.security.amount.charge"/></td>
			  <td width="8%"><bean:message key="label.global.action"/></td>
			</tr>
		  </thead>
		  <tbody>
			<% if (from_event.equals("process")) {
                List res = CompareOBUtil.compOBArray(newCollateral.getLimitCharges(),oldCollateral.getLimitCharges());
                ...
                if (list == null || list.size() == 0) { %>
                    <tr class="odd">
                    <td colspan="7"><bean:message key="label.security.charge.infor.none"/></td>
                    </tr>
<%                }
%>
		<logic:present name="limit" >
           ...
           %>
                    <tr class="<%=classtype%>">
                      ...
                      %>
                      <% if (i > 0) { %> <br> <% } %>
                      <%=limitID%>&nbsp;(<%=limitMap[i].getSCISysGenID()%>)&nbsp;
                      <% if (limitMap[i].getLimitType() != null) { %>
                      (<integro:empty-if-null value="<%=CommonDataSingleton.getCodeCategoryLabelByValue(CategoryCodeConstant.PRODUCT_DESCRIPTION, limitMap[i].getLimitType())%>"/>)
                      <% }
