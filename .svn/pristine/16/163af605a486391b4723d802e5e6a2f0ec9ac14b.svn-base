...
<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
    ...
    <td> 
        <% if (propertyValuation1.getEnvironmentRiskyStatus() != null) { %>
            <integro:empty-if-null value="<%=SecEnvRiskyList.getInstance().getSecEnvRiskyItem(propertyValuation1.getEnvironmentRiskyStatus().trim())%>"/>
        <%} else{ %>
            -
        <% }%>
    </td>
    ...
    <td><integro:date object="<%=propertyValuation1.getEnvironmentRiskDate()%>"/>&nbsp;</td>
</tr>
...
<script language="JavaScript" type="text/JavaScript">
   ...
   <%
    String EnvRiskyStatus = propertyValuation1.getEnvironmentRiskyStatus() ;
