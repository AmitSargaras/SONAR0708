...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'toTrackImageLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=maker_totrack_view_image&category="+catg;
		   document.forms[0].event.value="maker_totrack_view_image";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'toTrackImageLADDoc'){
			 document.forms[0].action="LADReceipt.do?event=toTrackImageLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
