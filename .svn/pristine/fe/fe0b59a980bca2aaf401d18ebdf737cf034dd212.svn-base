...
function submitForm() {
		document.forms[0].action = "BankingArrangementFacExclusion.do?event=maker_create";
		document.forms[0].submit();
}

function saveForm() {
		document.forms[0].action = "BankingArrangementFacExclusion.do?event=maker_draft";
		document.forms[0].submit();
}
...
<html:form action='BankingArrangementFacExclusion.do'>
	...
	<script>
		<%if("Y".equals(isNew)){%>
			document.all.excluded[0].checked=true;
		<%}%>
	</script>
</html:form>
</html>