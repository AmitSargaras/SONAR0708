...
<%
	int rowCount=0;
IFileUploadTrxValue trxValueOut=(IFileUploadTrxValue)session.getAttribute("com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsUploadAction.trxValueOut");
	...
	Set errDetLog=null;
	List totalUploadedList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsUploadAction.totalUploadedList");
	if(totalUploadedList!=null)
		...
		List errorList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsUploadAction.errorList");
	if(errorList!=null)
		pageContext.setAttribute("errorList",errorList);
	else
		pageContext.setAttribute("errorList",new ArrayList());
		
		List<FacilitydetailsError> finalList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsUploadAction.finalList");
...
<%@page import="com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsError"%><head>
...
function downloadImage(){ //v3.0
		var aLocation = "Facilitydetailsupload.do?event=downloadReport&system="+'<%=system%>';
...
function goBack(){
	document.forms[0].action = "Facilitydetailsupload.do?event=excelupload";		
	document.forms[0].submit(); 
}
...
<html:form action="Facilitydetailsupload.do" >
...
<thead>
   <%if(finalList.size()>=1){ %>
<h3>Facility Update Upload Error</h3>
<%}else{ %>
<h3>Facility Update Upload</h3>
<%} %>
<br></br>
</thead>
<tbody>
...
<table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                     ...
                     <logic:iterate id="ob"  offset="<%=String.valueOf(startIndex)%>"  indexId="counter"  name="finalList" type="com.integrosys.cms.ui.facilitydetailsupload.FacilitydetailsError" scope="page" length="100">
                     <%
					String rowClass = "";
					int i =0;
					if (rowCount % 2 != 0) {
