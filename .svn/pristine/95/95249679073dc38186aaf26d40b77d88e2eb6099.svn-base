 <%@ page import=" com.integrosys.cms.ui.diaryitem.DiaryItemForm,
...
<%
     ...
     * $Header: /home/cms2/cvsroot/cms2/public_html/diaryitem/view_full_diary_listing.jsp,v 1.21 2006/09/29 02:20:08 hshii Exp $
     * @author $Author: hshii $
...
<%
    SearchResult searchResult = (SearchResult)request.getAttribute("DiaryItemList");

    if (searchResult == null)
        searchResult = (SearchResult)session.getAttribute("com.integrosys.cms.ui.diaryitem.DiaryItemAction.session.DiaryItemList");
...
function goNextPrev(ind) {
    ...
    document.forms["DiaryItemForm"].frompage.value='list_non_expired';
    document.forms["DiaryItemForm"].event.value='list_non_expired';
}
...
function goAlphaIndex(ind) {
    ...
    document.forms[0].allowedCountry.value='<%=DiaryItemListMapper.NO_FILTER%>';

    document.forms[0].startExpiryDate.value='';
    document.forms[0].endExpiryDate.value='';
...
function goPage(pageNo){
    document.forms["DiaryItemForm"].event.value='list_non_expired';
    document.forms[0].startIndex.value = pageNo*<%=recordsPerPage%>;
}

function JumpOnSelectactive(selectObj)
{
    var selectIndex = selectObj.selectedIndex;
    {
        if ( selectObj.options[selectIndex].value != '<%=DiaryItemListMapper.NO_SELECT%>' ) {
            //document.forms["DiaryItemForm"].countryFilter.value=selectObj.options[selectIndex].value;
            location.href = "DiaryItem.do?event=list_non_expired"+"&countryFilter="+selectObj.options[selectIndex].value
        }
    }
}
...
function submitForm() {
    clearToken();    
    var strURL = 'ReportServlet?event=generateDiaryReport&flag=view'+
    			'&startExpiryDate='+document.forms[0].startExpiryDate.value+
    			'&endExpiryDate='+document.forms[0].endExpiryDate.value+
    			'&searchLeID='+document.forms[0].searchLeID.value+
    			'&searchCustomerName='+document.forms[0].searchCustomerName.value+
    			'&customerIndex='+document.forms[0].customerIndex.value;
    displayReportFrame.location=strURL;
    
/*    
	document.forms[0].action = 'ReportServlet';
	document.forms[0].event.value='generateDiaryReport';
	document.forms[0].submit();
*/
	
	document.forms[0].action = 'DiaryItem.do';
	document.forms[0].event.value = 'list_non_expired';
	    
    //document.forms[0].event.value='print';
    //document.forms[0].submit();
}
...
/*function refreshPage() {
	 ...
	 if(document.forms[0].searchCustomerName.value=='null')
	 {
	    ...
	    document.forms[0].searchLeID.value='';
	    if(document.forms[0].searchLeID.value=='null')
	 {
	     document.forms[0].searchLeID.value='';
	 }
                       ...
                       <TABLE width="70%" class=tblinfo id=custsearch border="0" align="left" cellpadding="0" cellspacing="0" style="margin-top:15px">
                         <THEAD>
                          ...
                          <TR class=odd>
                            ...
                            <TD><html:text property="searchLeID" size="50" maxlength="30" onkeypress="return handleEnter(this, window.event||event, 1);" /> 
                          ...
                          <TR class=even>
                            ...
                            <TD><html:text property="searchCustomerName" size="50" maxlength="50" onkeypress="return handleEnter(this, window.event||event, 2);" />  
            ...
            <tr >
                <td colspan="3" valign="top" style="height:100%;width:100%;">
                    <table width="90%" class="tblFormSection" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:15px">
                      ...
                      <tr>
                        <td width="24%">
                          <h3>All Diary Item List</h3>
                        </td>
                  <td width="76%" valign="bottom">
<!--                         <strong>Access Country:</strong>-->
                         <html:hidden property="allowedCountry" onchange="JumpOnSelectactive(this)"/>
                ...
                <logic:present name="diaryList">
                   ...
                   <%
                       ...
                       String itemURL = "DiaryItem.do?frompage=list_non_expired&itemId="+OB.getItemID();
...
<!--                <img src="img/refresh1.gif" name="ImageRefresh" width="80" height="20" border="0" id="ImageRefresh" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('ImageRefresh','','img/refresh2.gif',1)" onclick="refreshPage()"/>-->
              ...
              <!--//Commented as Index disappears if no items for a selected Index -->
              <%--<logic:present name="diaryList">--%>
              <% //if (diaryList != null && diaryList.size() > 0) { %>
              <% String alphaURL="DiaryItem.do?event=list_non_expired&frompage=list_non_expired";
                 if (searchResult != null) {
                     DefaultLogger.debug(this, "SearchResult.getNumItems():" +searchResult.getNItems());
                     System.out.println("SearchResult.getNumItems():" +searchResult.getNItems());
                     DefaultLogger.debug(this, "SearchResult.:" +searchResult.getCurrentIndex());
                     System.out.println("SearchResult.:" +searchResult.getCurrentIndex());
                 } else
                     DefaultLogger.debug(this, "SearchResult is empty!");
              %>
              <tr>
                <td colspan="3">
                    ...
                    <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                      <tr>
                         <td width="671" valign="middle" id="orgpanel0">
                            Index: <integro:alphaindex url='<%=alphaURL%>' submitFlag='<%=new Boolean(true)%>' />
                         </td>
                        <td width="3" valign="middle"><img src="img/icon/seprate_bar.gif" /></td>
                        <% if (searchResult != null) { %>
                         <td width="94" valign="middle" id="orgpanel">
                           <integro:pageindex pageIndex='<%=new PageIndex(lStartIndex, recordsPerPage, noofItems)%>' submitFlag='<%=new Boolean(true)%>' url='<%=alphaURL+"&startIndex="%>' />
                         </td>
                         <% } %>
                      </tr>
                    </table>
...
<iframe name='displayReportFrame' width='0' height='0' frameborder='0' scrolling='no'></iframe>
</body>
</html>
