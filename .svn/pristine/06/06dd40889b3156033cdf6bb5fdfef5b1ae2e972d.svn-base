...
<%
	...
	IFileUploadTrxValue itrxValue = (IFileUploadTrxValue) session
			.getAttribute("com.integrosys.cms.ui.fileUpload.FileUploadAction.IFileUploadTrxValue");
	List ubsList = (List) session
			.getAttribute("com.integrosys.cms.ui.fileUpload.FileUploadAction.ubsList");
	;
	if (ubsList != null) {
	...
	DefaultLogger.debug(this,"ubsList size : "+ ubsList.size());
	
	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> itrxValue>"
			+ itrxValue.getTransactionSubType());
	int sno = 0;
	String heading = "";
	String approve_event = "";
	if ("UBS_UPLOAD".equals(itrxValue.getTransactionSubType())) {
		heading = "UBS File Upload";
		approve_event = "checker_approve_create_ubs";
	} else if ("HONGKONG_UPLOAD".equals(itrxValue
			.getTransactionSubType())) {
		heading = "HongKong File Upload";
		approve_event = "checker_approve_create_hongkong";
	} else if ("FINWARE_UPLOAD".equals(itrxValue
			.getTransactionSubType())) {
		heading = "Finware File Upload";
		approve_event = "checker_approve_create_finware";
	} else if ("BAHRAIN_UPLOAD".equals(itrxValue
			.getTransactionSubType())) {
		heading = "Bahrain File Upload";
		approve_event = "checker_approve_create_bahrain";
	}else if ("FD_UPLOAD".equals(itrxValue
			.getTransactionSubType())) {
		heading = "FD File Upload";
		approve_event = "checker_approve_create_fd";
	}
%>
...
function approvePage() {
	var x = document.getElementById("confirm").value; 
		if (x == 'Y') {
		    document.forms[0].action="fileUpload.do?event="+'<%=approve_event%>';
...
function checkbox(){
	...
	if(document.getElementById("confirm").value=='N')
	{
...
function rejectPage() {
    document.forms[0].action="fileUpload.do?event=checker_reject_create";
    document.forms[0].submit();
}
function cancelPage(){
