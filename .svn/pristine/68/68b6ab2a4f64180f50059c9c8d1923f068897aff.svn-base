    ...
    boolean displayDateFSV = true;    // for MBB-1391 by jitu
        ...
        if (securityType !=  null && !"".equals(securityType)) {
            if (securityType.startsWith("Asset")){
                ...
                //System.out.println(" Sub types not  valid , " + securityType);
            }
        }

    IValuation[] iValuationArray = iCol.getValuationFromLOS();
   /* System.out.println("View ValuationFromLOS ==> Coll Action [" + formActionName+ "], " +
            "subtype [" + subtype+ "], " +
            "valuationFromLOSAction [" + valuationFromLOSAction+ "], " +
            ", from_event[" + from_event+"]");
            
       */       
       System.out.println("****event:::" + from_event);       
%>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInput">
                    <thead>
                        <tr>
                            ...
                            <td><span class="fieldname"><bean:message key="label.valuation.valuer" /></span></td>
                            <td><span class="fieldname"><bean:message key="label.valuation.omv.amount" /></span></td>
<!--                             <%if (!displayDateFSV){ %>
                              <td><span class="fieldname"><bean:message key="label.valuation.fsv.amount"/> </td>
                            <% } %>-->
                            <td><span class="fieldname"><bean:message key="label.valuation.date.omv" /></span></td>
                            <td><span class="fieldname"><bean:message key="label.valuation.date.received" /></span></td>
<!--                            <%if (displayDateFSV){ %>
                            <td><span class="fieldname"><bean:message key="label.valuation.date.of.fsv"/> </td>
                           <% } %>-->
                            <td><span class="fieldname"><bean:message key="label.global.action" /></span></td>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="odd">
                            <td colspan="7">
                                <bean:message key="label.security.no.valuation.available.frm.los" />
                            </td>
                        </tr>
                        <% } else {
