...
if(propertyValuation3.getMonitorProcess()!=null && !propertyValuation3.getMonitorProcess().equals("")){	
	if(propertyValuation3.getMonitorProcess().equals("Y")){	
		monitorProcess="Yes";
	}else if(propertyValuation3.getMonitorProcess().equals("N")){		
		monitorProcess="No";
	}
...
<%@page import="java.util.HashMap"%><tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
	...
	<td width="30%">
		<integro:empty-if-null value="<%= ((propertyValuation3.getCollateralId() == ICMSConstant.LONG_INVALID_VALUE ) ? null : String.valueOf(propertyValuation3.getCollateralId()))%>"/> 
<%--
      <% 
...
--%>
	...
	<td width="30%"><integro:empty-if-null value="<%=propertyValuation3.getSciRefNote()%>"/>&nbsp;
    </td>	
...
<%--Govind S: Commented as required for HDFC Dev 04/07/2011
	...
	<td width="30%"><integro:empty-if-null value="<%=propertyValuation3.getSCIReferenceNote()%>"/>&nbsp;
...
<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
  	...
  	<td width="30%" ><integro:empty-if-null value="<%=propertyValuation3.getTypeName()%>"/></td>
  	<td width="20%" class="fieldname">
  		<bean:message key="label.security.sub.type"/></td>
  	<td width="30%"><integro:empty-if-null value="<%=propertyValuation3.getSubTypeName()%>"/></td>
</tr>       
...
<%--<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
  	...
  	<td width="30%">
		integro:common-code categoryCode="<%= ICMSConstant.COMMON_CODE_SOURCE_CMS_SEC_TYPE_MAP%>" refEntryCode="<%= propertyValuation3.getSCISubTypeValue()%>" descWithCode="true" entryCode="<%=propertyValuation3.getSourceSecuritySubType() %>" display="true"/>
	<integro:empty-if-null value="<%= propertyValuation3.getCollateralCode()%>"/>
	</td>
	<td width="20%" class="fieldname">
              		<bean:message key="label.security.asset.priority"/></td>
              	<td width="30%" colspan="3">
              	<%if(propertyValuation3.getSecPriority()!=null){ %>
              	 <%=propertyValuation3.getSecPriority().equalsIgnoreCase("y")?"Primary":"Secondary"%>&nbsp;
              	 <%} else { %>
              	 -
              	 <%} %>
   </td>
...
--%>
<%
	if (!ICMSConstant.COLTYPE_NOCOLLATERAL.equals(propertyValuation3.getSubTypeName())) {
%>
<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
<%--Govind S: Commented as required by HDFC Dev 04/07/2011 
  	<td class="fieldname"><bean:message key="label.security.ori.currency"/> </td>
  	<td><integro:empty-if-null value="<%=propertyValuation3.getSCICurrencyCode()%>" />&nbsp;</td>
  	--%>
  	<td width="20%"class="fieldname">
 ...
 <td>
  <%String brCode = "";
	if(propertyValuation3.getSecurityCurrency()!=null){
		brCode = propertyValuation3.getSecurityCurrency().trim();
	}
	 %>
  <html:select property="collateralCurrency" value="<%=brCode.trim()%>" disabled="true" >
  	...
  	<td width="30%" ><integro:empty-if-null value="<%=propertyValuation3.getSecurityCurrency()%>" />&nbsp;</td>
--%><td width="20%" class="fieldname">
...
<%if(propertyValuation3.getMonitorFrequency()!=null && !propertyValuation3.getMonitorFrequency().equals("")){ %>
            <td><integro:common-code-single entryCode="<%=propertyValuation3.getMonitorFrequency()%>"  display="true"
...
<%--<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
	...
	<td width="30%"><integro:empty-if-null value="<%=CommonDataSingleton.getCodeCategoryLabelByValue(CategoryCodeConstant.COLLATERAL_STATUS,propertyValuation3.getCollateralStatus())%>" />&nbsp;</td>
...
<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
	...
	<td width="30%"><%--<integro:empty-if-null value="<%=CountryList.getInstance().getCountryName(propertyValuation3.getCollateralLocation())%>" />&nbsp; --%>
	...
	<td width="30%">
	<%String collCode = "";
	System.out.println("propertyValuation3.getCollateralCode():"+propertyValuation3.getCollateralCode());
	if(propertyValuation3.getCollateralCode()!=null){
	collCode = propertyValuation3.getCollateralCode();
	}
...
<tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
	...
	<td >
		<%if(propertyValuation3.getSecOrganization()!=null && orgMap != null){ %>
		<integro:empty-if-null value="<%=orgMap.get(propertyValuation3.getSecOrganization())%>" />&nbsp;(<integro:empty-if-null value="<%=propertyValuation3.getSecOrganization()%>" />)

		<%}else{%>-<%} %>
		&nbsp; </td>
		
		
		<%	if (ICMSConstant.COLTYPE_ASSET_GENERAL_CHARGE.equals(propertyValuation3.getSubTypeName())) { %>
	...
	<td class="fieldname">Type of Charge<font color="#FF0000">*
    	...
    	<td>
            <html:select property ="typeOfCharge" styleId="typeOfCharge" value ="<%=propertyValuation3.getChangeType()%>" disabled="true">
