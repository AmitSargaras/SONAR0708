...
<%
     ...
     * $Header: /home/cms2/cvsroot/cms2/public_html/reassignment/reassignment_ccc.jsp,v 1.4 2006/10/27 08:47:01 hmbao Exp $
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection"  >
    <thead>
    <tr>
        <td><h3>Pending Multi-level Approval C/C Checklist Found</h3></td>
    </tr>
    </thead>
    <tbody>
    <tr>
        <td>
            <table  width="100%" border="0" cellpadding="0" cellspacing="0" class="tblinfo" style="margin-top:0">
                ...
                <tr>
                    ...
                    <td width="12%">Documentation Originating Location</td>
                    <td width="13%">Organisation Code</td>
                    <td width="13%">Governing Law</td>
                ...
                <tbody>
                <%
                    CCCheckListSummary checklist = (CCCheckListSummary) summaryList.get(0);
                    ICheckListTrxValue trxValue = checklist.getTrxValue();
                    String countryName = CountryList.getInstance().getCountryName(checklist.getDomicileCtry());
                    String orgCode = "-";
                    if (checklist.getOrgCode() != null) {
                        orgCode = checklist.getOrgCode();
                        orgCode = CommonDataSingleton.getCodeCategoryLabelByValue(ICMSConstant.ORG_CODE,orgCode);
                    }
                %>
                <tr class="odd">
                <td valign="top">
                    <integro:empty-if-null value="<%=checklist.getCustCategory()%>"/>&nbsp;
                </td>
                <% if ((checklist.getLegalID() == null) || (checklist.getLegalID().equals("0"))) { %>
                <td valign="top">&nbsp; -
                <% } else { %>
                <td valign="top" >&nbsp;<%=checklist.getLegalID()%>
                <% } if (checklist.getPledgorReference() != null) { %>
                <br> <%= checklist.getPledgorReference()%> <% } %>
                </td>
                <td valign="top">
                    <integro:empty-if-null value="<%=checklist.getLegalName()%>"/>&nbsp;
                </td>
                <td valign="top"><integro:empty-if-null value="<%=countryName%>" />&nbsp;</td>
                <td valign="top"><integro:empty-if-null value="<%=orgCode%>" />&nbsp;</td>
                <td valign="top"><integro:empty-if-null value="<%=checklist.getGovernLaw()%>"/>&nbsp;</td>
                <td valign="top"><integro:empty-if-null value="<%=checklist.getCheckListStatus()%>"/>&nbsp;</td>
                <td valign="top">
                <a href="#" onclick="submitPage(0, '<%=checklist.getTrxID()%>', '<%=trxValue.getCustomerID()%>', '<%=trxValue.getLimitProfileID()%>')">View</a>
                </td>
                </tr>
                </tbody>
            </table>
                    ...
                    <td class="even" style="text-align:center" valign="middle">
                        ...
                        <tr class="even">
                            <td style="border:none;text-align:center">
                                ...
                                Sorry, there are no pending multi-level approval C/C Checklist records found.
                                </font></strong>
                            </td>
                        </tr>
                        </table>
