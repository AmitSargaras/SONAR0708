...
function submitForm() {
 	document.forms[0].action="MIPartyGroup.do?event=maker_confirm_resubmit_edit";
    document.forms[0].submit();
}
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tbody>
						...
						<tr class="odd">
							...
							<td class="fieldname" width="30%">Party Group Code<font color="red">*</font></td>
							...
							<td width="20%">
							<%
								if (partyGroupTrx.getFromState().equals("PENDING_ENABLE")
											|| partyGroupTrx.getFromState()
													.equals("PENDING_DELETE")) {
							%>
							<bean:write name="MIPartyGroupForm" property="partyName" />&nbsp;
							<%
								} else {
							%> <html:text name="MIPartyGroupForm" property="partyName" size="50" maxlength="100"></html:text>&nbsp;
							<%
								}
							%>
							...
							<%
								if (partyGroupTrx.getFromState().equals("PENDING_ENABLE")
											|| partyGroupTrx.getFromState()
													.equals("PENDING_DELETE")) {
							%>
							<bean:write name="MIPartyGroupForm" property="groupExpLimit" />&nbsp;
							<%
								} else {
							%> <html:text name="MIPartyGroupForm"
								property="groupExpLimit" size="30" maxlength="20" styleId="groupExpLimit" onblur="javascript:formatAmountAsCommaSeparated(this)"></html:text>&nbsp; <%
 	}
 %>
