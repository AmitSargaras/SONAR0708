			...
			<td valign="top">
				<table width="97%" border="0" align="center" cellpadding="0"
					...
					<thead>
						<tr>
							<td width="70%">
							<h3><bean:message key="label.facility.reduction" /></h3>
							</td>
				...
				<table width="97%" border="0" align="center" cellpadding="3" cellspacing="0" class="tblInput">
					...
					<%
							...
							if (FacilityMainAction.EVENT_LIST_WO_FRAME.equals(event)) {
								...
								labelAction = "label.edit";
							} else if (FacilityMainAction.TAB_FACILITY_REDUCTION_CHECKER.equals(event)
								...
								nextEvent = FacilityMainAction.EVENT_VIEW_DETAIL_CHECKER;
 										...
 										|| FacilityMainAction.TAB_FACILITY_REDUCTION_VIEW_WO_FRAME.equals(event)) {
								nextEvent = FacilityMainAction.EVENT_VIEW_DETAIL_ONLY;
							} else if (FacilityMainAction.EVENT_NAVIGATE_VIEW.equals(event)
										|| FacilityMainAction.TAB_FACILITY_REDUCTION_VIEW.equals(event)) {
					...
					%>
						<logic:empty name="facilityReductions">
							<tr class="odd">
								<td colspan="8"><bean:message
									key="label.global.not.found" /></td>
							</tr>
						</logic:empty>
						<logic:notEmpty name="facilityReductions">
							<%
								if (!isProcessHere) {
							%>													
							<logic:iterate id="reduction" name="facilityReductions"
								type="com.integrosys.cms.app.limit.bus.OBFacilityReduction"
								indexId="indexId">
								<tr class="<%=style[(indexId.intValue())%2] %>">
									<td class="index"><%=(indexId.intValue() + 1 + startIdx)%></td>
									<td><%= reduction.getIncrementalNumber()!=null?reduction.getIncrementalNumber().toString():"-" %></td>
									<td><integro:date object="<%=reduction.getDateEntered() %>"></integro:date>&nbsp;</td>
									<td><integro:empty-if-null value="<%=UIUtil.mapNumberObjectToString(reduction.getIncrementalLimit(), locale, 2) %>"/></td>
									<td><integro:empty-if-null value="<%=UIUtil.mapNumberObjectToString(reduction.getOriginalLimit(), locale, 2) %>"/></td>
									<td><integro:common-code categoryCode="<%=CategoryCodeConstant.FACILITY_STATUS %>" entryCode="<%=reduction.getFacilityStatus() %>" display="true"/> &nbsp;</td>
									<td><integro:common-code categoryCode="<%=CategoryCodeConstant.LMT_STATUS %>" entryCode="<%=reduction.getDocumentationStatusEntryCode() %>" display="true"/> &nbsp;</td>
							...
							<% } else { %>
							<logic:iterate id="result" name="facilityReductions"
								type="com.integrosys.base.techinfra.diff.CompareResult"
								indexId="indexId">		
								<%
									OBFacilityReduction reduction = (OBFacilityReduction)result.getObj();
								%>
								<tr class="<%=style[(indexId.intValue())%2] %>">
									...
									<%
										...
										if (isProcessHere){
											keyStr = reduction.getCmsReferenceId().toString();
										}
									%>
		...
		<table id="generalPurposeBar" width="100%" border="0" cellspacing="0"
			...
			<tr>
				...
				<%
					...
					String pageindexUrl = "FacilityReduction.do?event="+listEvent+"&startIdx=";
				%> 
				<integro:pageindex
