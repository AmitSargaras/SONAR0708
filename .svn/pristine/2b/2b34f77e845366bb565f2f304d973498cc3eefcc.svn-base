...
function returnPage() {
	document.forms[0].action="LADReceipt.do?event=prepare_totrack_view_lad_return";
	document.forms[0].submit();
}
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="LADReceipt.do?event=closeToTrackImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
...
<p align="center">
 <%   if ("mkr_view_totrack_img".equals(event)) { %>
