/* Patch application date = approval date  - EONCMSPRD-1104 */

drop table PATCH_APPLICATION_DATE_MO;

CREATE TABLE PATCH_APPLICATION_DATE_MO (
...
and lsm.APPLICATION_TYPE = 'MO'
and lmt.CIF_ID not in ('60041074', '80085439', '180053013');

-- 380 Records
select count(*) from PATCH_APPLICATION_DATE_MO pth, cms_facility_master facmas, cms_fac_general facgen
where pth.CMS_LSP_APPR_LMTS_ID = facmas.CMS_LSP_APPR_LMTS_ID
and facmas.ID = facgen.CMS_FAC_MASTER_ID;

-- 380 Records
select count(*) from PATCH_APPLICATION_DATE_MO pth, cms_stg_facility_master facmas, cms_stg_fac_general facgen
where pth.CMS_LSP_APPR_LMTS_ID = facmas.CMS_LSP_APPR_LMTS_ID
and facmas.ID = facgen.CMS_FAC_MASTER_ID;

-------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Insert into temporary patching table
insert into PATCH_APPLICATION_DATE_MO (CIF_ID, AA_NUMBER, LOS_AA_NUMBER, LIMIT_ID, APPROVED_DATE, APPLICATION_DATE)
(select lmt.CIF_ID, lmt.AA_NUMBER, lmt.LOS_AA_NUMBER, lmt.LIMIT_ID, lmt.APPROVED_DATE, lmt.APPLICATION_DATE 
	from MIG_CO019_PL_TEMP lsm, MIG_CA003_1_TEMP lmt  
	where lsm.LIMIT_ID = lmt.LIMIT_ID    
	and lsm.APPLICATION_NO = lmt.AA_NUMBER 
...
insert into PATCH_APPLICATION_DATE_MO (CIF_ID, AA_NUMBER, LOS_AA_NUMBER, LIMIT_ID, APPROVED_DATE, APPLICATION_DATE)
(select lmt.CIF_ID, lmt.AA_NUMBER, lmt.LOS_AA_NUMBER, lmt.LIMIT_ID, lmt.APPROVED_DATE, lmt.APPLICATION_DATE 
	from MIG_CO019_PL lsm, MIG_CA003_1 lmt  
	where lsm.LIMIT_ID = lmt.LIMIT_ID    
	and lsm.APPLICATION_NO = lmt.AA_NUMBER 
	and lmt.APPROVED_DATE < lmt.APPLICATION_DATE 
	and lmt.APPROVED_DATE is not null  
...
update PATCH_APPLICATION_DATE_MO pth
...
set APPLICATION_DATE = (select pth.APPROVED_DATE
                        from PATCH_APPLICATION_DATE_MO pth, CMS_FACILITY_MASTER facmas 
                        where pth.CMS_LSP_APPR_LMTS_ID = facmas.CMS_LSP_APPR_LMTS_ID
                        and facmas.ID = facgen.CMS_FAC_MASTER_ID fetch first row only)
where exists (select 1 from PATCH_APPLICATION_DATE_MO pth1, CMS_FACILITY_MASTER facmas1
...
set APPLICATION_DATE = (select pth.APPROVED_DATE
                        from PATCH_APPLICATION_DATE_MO pth, CMS_STG_FACILITY_MASTER facmas
                        where pth.CMS_LSP_APPR_LMTS_ID = facmas.CMS_LSP_APPR_LMTS_ID
                        and facmas.ID = facgen.CMS_FAC_MASTER_ID fetch first row only)
where exists (select 1 from PATCH_APPLICATION_DATE_MO pth1, CMS_STG_FACILITY_MASTER facmas1
                where pth1.CMS_LSP_APPR_LMTS_ID = facmas1.CMS_LSP_APPR_LMTS_ID
                and facmas1.ID = facgen.CMS_FAC_MASTER_ID);
