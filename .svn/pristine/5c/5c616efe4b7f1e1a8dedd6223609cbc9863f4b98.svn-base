...
function submitPage () {
	document.forms[0].action="Region.do?event=maker_confirm_resubmit_edit_region";
    document.forms[0].submit();
}
		...
		<thead>
			<tr>
				<td>
					<br>
					<h3>
							<%if( regionTrx.getFromState().equals("PENDING_DELETE") ){ %>
								Disable Region
						<%} else if( regionTrx.getFromState().equals("PENDING_ACTIVATE") ){ %>
								Enable Region
						<%} else { %>
								Region
						<%} %>
			...
			<tr>
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tbody>	
						<%
							String fromState = regionTrx.getFromState();
							if( fromState.equals("PENDING_ACTIVATE") || fromState.equals("PENDING_DELETE") )
							{
						%>
						<tr class="odd">	
							<td class="fieldname" width="20%">Region Code</td>
	                        <td class="even" width="20%"><bean:write property="regionCode" name="RegionForm"/></td>
							
							<td class="fieldname" width="20%">Region Name</td>
	                        <td class="even" width="20%"><bean:write property="regionName" name="RegionForm"/></td>
						</tr>
...
<html:hidden  name="RegionForm" property="countryOBId" />
