...
'CUST-ALLOC-0013', 'Beauty Beauty Salon', 'Beauty Beauty Salon', 
'56', 'C', 'MY', 'CUST-ALLOC-0013-BUS', 'ARBS');
...
from sci_le_main_profile where lmp_le_id = 'CUST-ALLOC-0013';
...
lsp_le_id, lsp_id, '80000013', 'ALLOC-AA-0013', cms_le_sub_profile_id, 'ACTIVE', 
'MY', '2', 'ARBS', 'N', 'BANKING', 'Y', 'Y', 'CO', 'CON'
from sci_le_sub_profile where lsp_le_id = 'CUST-ALLOC-0013';
...
where llp_le_id = 'CUST-ALLOC-0013';
...
cms_lsp_lmt_profile_id, '700000026', '27', 'HP',
'25', 'OUTER', 300000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 300000000, '210', 1, 'FAC-ALLOC-0026', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0013';
...
account_type, lmt_bca_ref_num, los_bca_ref_num)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for limit_seq), 2)) AS bigint), 
cms_lsp_lmt_profile_id, '700000027', '27', 'HP',
'25', 'OUTER', 200000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 200000000, '210', 1, 'FAC-ALLOC-0027', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0013';
...
account_type, lmt_bca_ref_num, los_bca_ref_num)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for limit_seq), 2)) AS bigint), 
cms_lsp_lmt_profile_id, '700000028', '27', 'HP',
'25', 'OUTER', 500000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 500000000, '210', 1, 'FAC-ALLOC-0028', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0013';
...
where los_lmt_id in ('FAC-ALLOC-0026','FAC-ALLOC-0027', 'FAC-ALLOC-0028') ;
...
'--', '--', '--', '-999999999', -999999999, -999999999
from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0026','FAC-ALLOC-0027', 'FAC-ALLOC-0028');
...
'AB101', 'AB101-ALLOC-0002', 'AB101-ALLOC-0002', 'MY', subtype_name, security_type_name, 
'AB101', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
500000000, 'MYR', 400000000, 'MYR'
from cms_security_sub_type where security_sub_type_id = 'AB101';

insert into cms_asset (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0002';	
...
cmv, cmv_currency, fsv, fsv_currency)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for collateral_seq), 2)) AS bigint), 
'AB101', 'AB101-ALLOC-0003', 'AB101-ALLOC-0003', 'MY', subtype_name, security_type_name, 
'AB101', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
400000000, 'MYR', 300000000, 'MYR'
from cms_security_sub_type where security_sub_type_id = 'AB101';

insert into cms_asset (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0003';	
...
cmv, cmv_currency, fsv, fsv_currency)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for collateral_seq), 2)) AS bigint), 
'AB101', 'AB101-ALLOC-0004', 'AB101-ALLOC-0004', 'MY', subtype_name, security_type_name, 
'AB101', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
600000000, 'MYR', 500000000, 'MYR'
from cms_security_sub_type where security_sub_type_id = 'AB101';

insert into cms_asset (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0004';	
...
sci_orig_security_currency, security_organisation,
cmv, cmv_currency, fsv, fsv_currency
from cms_security where sci_security_dtl_id in ('AB101-ALLOC-0002','AB101-ALLOC-0003','AB101-ALLOC-0004');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	300000000, 'MYR', 1, 'G', s.cms_collateral_id,
	CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint),
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'AB101-ALLOC-0002';

insert into cms_charge_detail 
	(charge_detail_id, charge_amount, charge_currency_code, security_rank, charge_type, cms_collateral_id, 
	cms_ref_id, status, prior_charge_type, source_id, is_legal_enforce, caveat_waived_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	200000000, 'MYR', 2, 'S', s.cms_collateral_id,
	CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint),
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'AB101-ALLOC-0003';

insert into cms_charge_detail 
	(charge_detail_id, charge_amount, charge_currency_code, security_rank, charge_type, cms_collateral_id, 
	cms_ref_id, status, prior_charge_type, source_id, is_legal_enforce, caveat_waived_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	400000000, 'MYR', 3, 'G', s.cms_collateral_id,
	CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint),
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'AB101-ALLOC-0004';
...
and s.sci_security_dtl_id in ('AB101-ALLOC-0002','AB101-ALLOC-0003','AB101-ALLOC-0004');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0002'), 'AB101-ALLOC-0002',
	cms_lsp_appr_lmts_id, lmt_id, cms_limit_profile_id, 'MB', current timestamp , 'I'
from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0026', 'FAC-ALLOC-0027');	

insert into cms_limit_security_map
(charge_id, cms_collateral_id, sci_las_sec_id, cms_lsp_appr_lmts_id, sci_las_lmt_id, cms_lsp_lmt_profile_id, 
customer_category,create_date, update_status_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0003'), 'AB101-ALLOC-0003',
	cms_lsp_appr_lmts_id, lmt_id, cms_limit_profile_id, 'MB', current timestamp , 'I'
from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0028');	
...
customer_category,create_date, update_status_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB101-ALLOC-0004'), 'AB101-ALLOC-0004',
	cms_lsp_appr_lmts_id, lmt_id, cms_limit_profile_id, 'MB', current timestamp , 'I'
from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0026', 'FAC-ALLOC-0027','FAC-ALLOC-0028');	
...
-- limit charge for charge rank 1
...
from cms_limit_security_map lsm, cms_security s, cms_charge_detail dtl, sci_lsp_appr_lmts lmts
where lsm.cms_collateral_id = s.cms_collateral_id
and s.cms_collateral_id = dtl.cms_collateral_id
and dtl.security_rank = 1
and s.sci_security_dtl_id in ('AB101-ALLOC-0002')
and lmts.cms_lsp_appr_lmts_id = lsm.cms_lsp_appr_lmts_id
and lmts.los_lmt_id in ('FAC-ALLOC-0026', 'FAC-ALLOC-0027');

-- limit charge map for charge rank 2
insert into cms_limit_charge_map
(limit_charge_map_id, cms_lsp_appr_lmts_id, charge_id, charge_detail_id, cms_collateral_id,status, 
	customer_category, cms_lsp_lmt_profile_id)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for COL_LIMIT_CHARGE_SEQ), 2)) AS bigint), 
...
where lsm.cms_collateral_id = s.cms_collateral_id
and s.cms_collateral_id = dtl.cms_collateral_id
and dtl.security_rank = 2
and s.sci_security_dtl_id in ('AB101-ALLOC-0003')
and lmts.cms_lsp_appr_lmts_id = lsm.cms_lsp_appr_lmts_id
and lmts.los_lmt_id in ('FAC-ALLOC-0028');

-- limit charge map for charge rank 3
insert into cms_limit_charge_map
(limit_charge_map_id, cms_lsp_appr_lmts_id, charge_id, charge_detail_id, cms_collateral_id,status, 
	customer_category, cms_lsp_lmt_profile_id)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for COL_LIMIT_CHARGE_SEQ), 2)) AS bigint), 
...
where lsm.cms_collateral_id = s.cms_collateral_id
and s.cms_collateral_id = dtl.cms_collateral_id
and dtl.security_rank = 3
and s.sci_security_dtl_id in ('AB101-ALLOC-0004')
and lmts.cms_lsp_appr_lmts_id = lsm.cms_lsp_appr_lmts_id
and lmts.los_lmt_id in ('FAC-ALLOC-0026', 'FAC-ALLOC-0027','FAC-ALLOC-0028');
...
'MYR', (current timestamp - 5 days), 'V9999', 500000000, 400000000, s.cms_collateral_id, 'S', 'ARBS'
from cms_security s
where s.sci_security_dtl_id in ('AB101-ALLOC-0002');


insert into cms_valuation
(valuation_id, valuation_currency, valuation_date, valuer, cmv, fsv, cms_collateral_id, source_type, source_id)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for VALUATION_SEQ), 2)) AS bigint), 
'MYR', (current timestamp - 5 days), 'V9999', 400000000, 300000000, s.cms_collateral_id, 'S', 'ARBS'
from cms_security s
where s.sci_security_dtl_id in ('AB101-ALLOC-0003');

insert into cms_valuation
(valuation_id, valuation_currency, valuation_date, valuer, cmv, fsv, cms_collateral_id, source_type, source_id)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for VALUATION_SEQ), 2)) AS bigint), 
'MYR', (current timestamp - 5 days), 'V9999', 600000000, 500000000, s.cms_collateral_id, 'S', 'ARBS'
from cms_security s
where s.sci_security_dtl_id in ('AB101-ALLOC-0004');
...
from cms_security
where sci_security_dtl_id in ('AB101-ALLOC-0002','AB101-ALLOC-0003','AB101-ALLOC-0004');
...
'37', 'ARBS', 'SIBS-ACT-0032', 'MY', 'N', 'A', 'L', 1, 300000000, 'I');
...
values
( CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for customer_x_ref_seq), 2)) AS bigint), 
'37', 'ARBS', 'SIBS-ACT-0033', 'MY', 'N', 'A', 'L', 1, 200000000, 'I');
...
values
( CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for customer_x_ref_seq), 2)) AS bigint), 
'37', 'ARBS', 'SIBS-ACT-0034', 'MY', 'N', 'A', 'L', 1, 500000000, 'I');
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	...
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0032'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0026';
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	(select xref.cms_lsp_sys_xref_id
	from sci_lsp_sys_xref xref
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0033'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0027';
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	(select xref.cms_lsp_sys_xref_id
	from sci_lsp_sys_xref xref
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0034'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0028';
