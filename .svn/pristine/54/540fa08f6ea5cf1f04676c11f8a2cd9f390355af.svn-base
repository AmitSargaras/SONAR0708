...
String st_adm[]={
"-- Please Select --","ACTIVE","APPROVED","CLOSED","DELETED","DISABLE","DRAFT","ENABLE","PENDING CREATE","PENDING DELETE","PENDING DISABLE","PENDING ENABLE","PENDING UPDATE","REJECTED","REJECTED CREATE","REJECTED UPDATE"
};

String st_tran[]={
		"-- Please Select --","ACTIVE","APPROVED","CLOSED","DELETED","DRAFT","PENDING CREATE","PENDING DELETE","PENDING UPDATE","REJECTED","REJECTED CREATE","REJECTED UPDATE"
};
   String lastUpdatedBy=(String)session.getAttribute(ICommonEventConstant.GLOBAL_SCOPE+"."+"lastUpdatedBy"); 
    ...
    String searchAANumber=(String)session.getAttribute(ICommonEventConstant.GLOBAL_SCOPE+"."+"searchAANumber"); 
    ...
    if (oneTeamMshipID == ICMSConstant.TEAM_TYPE_SSC_MAKER ||
	   ...
	   oneTeamMshipID==ICMSConstant.CPU_MAKER_CHECKER ||
	   oneTeamMshipID == ICMSConstant.TEAM_TYPE_CPU_MAKER_I
        	...
        	||oneTeamMshipID==ICMSConstant.CPU_MAKER_CHECKER_WFH
			...
			||oneTeamMshipID==ICMSConstant.TEAM_TYPE_CPU_MAKER_II
        	||oneTeamMshipID==ICMSConstant.TEAM_TYPE_CPU_MAKER_II_WFH
    ...
    if (oneTeamMshipID == ICMSConstant.TEAM_TYPE_SC_MAKER ||
            oneTeamMshipID == ICMSConstant.TEAM_TYPE_SC_CHECKER||oneTeamMshipID==ICMSConstant.CPU_ADMIN_MAKER_CHECKER
            ||oneTeamMshipID==ICMSConstant.TEAM_TYPE_CAD_MAKER||oneTeamMshipID==ICMSConstant.TEAM_TYPE_CAD_CHECKER
			||oneTeamMshipID == ICMSConstant.TEAM_TYPE_SC_MAKER_WFH
    		||oneTeamMshipID == ICMSConstant.TEAM_TYPE_SC_CHECKER_WFH
    		||oneTeamMshipID==ICMSConstant.CPU_ADMIN_MAKER_CHECKER_WFH
...
<%@page import="com.integrosys.cms.app.collateral.bus.OBCollateralLimitResult"%><html>
<head>
...
document.forms["WorkspaceForm"].action="ToDo.do?event="+event1+"&resetCriteria=on";
...
function submitPage(idx) {
	...
	//	document.forms[0].searchCustomerName.value='';
		...
		// document.forms["WorkspaceForm"].legalName.value=''; here
		document.forms[0].isBtnClicked.value='Y';
	}else if (idx == 6) {
		document.forms[0].isBtnClicked.value='Y';	
	}else if (idx == 7) {
	 // alert(document.forms["WorkspaceForm"].toDoStatus.value); 
		document.forms[0].isBtnClicked.value='Y';	
	}
                  ...
                  <td width="100%">
                   <TABLE width="100%" class=tblinfo id=custsearch border="0" align="left" cellpadding="0" cellspacing="0" style="margin-top:15px">
					...
					<%--
                     ...
                     --%><%if(!isSc){ %>
                      <TR class="odd">
                        <TD class="fieldname" width="25%"><bean:message key="label.customer.id"/> :</TD>
                        <TD width="25%"><html:text property="searchLeID" value="<%= searchLeID %>" maxlength="20" size="30" onkeypress="return handleEnter(this, window.event||event, 2);" /> 
							<html:errors property="leIDError"/> 
                    
                     
                        <TD class="fieldname" width="35%"><bean:message key="label.customer.name" /> :</TD>
                        <TD width="25%"><html:text property="searchCustomerName" value="<%= searchCustomerName %>" maxlength="40" size="30" onkeypress="return handleEnter(this, window.event||event, 3);" /> 
							<html:errors property="customerNameError"/>
                        </TD>
                       <!-- put here -->
                         </TR>
                      <TR class="odd">
                        <TD class="fieldname" width="25%"><bean:message key="label.global.aa.number" /> :</TD>
                        <TD width="25%"><html:text property="searchAANumber" value="<%= searchAANumber %>" maxlength="40" size="30" onkeypress="return handleEnter(this, window.event||event, 4);" /> 
							<html:errors property="aaNumberError"/>
						</TD>
                        <TD class="fieldname" width="35%"><bean:message key="label.todo.last.updated.by" /> :</TD>
                        <TD width="25%"><html:text styleId="last" property="lastUpdatedBy" value="<%= lastUpdatedBy %>" maxlength="40" size="30" onkeypress="return handleEnter(this, window.event||event, 5);" /> 
							<html:errors property="lastupdatedbyerror"/>
                        </TD>
                         </TR>
                        <TR class="odd">
                        <TD class="fieldname" width="25%"><bean:message key="label.todo.transactiontype" /> :</TD>
                        <TD width="25%">
                         <html:select styleId="selBox" property="transactionType">
                         <%if(null!=transactionType||"".equalsIgnoreCase(transactionType)){%>
                         <html:option value="select">-- Please Select --</html:option>
                         <%} %>
                         <%
                         if(null!=transactionType||"".equalsIgnoreCase(transactionType)){
                         for(int i=0;i<transactionTypeList.size();i++){
                         LabelValueBean labelValueBean = (LabelValueBean)transactionTypeList.get(i);
                         if(labelValueBean.getLabel().equals(transactionType)){ %>
                        ...
                        <% 
                         ...
                         <html:option value="">-- Please Select --</html:option>
                           <html:options collection="transactionTypeList" labelProperty="label" property="value"  />
                           <%} %>
                        </html:select>
							<html:errors property="transactiontypeerror"/>
						</TD>
                       
                        <TD class="fieldname" width="35%"><bean:message key="label.status" /> :</TD>
                        <TD width="25%">
                        <html:select  property="toDoStatus" >
                        <% for(int y=0;y<st_tran.length;y++) {%>
                        <%if(st_tran[y].equalsIgnoreCase("-- Please Select --")){%>
                        <option value=""><%=st_tran[y] %></option>
                        <%}else{if(toDoStatus!=null&&toDoStatus.replace('_',' ').equalsIgnoreCase(st_tran[y])){ %>
                         <option  selected value="<%= st_tran[y] %>"><%= st_tran[y] %></option>
                         <%}else{ %>
                        <option  value="<%= st_tran[y] %>"><%= st_tran[y] %></option>
                        <%} %>
                        <%}} %>
                        </html:select>
							<html:errors property="statuserror"/>
						</TD>
                        </TR>
                         <%} %>
                          <%if(isSc){ %>
                       ...
                       <TR class="odd">
                        ...
                        <TD width="25%">
                        <html:select  property="toDoStatus" >
                        <% for(int y=0;y<st_adm.length;y++) {%>
                        <%if(st_adm[y].equalsIgnoreCase("-- Please Select --")){%>
                        <option value=""><%=st_adm[y] %></option>
                        <%}else{if(toDoStatus!=null&&toDoStatus.replace('_',' ').equalsIgnoreCase(st_adm[y])){ %>
                         <option  selected value="<%= st_adm[y] %>"><%= st_adm[y] %></option>
                         <%}else{ %>
                        <option  value="<%= st_adm[y] %>"><%= st_adm[y] %></option>
                        <%} %>
                        <%}} %>
                        </html:select>
							<html:errors property="statuserror"/>
...
<!--                        </TD></TR>-->
                        <tr class="odd">
                        <td colspan="4"><center>
                        <input class=btnNormal id=Search3 style="margin-left: 10px; WIDTH: 50px; MARGIN-RIGHT: 5px" onclick=submitPage(3) type=button value=Go name=Search3>
                       
                       <%if(isSc){ %>
                        <input class=btnNormal  style="margin-left: 18px; WIDTH: 100px; MARGIN-RIGHT: 5px" onclick=resetCriteria1("<%= event %>") type=button value="Reset Criteria"/>
                        <%}else{ %>
		...
		<tr>
		  <td align="center">
              ...
              %>
				...
				<tr><td></td>
				<td><%if("prepare".equals(event)||"searchtodo".equals(event)){ %>
                      <a href="ToDo.do?event=prepare&sort=asc&field=user_info&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=prepare&sort=desc&field=user_info&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
                         <%}else{ %>
                        <a href="ToDo.do?event=totrack&sort=asc&field=customer_name&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=totrack&sort=desc&field=customer_name&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>  
                       <%} %></td>
				<td></td>
				<td><%if("prepare".equals(event)||"searchtodo".equals(event)){ %>
				  <a href="ToDo.do?event=prepare&sort=asc&field=transaction_date&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=prepare&sort=desc&field=transaction_date&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
                  <%}else{ %>
                  <a href="ToDo.do?event=totrack&sort=asc&field=transaction_date&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=totrack&sort=desc&field=transaction_date&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
				...
				<tr>
				  <td width="3%"><bean:message key="label.global.sn"/></td>
				  <td width="11%"><bean:message key="label.todo.last.updated.by"/>
				  </td>
				  ...
				  <td width="9%">Transaction Date<br></td>
<%
...
%>
				  ...
				  <td width="7%">Minimum Employee Grade as per LOA</td>
				...
				<td><%if("prepare".equals(event)||"searchtodo".equals(event)){ %>
				  <a href="ToDo.do?event=prepare&sort=asc&field=user_info&isBtnClicked=Y"><img  width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=prepare&sort=desc&field=user_info&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
				  <%}else{ %>
				   <a href="ToDo.do?event=totrack&sort=asc&field=user_info&isBtnClicked=Y"><img  width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=totrack&sort=desc&field=user_info&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
				  <%} %></td>
				<td><%if("prepare".equals(event)||"searchtodo".equals(event)){ %>
                      <a href="ToDo.do?event=prepare&sort=asc&field=lname&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=prepare&sort=desc&field=lname&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>
                         <%}else{ %>
                        <a href="ToDo.do?event=totrack&sort=asc&field=customer_name&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowup.gif" border="0"/></a>  <a href="ToDo.do?event=totrack&sort=desc&field=customer_name&isBtnClicked=Y"><img width="14px" height="14px" src="img/arrowdown.gif" border="0"/></a>  
                       <%} %></td>
				<td></td>
...
<!--				  <td><integro:empty-if-null value="<%=obTodoObject.getDealNo()%>"/>&nbsp;</td> -->
				  ...
				  <%if(!isNewNonBorrower && !isSys && !isSc && !isMr){ %>
				  	<td> <integro:empty-if-null value="<%=obTodoObject.getMinEmployeeGradeLoa()%>"/></td>
				  <%} %>
				  ...
				  <td><%=limitProfileRefNum%></td>
				  <td><%=limitProfileRefNum%></td>
				  <td>= <br><integro:date object="<%=obTodoObject.getTransactionDate()%>" /></td>
				  <%
					  }
				  %>
				  <%
