...
<%
    System.out.println("Entered guarantorList.jsp");
...
<%
    ...
    aForm = (ReportForm) request.getAttribute("ReportForm");
    
    if (aForm == null) {
    	aForm = (ReportForm) request.getAttribute("CustodianSearchForm");
    }
    ...
    List guarantorList = (List)session.getAttribute("com.integrosys.cms.ui.poi.report.ReportAction.guarantorList");
    
    /* if(sresult!=null){
    	System.out.println("-------------------------2-----------------------------"+sresult.getResultList());
    	results = sresult.getResultList();
    	aForm.setNumItems(sresult.getNItems());
   		pageContext.setAttribute("guarantorList",guarantorList);
   	} */
/*     if(results!=null) {
       pageContext.setAttribute("guarantorList",results);
    }
 */
 	if(guarantorList!=null) {
       pageContext.setAttribute("guarantorList",guarantorList);
    }else{
       pageContext.setAttribute("guarantorList",new ArrayList());
    }
...
<%@page import="com.integrosys.cms.ui.poi.report.ReportForm"%><html>
<head>
...
<!--
    ...
    function submitForm(){
		...
		if(isadded){
	        ...
	        formObj.event.value = "select_guarantor";
	    	formObj.submit();
		}else{
						return false;
		}
          ...
          <tr>
           <td>
            <table  width="100%" class="tblinfo" style="margin-top:0" border="0" cellspacing="0" cellpadding="0">
             ...
             <tbody>
       <% //if(results!=null)  {%>
              <logic:present name="guarantorList">
               <logic:iterate id="OB" name="guarantorList" indexId="index" type="com.integrosys.cms.app.customer.bus.OBCMSCustomer" scope="page" >
				<%
					String rowClass = "";
								sno++;
								if (sno % 2 != 0) {
				...
				%>
               <input type="hidden" name="customerID" value ="<%=OB.getCustomerID()%>">
               ...
               <input type="hidden" name="agreementID" value ="<%=agreementID%>"> --%>
                  ...
                  <td><integro:empty-if-null value="<%=OB.getCifId()%>"/>&nbsp;<br>
...
<!--                  <input type="checkbox" name="itemID" value="<%=sno%>" />-->
               ...
               <%//} else {%>
               <logic:empty name="guarantorList" scope="page">
			   <td colspan="4" class="odd"><bean:message key="label.global.not.found"/>&nbsp;</td>
               </logic:empty>
			   <%//}%>
              </tbody>
			</table>
