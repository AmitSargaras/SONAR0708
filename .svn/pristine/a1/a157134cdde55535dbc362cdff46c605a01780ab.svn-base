...
<html>
<head>
<title>Exclusion line for STP of SRM</title>
...
function prepareCreate() {
 	document.forms[0].action="ExcLineForSTPSRM.do?event=maker_prepare_create&isNew=Y";
			...
			<tr>
		   		<td valign="top">
					<table width="70%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
		            	<thead>
		                 	<tr>
			                    <td width="82%"><h3>Exclusion line for STP of SRM</h3></td>
		                     </tr>
		                     <tr>
		                     <%if(isMaker){%>
		                    	...
		                    	<td colspan="4">
		                    		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
			                        	...
			                        	<tbody>
					                        ...
					                        type="com.integrosys.cms.app.excLineforstpsrm.bus.IExcLineForSTPSRM" scope="page">
					                        <%
					                            String rowClass="";
					                            sno++;
					                            if(sno%2!=0){
                                    				...
                                    				<td><center>
                                  					...
                                  					<td>
                                  						<select name="menu1" onchange="MM_jumpMenu('self',this,0)" style="width:110">
					                                       ...
					                                       <%if(isMaker){%>
                                      						<option value='ExcLineForSTPSRM.do?event=maker_view&id=<%=item.getId()%>&startIndex=<%=startIndex%>'>View</option>
                                      						<option value='ExcLineForSTPSRM.do?event=maker_edit_read&id=<%=item.getId()%>&startIndex=<%=startIndex%>'>Edit</option>
                                      						<option value='ExcLineForSTPSRM.do?event=maker_delete_read&id=<%=item.getId()%>&startIndex=<%=startIndex%>'>Delete</option>
									  					<%}else{ %>
									  						 <option value='ExcLineForSTPSRM.do?event=checker_view&id=<%=item.getId()%>&startIndex=<%=startIndex%>'>View</option>
									  					<%} %>
                                    					</select> 
                                    				</td>
					                        	</tr>
				       ...
				       <td height="25">
				           <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="0">
				             <tr>
				               ...
				               <%
				                   String pageIndexUrl="ExcLineForSTPSRM.do?event=maker_list&startIndex=";
				               %>
