...
<%
	...
	System.out.println("transDenied--"+transDenied);
	String codeNotExist = (String)request.getAttribute("codeNotExist");
	System.out.println("codeNotExist--"+codeNotExist);
	
	if(!fileType.equals("NOT_CSV") && !preStatus.equals("PENDING") && !fileCheckSum.equals("MISMATCH") && transDenied.equals("false") && ! codeNotExist.equals("CODENOTFOUND")){
...
System.out.println("stockType ///////////: "+request.getParameter("stockType"));
String title = "";
 if(stockType.trim().equals("001")){
    	title = "BSE";
    }else if(stockType.trim().equals("002")){
    	title = "NSE";
    }else{
    	title = "Others";
    }
...
function cancelPage(){
	...
	document.forms[0].action="feed_stock_item.do?event=maker_prepare_upload_stockItem&stockType=<%=stockType%>";
						...
						<td>
							<h3>
								Error <%=title%> List
													...
													<% }else 
														if(codeNotExist.equals("CODENOTFOUND")){
													%>
													<tr class="odd">
														<td colspan="10">
														<bean:message key="label.global.file.code.not.exist" />
														</td>
													</tr>
													<% } %>
