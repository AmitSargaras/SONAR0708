...
if(imageTagMapForm!=null){
	...
	//request.setAttribute("migCheckListImages",list1);
...
function closePage() {
	document.forms["CAMReceiptForm"].event.value="list";
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="CAMReceipt.do?event=retrieveMigImageGallaryCAMDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg;
	document.forms[0].submit();
}
																...
																<tbody>
																	...
																	<tr class=<%=rowClass%>>
																		...
																		<td width="25%">
																			...
																			<%if(status==3){ %>
																		<a href="#"
onclick="javascript:getImageGallaryCAMDoc('<%=DocumentPath%>','<%=DocumentName%>','<%=status%>','<%=counter1-1 %>')"><%=DocumentName%>
</a>	
<%}else{ %>
<%=DocumentName%>
<%} %>
