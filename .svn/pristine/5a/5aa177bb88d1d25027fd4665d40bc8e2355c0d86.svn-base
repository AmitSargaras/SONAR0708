...
List deferCreditApproverList= new ArrayList();

if("PropCommGeneral".equals(subType)){
	deferCreditApproverList= (List)session.getAttribute("com.integrosys.cms.ui.collateral.property.PropertyAction.deferCreditApproverList");

	if(deferCreditApproverList!=null){
		System.out.println("sdfjhfdghjdggfjhjk");
    	pageContext.setAttribute("deferCreditApproverList",deferCreditApproverList);
    }
}
else{
	deferCreditApproverList = (List)session.getAttribute("com.integrosys.cms.ui.collateral.assetbased.AssetBasedAction.deferCreditApproverList");
               ...
               <tr class="even">
                ...
                <td width="30%">
                 ...
                 if(aForm!= null){
                	 ...
                	 if(aForm.getDateDeferred()!=null && (!"".equals(aForm.getDateDeferred()))){
                          dateDeferred=df.format(df.parse(aForm.getDateDeferred()));
                	    }
                	    if(aForm.getNextDueDate()!=null && (!"".equals(aForm.getNextDueDate()))){
                	    	nextDueDate=df.format(df.parse(aForm.getNextDueDate()));
                  	    }
...
<table width="130" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <td colspan="4">
	...
	<%	if(event.equals("maker_view_insurance_deferred")){%>		
				<center>
							...
							<%	}else if(event.equals("checker_view_insurance_deferred") && "read".equals(from_event)){%>		
								<center>
							...
							<%	}else if(event.equals("checker_view_insurance_deferred")){%>		
							<center>
							<a href="javascript:cancelPageMakerView1();"
								onmouseout="MM_swapImgRestore()"
								onmouseover="MM_swapImage('Image4411','','img/return2.gif',1)"><img
	...
	<% if(isChecker){%>
  <td colspan="4">
				<%if(event.equals("checker_view_insurance_deferred") && "read".equals(from_event)){ %>
