...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'makerretrieveImageLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=checker_view_attach_image&category="+catg;
		   document.forms[0].event.value="checker_view_attach_image";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'makerretrieveImageLADDoc'){
			 document.forms[0].action="LADReceipt.do?event=makerretrieveImageLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
