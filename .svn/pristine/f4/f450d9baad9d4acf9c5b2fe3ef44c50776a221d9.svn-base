...
if( Long.parseLong(teamTypeMemID)==ICMSConstant.TEAM_TYPE_PARTY_MAKER){
	isUserCpcMaker = true;
} else {
...
function goAlphaIndex(ind) {
  ...
  var event = "<%=event%>";
    ...
    else if(event == 'list_customers_partymaker')
    {
    	document.forms[0].event.value = "list_customers_partymaker";
    }
    else
    {
...
function getAction(id, cif, ind) {
	...
	if( type == "View"){
		...
		document.forms[0].event.value = "view_customer_partymaker";
		document.forms[0].submit();
	}
		...
		if(event=="list_customers_partymaker")
				{
			document.forms[0].event.value = "edit_customer_details_partymaker";  
				}
		else{
		...
		cellspacing="0" class="tblFormSection">
			...
			<%if(event.equals("checker_list_party_detail") || event.equals("list_customer_checker")){ %>
				...
				<%}else if(event.equals("brchecker_list_party_detail") || event.equals("list_customer_brchecker")){ %>
						<td align="right"></td>
					<%}else if(event.equals("list_customers_partymaker")){ %>
					 <%if(isUserCpcMaker) { %>
					<td align="right"><input type="button" id="AddNew" value="Add"
					onclick="javascript:createNewCustomer('prepare_create_customer_partymaker')" class="btnNormal"
			...
			<tr>
				...
				<table width="100%" class="tblinfo" style="margin-top: 0" border="0"
              	...
              	--></select></center></td>
              	<%}else{ %>
              		<%if(ob.getStatus()!= null && ob.getStatus().equals("INACTIVE")){ %>
              	<td><center><select id="actionType<%=sno+startIndex%>" onchange="getAction('<%=ob.getCustomerID() %>','<%=ob.getCifId() %>','<%=sno+startIndex%>')">
              		<option value="">Please Select </option>
...
<td height="25">
 ...
 {
	 pageIndexUrl = "ManualInputCustomer.do?event=list_customers_partymaker&customerNameShort="+customerNameShort+"&startIndex=";
 }
 else{
...
<% if (list!=null) { %>
 ...
 <% if(event.equals("checker_list_party_detail") || event.equals("list_customer_checker")){%>
   <td width="580" valign="middle">
               ...
               <%  }else if(event.equals("list_customers_partymaker")){ %>
               <td width="580" valign="middle">
               <integro:alphaindex url='<%="ManualInputCustomer.do?event=list_customers_partymaker"%>' submitFlag='<%=new Boolean(true)%>'/>
                </td>
                  <%}else{ %>
               <td width="580" valign="middle">
