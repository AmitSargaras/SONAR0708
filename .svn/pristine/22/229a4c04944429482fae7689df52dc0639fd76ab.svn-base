...
<%
FacilityReceiptForm facilityReceiptForm = (FacilityReceiptForm) request.getAttribute("FacilityReceiptForm");
HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.checklist.facilityreceipt.FacilityReceiptAction.selectedArrayMap");
...
else if("view_image_page_to_track".equals(event)){
	canView=true;
}
%>
...
function closePage() {
	document.forms["FacilityReceiptForm"].event.value="view_return";
	document.forms["FacilityReceiptForm"].submit();
}
function returnPage() {
    document.forms["FacilityReceiptForm"].event.value = "view_return_to_track";
    document.forms["FacilityReceiptForm"].submit();
}
...
function downloadImage(imagePath,imageName,status){ //v3.0
	var aLocation = "FacilityReceipt.do?event=downloadImage&imageName="+imageName+"&pID="+imagePath+"&status="+status;
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "FacilityReceipt.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
	window.location.href = aLocation ;
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="FacilityReceipt.do?event=retrieveImageGallaryFacilityDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("FacilityReceipt.do?event=print_image&custId=<%=custId%>&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
...
<p align="center">
 <%   if (canView) {System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>> IF"); %>
                <a href="#" onclick="returnPage(1)" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image1','','img/return2.gif',1)"><img src="img/return1.gif" name="Image1"  border="0" id="Image1" /></a>
