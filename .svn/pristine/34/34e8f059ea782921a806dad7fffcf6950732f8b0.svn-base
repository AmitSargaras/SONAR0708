...
<%
	...
	String rejectEvent;
	if (event.equals("checker_view_party_group")) {
		rejectEvent = "checker_list_party_group";
	} else {
		rejectEvent = "maker_list_party_group";
	}
	int sno = 0;
	int startIndex = 0;
%>
...
function cancelPage(){
    document.forms[0].action="MIPartyGroup.do?event=<%=rejectEvent%>&startIndex=<%=request.getAttribute("startIndex")%>";
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tbody>
						...
						<tr class="even">
							<td class="fieldname" width="30%">Party Group Exposure Limit
							</td>
							<td width="20%"><%=partyGroupForm.getGroupExpLimit()%> INR&nbsp;
							</td>
						...
						<%
							if( ! partyGroupTrx.getStatus().equals("ACTIVE") ){ 
						%>
						<tr class="odd">
							<td class="fieldname" width="30%">Last Remarks Made</td>
							<td colspan="3" width="20%"><%=partyGroupTrx.getRemarks() != null ? partyGroupTrx
						.getRemarks() : ""%>&nbsp;</td>
						</tr>
						<tr class="even">
