...
<%
CAMReceiptForm CAMReceiptForm = (CAMReceiptForm) request.getAttribute("CAMReceiptForm");
String catg="";
String docTye="";
String custLimitProfileID=request.getParameter("custLimitProfileID");
	...
	HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.selectedArrayMap");
	...
	// System.out.println("value of records_per_page"+records_per_page);
		...
		String isLatest = (String) session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.isLatest");
...
if(!isUserCpcMaker || !isActive){
	canView=true;
}else if(isLatest!=null && isLatest.equals("Y")){
	canView=true;
}
...
function returnPage() {
    document.forms["CAMReceiptForm"].event.value = "view";
    document.forms["CAMReceiptForm"].submit();
}
function returnPage2() {
    document.forms["CAMReceiptForm"].event.value = "view2";
    document.forms["CAMReceiptForm"].submit();
}
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="CAMReceipt.do?event=retrieveImageGallaryCAMDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}

function downloadImage(imagePath,imageName,status){ //v3.0
	var aLocation = "CAMReceipt.do?event=downloadImage&imageName="+imageName+"&pID="+imagePath+"&status="+status;
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "CAMReceipt.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
	window.location.href = aLocation ;
...
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("CAMReceipt.do?event=print_image&custId="+custId1+"&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
...
<p align="center">
 ...
 <%   if (canView && !"view_image_page2".equals(event)) { %>
     ...
     <a href="#" onclick="closePage()" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image1','','img/return2.gif',1)"><img src="img/return1.gif" name="Image1"  border="0" id="Image1" /></a>
 <%}else if("view_image_page2".equals(event)){ %>
             <a href="#" onclick="returnPage2()" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image1','','img/return2.gif',1)"><img src="img/return1.gif" name="Image1"  border="0" id="Image1" /></a>   
             <% }else{ %>
