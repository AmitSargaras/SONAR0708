<%@ page import="com.integrosys.cms.app.common.constant.ICMSConstant,
                 ...
                 java.util.Arrays,
                 com.integrosys.cms.app.commodity.main.bus.profile.*,
...
<%
    ...
    * $Header: /home/cms2/cvsroot/cms2/public_html/commodityglobal/commodityprofile/list/chk_view_commprofile_list.jsp,v 1.11 2006/10/27 08:41:29 hmbao Exp $
    *
    * Purpose:
    * Description:
    *
...
<%
	...
	CommodityCategoryList profileList = CommodityCategoryList.getInstance();
%>
...
<input type="hidden" name="event" value="<%=CommProfileListAction.EVENT_APPROVE%>"/>
...
<%
        ...
        Arrays.sort(resultList, new ProfileComparator(ProfileComparator.BY_CAT_PDTTYPE_SUBTYPE));
        ArrayList list = new ArrayList(Arrays.asList(resultList));
        pageContext.setAttribute("commProfile",list);
        if (list == null || list.size() == 0) {
%>
            <tr class="odd">
            <td colspan="5">There is no Commodity Profile.</td>
            </tr>
<%
        } else {
%>
		<logic:present name="commProfile" >
        <logic:iterate id="compResult" name="commProfile"  type="com.integrosys.base.techinfra.diff.CompareResult"  >
        <%
            IProfile commProfileItem = (IProfile)compResult.getObj();
            String priceType =  CMDTProfHelper.getPriceTypeDesc(commProfileItem.getPriceType());
        %>
        <tr class="<%=counter%2==0?"odd":"even"%>">
          <td class="<bean:write name="compResult" property="key" />" width="4%"><%=counter + 1%></td>
              <td width="23%"><integro:empty-if-null value="<%=profileList.getCommCategoryItem(commProfileItem.getCategory())%>"/></td>
              <td width="23%"><integro:empty-if-null value="<%=profileList.getCommProductItem(commProfileItem.getCategory(), commProfileItem.getProductType())%>"/></td>
              <td width="15%"><integro:empty-if-null value="<%=priceType%>"/></td>
              <td width="15%"><integro:empty-if-null value="<%=commProfileItem.getReuterSymbol()%>"/></td>
              <td width="23%"><integro:empty-if-null value="<%=commProfileItem.getProductSubType()%>"/></td>
              <td width="12%"><a href="javascript:viewItem(<%=commProfileItem.getCommonRef()%>)">View</a></td>
            </tr>
<%
            counter++;
%>
