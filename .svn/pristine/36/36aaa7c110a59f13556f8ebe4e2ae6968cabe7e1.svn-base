...
ArrayList deferCreditApproverList = (ArrayList)session.getAttribute("com.integrosys.cms.ui.checklist.secreceipt.SecurityReceiptAction.deferCreditApproverList");
    ...
    if(flag.equals("")){
      	 flag= (String)session.getAttribute("com.integrosys.cms.ui.checklist.secreceipt.SecurityReceiptAction.flag");
      }
      if(flag==null){
   	   flag="";
      }
    ...
    if(!toTrack || "cancel_checklist_item".equals(event)||"search_security_doc_checker_close".equals(event)){
 	   searchEvent="search_security_doc_checker_close";
    }
    if("close_checklist_item".equals(event)){
    	searchEvent="search_security_doc_checker_close";
    }
    if("edit_staging_checklist_item".equals(event)){
    	searchEvent="search_security_doc_maker_close";
    }
    if(isUserCpcMaker && "view_return".equals(event) && !toTrack){
    	searchEvent="search_security_doc_checker_close";
    }
    if(!isUserCpcMaker && !toTrack ){
    	searchEvent="checker_search_security_doc";
    }
%>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  <thead>
    <tr>
      <td> <h3><%=("Cancel".equals(flag)?"Close":flag)%> Security Doc Receipt</h3>
...
<!--		      <% 	
		      ...
		      %>
<!--				<html:radio property="lawFirmPanelFlag"  name="SecurityReceiptForm" value="Y" disabled="true" />Yes &nbsp;-->
<!--                <html:radio property="lawFirmPanelFlag"  name="SecurityReceiptForm" value="N" disabled="true" /> No &nbsp;-->
<!--				<html:radio property="lawFirmPanelFlag"  name="SecurityReceiptForm" value="-"  disabled="true" /> N/A-->
<!--		      </td>-->
<!--		    </tr>-->
<!--		    </tbody>-->
<!--		  </table>-->
...
<!--					<%if(aa){ %><font color="#0000FF" >*</font><%} %></td>-->
<!--					<td width="80%"> -->
...
<!--               <td>&nbsp;</td>-->
  ...
  <%String checkListID = request.getParameter("checkListID"); %>
              ...
              <a href="#" onClick="viewImageToTrack('<%=checkListID%>','<%=row-1 %>')">View Images</a>
              <%}else if("view_return".equals(event)||"close_checklist_item".equals(event)||"cancel_checklist_item".equals(event)||"search_security_doc_maker_close".equals(event)||"search_security_doc_checker_close".equals(event)){ %>
               <a href="#" onClick="viewImageToDo('<%=checkListID%>','<%=row-1 %>')">View Images</a>
              <%}else { %>
              ...
              <%  if ("view".equals(request.getParameter("event"))) {
                    String checkListID1 = request.getParameter("checkListID");
              %>
                <a href="#" onClick="goView('<%=OB.getCheckListItemRef()%>', '<%=checkListID1%>')">View</a>
              <% } else { %>
...
<% if(row==0){%>
            <tr class="odd">
              <td colspan="16">There are no securities linked</td>
            </tr>
<% }%>
          </tbody>
        </table></td>
...
<table width="150" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <%if((!toTrack && ("Close".equals(flag)||"Edit".equals(flag)))&& !checkListTrxVal.getStatus().equalsIgnoreCase("DRAFT")) { %>
    ...
    <td><a href="SecurityReceipt.do?event=cancel" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image4','','img/close2a.gif',1)"><img src="img/close1a.gif" name="Image4" width="70" height="20" border="0" id="Image4" /></a>&nbsp;&nbsp;&nbsp;<a href="ToDo.do" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image4411','','img/cancel2.gif',1)"><img src="img/cancel1.gif" name="Image4411"  border="0" id="Image4411" /></a></td>
    <%}else{%>
    <td><a href="SecurityReceipt.do?event=list" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image4','','img/return2.gif',1)"><img src="img/return1.gif" name="Image4" width="70" height="20" border="0" id="Image4" /></a></a></td>
    <%} %>
  </tr>
  <tr>
    <td>&nbsp;</td>
