LOAD DATA
INFILE 'FILES\Common Code_Industry Category(Broad).csv'
 "str '~~~'"
BADFILE 'PARTIAL_ERROR_LOG\Common Code_Industry Category(Broad).bad'
DISCARDFILE 'PARTIAL_ERROR_LOG\Common Code_Industry Category(Broad).dsc'
DISCARDMAX 1241513983
APPEND INTO TABLE MIG_COMMON_CODE_CATEGORY_IND
FIELDS TERMINATED BY '|||'
TRAILING NULLCOLS
(
COMMONID "REPLACE(REPLACE(DECODE(UPPER(:COMMONID),'NULL',NULL,:COMMONID), CHR(10),''), CHR(13),'')",
COMMONNAME "DECODE(UPPER(:COMMONNAME),'NULL',NULL,:COMMONNAME)"
)