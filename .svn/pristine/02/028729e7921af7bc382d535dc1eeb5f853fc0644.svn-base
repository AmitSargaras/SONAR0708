...
<script type="text/javascript">
	...
	function submitPage() {    
        ...
        document.forms[0].action =   "/user.do?event=broadcast_message&customerID=-999999999&limitProfileID=-999999999"
	...
	if (syncBCMessageEnabled)
	{
		if (!"".equalsIgnoreCase(requestAction))
		{
			...
			for (int app = 0; app < applist.length; app++)
			{
				...
				String urlContext = PropertyManager.getValue("console.broadcastmessage.urlcontextpath" + currentApp, "/cms/console/monitor/create_broadcast_msg.jsp").trim();
				System.out.println("      ##### " + "console.broadcastmessage.urlcontextpath" + currentApp + ": " + urlContext);
				String serverList = PropertyManager.getValue("console.broadcastmessage.serverlist" + currentApp, "localhost:7003").trim();
...
<%
			...
			while (itr.hasNext()) {		
				...
				resultString.append("<td><center>" + ((String)bc_message_viewed.get(messageKey)) + "</center></td>");
...
//	Properties connProperties = null;
    ...
    try {
		...
		//Start : commented by Sachin on 25-11-2011 This code is for set online user in dropdown list
		/*StringBuffer resultString = new StringBuffer ();
		resultString = (StringBuffer)request.getAttribute("resultString");
...
%>
	...
	<tr class="odd">
		...
		<td>
			...
			<%--<a href="create_broadcast_msg.jsp"><img src="<%=request.getContextPath()%>/img/refresh1.gif" border="0"></a>
			--%>
			<input type="submit" onclick="javascript:submitPage()"
