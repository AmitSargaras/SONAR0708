...
function submitPage(event) {
	if(event=="maker_draft_close_process"){
		document.forms[0].action="valuationAmountAndRating.do?event=maker_confirm_draft_close";
    }else {
    	document.forms[0].action="valuationAmountAndRating.do?event=maker_confirm_close";
    }
    document.forms[0].submit();
}
</script>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tbody>
								<tr class="even">
									...
									<td ><%=valuationAmountAndRatingForm.getCriteria()%> &nbsp;</td>

									<td class="fieldname" >Valuation Amount</td>
									<td ><%=valuationAmountAndRatingForm.getValuationAmount()%>&nbsp;</td>
								</tr>
								<tr class="even">
									<td class="fieldname" >RAM Rating</td>
									<td ><%=valuationAmountAndRatingForm.getRamRating()%> &nbsp;</td>
									<td class="fieldname" >Exclude Party ID</td>
									<%if(valuationAmountAndRatingForm.getExcludePartyId() != null){ %>
									<td ><%=valuationAmountAndRatingForm.getExcludePartyId()%> &nbsp;</td>
									<%}else{ %>
									<td >-</td>
									<%} %>
								</tr>
