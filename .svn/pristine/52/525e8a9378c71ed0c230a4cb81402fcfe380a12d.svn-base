...
/*Modified by Boon Yi, 25/08/2003, CMS
Change format of month display from mm=08 and M=Aug.
to mm=Aug, M=08.
*/
...
// Jimmy for Removing input Text which is disable
Calendar._TT["REMOVE"] = "Remove";
Calendar._TT["DO_REMOVE"] = "Remove Date";
...
// fixed a vertical scroll problem - by sjamsul
...
Calendar.cellClick = function(el) {
	...
	if (typeof el.navtype == 'undefined') {		
		...
		// Jimmy: to fix the bug:
		// when input textbox is empty, Today button doesn't work
		if(el.navtype==0) {
			newdate=true;
		}
		...
		if(el.navtype==199) {
			// removing value from the existing input textbox
			cal.sel.value="";
			newdate=false;
			cal.date = null;
			cal.callHandler();
...
Calendar.prototype.create = function (_par) {
	...
	hh(Calendar._TT["REMOVE"], 2, 199).ttip = Calendar._TT["DO_REMOVE"];
	//cell = hh(Calendar._TT["SEL_DATE"], 7, 300);
	cell = hh(Calendar._TT["SEL_DATE"], 5, 300);
	cell.className = "ttip";
	if (this.isPopup) {
...
Calendar.prototype.formatDate = function () {
	...
	s['DD'] = Calendar._DN[w];
	s['mm'] = Calendar._MN[m].substr(0, 3);
	s['MM'] = Calendar._MN[m];
	var re = /(.*)(\W|^)(d|dd|m|M|y|yy|MM|mm|DD|D)(\W|$)(.*)/;
