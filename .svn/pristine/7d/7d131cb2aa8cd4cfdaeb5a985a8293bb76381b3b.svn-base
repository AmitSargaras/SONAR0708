...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10">
  ...
  <%    
   ...
   %>
    <tr class="<%= rowClass%>"> 
        <td  class="index" valign="top" width="3%" ><%= row%></td>
        <td valign="top" width="8%" ><%= colAuditList[ii].getCollateralRef()%></td>
        <td valign="top" width="11%" ><%= colAuditList[ii].getCollateralType().getTypeName()%> / <%= colAuditList[ii].getCollateralSubType().getSubTypeName()%></td>
        <td valign="top" width="13%" ><%= colDesc %>
<!-- collateral description -->
        </td>
        <td valign="top" width="17%" ><%= chargeInfo %>
<!-- chargeb info -->        
        </td>
...
<%
            ...
            if (itemList != null)
            {
                for (int jj=0; jj<itemList.length; jj++)
                {
%>                  <%= itemList[jj].getDescription()%><br /><br />
<%              }
            }
%>
            </p>
...
<%          if (itemList != null)
            {
                for (int jj=0; jj<itemList.length; jj++)
                {
%>                  <%=DateUtil.formatDate("dd/MMM/yyyy",itemList[jj].getDocumentDate())%><br /><br />
<%              }
            }
%>
            </p>
