...
function createGoodsMaster() {
	...
	function searchGoodsMaster()
	{
		...
		if(searchType == '' && searchText == '')
        {
	        ...
	        document.forms[0].event.value       = "maker_list_goods_master";
	        document.forms[0].submit();
        }
        else
        {
			...
			<table width="95%" border="0" align="center" cellpadding="0"
				...
				<thead>
                   ...
                   <td><label>Search :&nbsp;</label>
                   	<select id="selType">
	                   	...
	                   	<option value="goodsName">Goods Desc</option>
                   	</select> 
                   ...
                   <td  valign="bottom" align="right">						
						<input type="button" name="Submit" value="Add New" class="btnNormal" onclick="createGoodsMaster()" />
				   </td>    
               </tr>
               <tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<thead>
								<tr>
									...
									<td width="16%">Goods Description</td>
									<td width="16%">Restriction Type</td>
                        ...
                        %>
                                <tr class="<%=rowClass%>">
                                  <td class="index"><%=counter.intValue()+1%></td>
                                    ...
                                    <td><center><%=ob.getRestrictionType()%>&nbsp;</center></td>
                                    <%System.out.println("ID from goods Master "+ob.getId()); %>
                                     <%System.out.println("Master ID from goods Master "+ob.getMasterId()); %>
              ...
              <%}else{ %>Disable<%} %>&nbsp;</td>
                                  <td> <select name="menu1" onchange="MM_jumpMenu('self',this,0)" style="width:110">
                                      ...
                                      <%if((event.equals("checker_goods_master"))||event.equals("checker_list_pagination")){%>
                                      <option value='goodsMaster.do?event=checker_view_goods_master&id=<%=ob.getId()%>&goodsCode=<%=ob.getGoodsCode()%>&goodsName=<%=ob.getGoodsName()%>&startIndex=<%=startIndex%>'>View</option>
                                       <%}else{ %>
                                      
                                      <option value='goodsMaster.do?event=maker_view_goods_master&id=<%=ob.getId()%>&goodsCode=<%=ob.getGoodsCode()%>&goodsName=<%=ob.getGoodsName()%>&startIndex=<%=startIndex%>'>View</option>
								
                                      <option value='goodsMaster.do?event=maker_edit_goods_master_read&id=<%=ob.getId()%>&goodsCode=<%=ob.getGoodsCode()%>&goodsName=<%=ob.getGoodsName()%>&startIndex=<%=startIndex%>'>Edit</option>
                                      
                                      <option value='goodsMaster.do?event=maker_delete_goods_master_read&id=<%=ob.getId()%>&goodsCode=<%=ob.getGoodsCode()%>&goodsName=<%=ob.getGoodsName()%>&startIndex=<%=startIndex%>'>Delete</option>
					        ...
					        <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
					            <tr>
					                <td valign="middle">
					                    <integro:pageindex
					                            pageIndex="<%=new PageIndex(startIndex, 10, sr.getResultList().size())%>"
					                            url="goodsMaster.do?event=list_pagination&startIndex="/>
					                </td>
					            </tr>
					        </table>
					    </td>
