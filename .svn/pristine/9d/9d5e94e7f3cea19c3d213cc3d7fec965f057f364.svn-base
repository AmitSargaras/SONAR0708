<%@ page import="com.integrosys.cms.ui.collateral.marketablesec.PortItemForm,
                 ...
                 com.integrosys.cms.ui.collateral.marketablesec.marksecbondforeign.BondTypeList,
    ...
    <tr>
      <td> <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo">
          <tbody>
            ...
            <tr class="odd">
              <td width="20%" class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "noOfUnits")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.marketsec.port.detail.noofunits"/>&nbsp;
            ...
            <tr class="even">
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "leadManager")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.lead.manager.syndicate"/></td>
              <td ><bean:write name="PortItemForm" property="leadMgr"/>&nbsp;</td>
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "stockExchangeCountry")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.bond.market.country"/></td>
              <td><integro:empty-if-null value="<%=CountryList.getInstance().getCountryName(aForm.getCountryBondMarket())%>"/>&nbsp;</td>
            </tr>
            <tr class="odd">
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "settlementOrganisation")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.settlement.org"/></td>
              <td ><integro:empty-if-null value="<%=SettleOrgList.getInstance().getSettlementOrgItem(aForm.getSettleOrg())%>"/>&nbsp;</td>
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "bondIssueDate")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.bond.issue.date"/></td>
              <td ><bean:write name="PortItemForm" property="bondIssueDate"/>&nbsp;</td>
            </tr>
            <tr class="even">
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "bondMaturityDate")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.bond.maturity.date"/></td>
              <td ><bean:write name="PortItemForm" property="bondMatDate"/>&nbsp;</td>
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "stockExchange")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.bond.market"/></td>
              <td ><integro:empty-if-null value="<%=StockExchangeList.getInstance().getStockExchangeName(aForm.getBondMarket())%>"/>&nbsp;</td>
            </tr>
            <tr class="odd">
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "equityType")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.bond.type"/>&nbsp;<span class="mandatory">*</span></td>
