...
{%>
   ...
   %>
    <tr class="<%=rowClass%>">
              	...
              	<td valign="top" >&nbsp;
              	<%
              	if(paripassuChecklistId .equals("-999999999")){
              	%>
              	-
              	<%}else {%>
              	
              	<%=paripassuChecklistId %>
        ...
        <a href="MaintainPariPassuCheckListAction.do?event=maintain&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=paripassuChecklistId %>&&secType=PARIPASSU&&secSubType=PARIPASSU"  ><bean:message key="label.process"/></a>
