...
<%
		...
		String fileUploadSystem=(String)request.getAttribute("fileUploadSystem");
		if(fileUploadSystem==null){
			fileUploadSystem = "";
		}
...
<table width="50%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:30px">
 ...
 <thead>
    <tr>
      <td><h3>
      <%if(fileUploadSystem.equals("UBS")){ %>
      UBS File List
      <%}else if(fileUploadSystem.equals("FINWARE")){ %>
       FINWARE File List
      <%}else if (fileUploadSystem.equals("HONGKONG")){ %>
       HONGKONG File List
      <%}else if (fileUploadSystem.equals("BAHRAIN")){ %>
       BAHRAIN File List
      <%}else if ("FD".equals(fileUploadSystem)){ %>
       FD File List
      <%}
      %>
          <hr/>
                     ...
                     type="java.io.File" scope="page">
                        ...
                        %>
                                <tr class="<%=rowClass%>">
                                  ...
                                  <%if(fileUploadSystem.equals("UBS")){ %>
                                   <td><center><a href="fileUpload.do?event=maker_upload_ubs_file&path=<%=ob.getPath() %>" onclick="javascript : newShowSilkScreen()"  ><%=ob.getName() %></a></center></td> 
                                   <%}else if(fileUploadSystem.equals("FINWARE")){ %>
                                     <td><center><a href="fileUpload.do?event=maker_upload_finware_file&path=<%=ob.getPath() %>"  onclick="javascript : newShowSilkScreen()"   ><%=ob.getName() %></a></center></td> 
                                   <%}else if(fileUploadSystem.equals("HONGKONG")){ %>
                                     <td><center><a href="fileUpload.do?event=maker_upload_hongkong_file&path=<%=ob.getPath() %>"  onclick="javascript : newShowSilkScreen()"   ><%=ob.getName() %></a></center></td> 
                                    <%}else if(fileUploadSystem.equals("BAHRAIN")){ %>
                                     <td><center><a href="fileUpload.do?event=maker_upload_bahrain_file&path=<%=ob.getPath() %>"  onclick="javascript : newShowSilkScreen()"   ><%=ob.getName() %></a></center></td> 
                                     <%} else if("FD".equals(fileUploadSystem)){ %>
                                     <td><center><a href="fileUpload.do?event=maker_upload_fd_file&path=<%=ob.getPath() %>"  onclick="javascript : newShowSilkScreen()"   ><%=ob.getName() %></a></center></td> 
                                     <%}
                                     %>
                                </tr>
                                </logic:iterate>
