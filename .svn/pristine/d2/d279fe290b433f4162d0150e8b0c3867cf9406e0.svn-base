...
List tageedImageList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.LADreceipt.LADReceiptAction.tagDetailList");
...
function showListing() {
	//alert('1');
	var catg= '<%=category%>';
	if('<%=event%>'== 'draft_lad_img')
	{
		 document.forms[0].action="LADReceipt.do?event=close_draft_view_image&category="+catg;
		   document.forms[0].event.value="close_draft_view_image";
		  
		    document.forms[0].submit();
	}
	if('<%=event%>'== 'draft_lad_process_img')
	{
		 document.forms[0].action="LADReceipt.do?event=process_draft_view_image&category="+catg;
		   document.forms[0].event.value="process_draft_view_image";
		  
		    document.forms[0].submit();
	}
}

function showChkListing() {
	//alert('2');
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=view_attach_image&category="+catg;
		   document.forms[0].event.value="view_attach_image";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'draft_lad_img'){
			 document.forms[0].action="LADReceipt.do?event=draft_lad_img&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
			if('<%=event%>' == 'draft_lad_process_img'){
				 document.forms[0].action="LADReceipt.do?event=draft_lad_process_img&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
