...
* $Header:$
*
* credit_risk_param_unittrust_select.jsp
*
* Describe this JSP.
* Purpose:
* Description:
...
function submitForm(thisForm) {
    var list = thisForm.groupFeedId;
    document.forms[0].countrylabel.value = list.options[list.selectedIndex].text;
...
<%
    ...
    String event = null;
//    if (actionForm.getEvent().equals(CreditRiskParamUnitTrustAction.EVENT_SELECT) ||
//            actionForm.getEvent().equals(CreditRiskParamUnitTrustAction.EVENT_READ)) {
//        event = CreditRiskParamUnitTrustAction.EVENT_READ;
//    } else 
    if (actionForm.getEvent().equals(CreditRiskParamUnitTrustAction.EVENT_SELECT_MAKER_EDIT)) {
        ...
        if (roleType == ICMSConstant.TEAM_TYPE_SC_MAKER||roleType == ICMSConstant.TEAM_TYPE_SC_MAKER_WFH) {
            event = CreditRiskParamUnitTrustAction.EVENT_PREPARE;
        }
        else {
            event = CreditRiskParamUnitTrustAction.EVENT_TO_TRACK;
        }     
    }
...
<html:form action="CreditRiskParamUnitTrust.do">
    ...
    <input type="hidden" name="firstEvent" value=<%=actionForm.getEvent()%>/>
    <input type="hidden" name="offset" value="0"/>
    <input type="hidden" name="length" value="10"/>
    <input type="hidden" name="countrylabel"/>
            ...
            <tr>
                <td>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                        <thead>
                            <tr>
                                <td colspan="2"><bean:message key="label.please.select.the.country"/></td>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="odd">
                                ...
                                <td>
                                    <html:select property="groupFeedId" onchange="submitForm(this.form)">
                                        <html:option value=""><bean:message key="label.please.select"/></html:option>
                                        <html:options name="listValue" labelName="listLabel"/>
                                        <%--<integro:country-list/>--%>
                                    </html:select>
                                    <html:errors property = "groupFeedId"/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
