...
<head>
    ...
    <script language="JavaScript" type="text/JavaScript">
        ...
        function goView(folderPath, name, reportID, fileName) {
            var strURL = 'ReportServlet?flag=view&scope=<%=ReportConstants.ANALYSIS_CATEGORY%>&save='+fileName+'&reportId='+reportID+'&path='+folderPath+'&name='+name;
...
<form action="ConcentrationReportSearch.do">
    ...
    <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
        <thead>
            <tr>
                <td><h3><bean:message key="title.whatifanalysis" /> <bean:message key="title.misreport" /> - <%=searchDate%></h3></td>
            </tr>
            <tr>
                <td><hr/></td>
            </tr>
            ...
            <tr>
                <td>
                    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
                        <thead>
                            <tr>
                                <td width="9%"><bean:message key="label.global.sn"/></td>
                                <td width="43%"><bean:message key="label.whatifcond.report.name" /></td>
                                <td width="26%"><bean:message key="label.whatifcond.report.short.desc" /></td>
                                <td width="22%"><bean:message key="label.global.action" /></td>
                            </tr>
                        </thead>
                        <tbody>
                            <logic:present name="OBReportList">
                                <logic:iterate id="OB" name="OBReportList" type="com.integrosys.cms.batch.reports.OBReport">
                                    ...
                                    <tr class="<%=(rowIdx++)%2==0?"even":"odd"%>">
                                        ...
                                        <td>
											<% if (OB.getMISReportNumber().equals(WhatIfCondReportConstants.REPORT_TYPE_STATE)) { %>
                                        		<bean:message key="label.report.ANA001.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals(WhatIfCondReportConstants.REPORT_TYPE_INDEX_TYPE)) {  %>
                                        		<bean:message key="label.report.ANA002.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals(WhatIfCondReportConstants.REPORT_TYPE_MAKE)) {  %>
                                        		<bean:message key="label.report.ANA003.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals(WhatIfCondReportConstants.REPORT_TYPE_GOLD_GRADE)) {  %>
                                        		<bean:message key="label.report.ANA004.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals(WhatIfCondReportConstants.REPORT_TYPE_ALL_MAKE)) {  %>
                                        		<bean:message key="label.report.ANA003.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals("ANA006")) {  %>
                                        		<bean:message key="label.report.ANA001.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals("ANA007")) {  %>
                                        		<bean:message key="label.report.ANA002.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals("ANA008")) {  %>
                                        		<bean:message key="label.report.ANA003.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals("ANA009")) {  %>
                                        		<bean:message key="label.report.ANA004.short.desc"/>&nbsp;
											<% } else if (OB.getMISReportNumber().equals("ANA010")) {  %>
                                        		<bean:message key="label.report.ANA003.short.desc"/>&nbsp;
											<% } %>
