...
<%
	...
	if (sr != null) {
		holidayList = new ArrayList(sr.getResultList());
    ...
    if(user.getTeamTypeMembership().getMembershipID()==1004||user.getTeamTypeMembership().getMembershipID()==1031
    		||user.getTeamTypeMembership().getMembershipID()==1033){
    	isMaker=true;
    }else {
    	isChecker=true;
    }
...
<!--
	...
	function searchBranch() {
		...
		if (document.getElementById("searchText").value == ""
				...
				|| document.getElementById("searchByCriteria").value == "") {
			...
			document.forms[0].action = "systemBank.do?event=maker_search_list_holiday&searchBy="
	...
	function checkerSearchBranch() {
		...
		if (document.getElementById("searchText").value == ""
			...
			document.forms[0].action = "holiday.do?event=checker_search_list_holiday&searchBy="
					+ searchBy + "&searchText=" + searchText;
	...
	function uploadMasterFile() {
		document.forms[0].action = "holiday.do?event=maker_prepare_upload_holiday";
		document.forms[0].submit();
	}
//-->
</script>
					...
					<table width="70%" border="0" align="center" cellpadding="0"
						...
						<thead>
							...
							<tr>
								...
								<td valign="baseline"><input type="button" name="Submit"
									value="Add New" class="btnNormal" onclick="createHoliday()" /></td>
								<td valign="baseline"><input type="button"
									name="fileupload" value="Upload" class="btnNormal"
									onclick="uploadMasterFile()" style="margin-left: 10px;" /></td>
								...
								<td colspan="4"><table width="100%" border="0"
										...
										<tbody>
											<logic:present name="holidayList">
												<logic:iterate id="ob" name="holidayList" length="10"
													offset="<%=String.valueOf(startIndex)%>" indexId="counter"
													type="com.integrosys.cms.app.holiday.bus.OBHoliday"
													scope="page">
													<%
														String rowClass = "";
																	sno++;
													...
													<tr class="<%=rowClass%>">
														...
														<td><integro:date
																object="<%=ob.getLastUpdateDate()%>"></integro:date>&nbsp;</td>
														...
														<td><select name="menu1"
															onchange="MM_jumpMenu('self',this,0)" style="width: 110">
																...
																<option
																	value='holiday.do?event=maker_view_holiday&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>

																<option
																	value='holiday.do?event=maker_edit_holiday_read&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Edit</option>
																<option
																	value='holiday.do?event=maker_delete_holiday_read&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Delete</option>
																...
																<option
																	value='holiday.do?event=checker_view_holiday&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
																<%
																	}
																%>
														</select></td>
							...
							<tr>
								...
								<%
									String pageIndexUrl = null;
											...
											pageIndexUrl = "holiday.do?event=maker_list_holiday&startIndex=";
											System.out.println(pageIndexUrl);
											System.out.println(sr.getNItems());
								%>
								<td width="86" valign="middle">&nbsp;<integro:pageindex
										pageIndex='<%=new PageIndex(startIndex, 10, holidayList.size())%>'
										url='<%=pageIndexUrl%>' />
