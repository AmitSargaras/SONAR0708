<%@ page import="com.integrosys.cms.ui.commoditydeal.finance.FinanceAction,
                 ...
                 com.integrosys.cms.app.commodity.deal.bus.finance.SettleWarehouseReceiptComparator,
                 java.util.Arrays,
...
* $Header: /home/cms2/cvsroot/cms2/public_html/commoditydeal/finance/finance_process.jsp,v 1.24 2006/10/27 08:39:32 hmbao Exp $
*
  ...
  <tr>
    <td colspan="2">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
        ...
        <tbody>
<%
        ISettlement[] actualSettlement = null;
        if (actual != null) {
            actualSettlement = actual.getSettlements();
        }
        ISettlement[] stageSettlement = null;
        if (staging != null) {
            stageSettlement = staging.getSettlements();
        }

        ArrayList list = null;
        if (actualSettlement != null || stageSettlement != null) {
            List res = CompareOBUtil.compOBArray(stageSettlement,actualSettlement);
            list = new ArrayList(res);
            pageContext.setAttribute("settlement",list);
        } else {
            list = null;
        }
...
<%
            ...
            int count = 0;
%>
		<logic:present name="settlement" >
        <logic:iterate id="compResult" name="settlement"  type="com.integrosys.base.techinfra.diff.CompareResult"  >
        <%
            ISettlement settlementObj = (ISettlement)compResult.getObj();
        %>
        <tr class="<%=count%2==0?"odd":"even"%>">
          <td class="<bean:write name="compResult" property="key" />" style="text-align:center" width="8%"><%=count + 1%></td>
          ...
          <%        String paymentAmt = null;
                    if (settlementObj.getPaymentAmt() != null && settlementObj.getPaymentAmt().getAmount()>= 0) {
                        paymentAmt = settlementObj.getPaymentAmt().getCurrencyCode()+" "+
                                MapperUtil.mapDoubleToString(settlementObj.getPaymentAmt().getAmount(), 6, locale);
                    }
          %>
          <td style="text-align:center" width="38%"><integro:empty-if-null value="<%=paymentAmt%>"/></td>
          <td style="text-align:center" width="16%"><a href="javascript:viewSettlement(<%=settlementObj.getRefID()%>)">View</a></td>
        </tr>
<%
            count++;
%>
  ...
  <tr>
    <td colspan="2">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
        ...
        <tbody>
<%
        IReceiptRelease[] actualRelease = null;
        SettleWarehouseReceiptComparator c =  new SettleWarehouseReceiptComparator();
        if (actual != null) {
            actualRelease = actual.getReceiptReleases();
            if (actualRelease != null) {
                for (int i = 0; i < actualRelease.length; i++) {
                    ISettleWarehouseReceipt[] settleWR = actualRelease[i].getSettleWarehouseReceipts();
                    if (settleWR != null) {
                        Arrays.sort(settleWR, c);
                    }
                    actualRelease[i].setSettleWarehouseReceipts(settleWR);
                }
            }
        }
        IReceiptRelease[] stageRelease = null;
        if (staging != null) {
            stageRelease = staging.getReceiptReleases();
            if (stageRelease != null) {
                for (int i = 0; i < stageRelease.length; i++) {
                    ISettleWarehouseReceipt[] settleWR = stageRelease[i].getSettleWarehouseReceipts();
                    if (settleWR != null) {
                        Arrays.sort(settleWR, c);
                    }
                    stageRelease[i].setSettleWarehouseReceipts(settleWR);
                }
            }
        }
        ...
        if (actualRelease != null || stageRelease != null) {
            List res = CompareOBUtil.compOBArray(stageRelease,actualRelease);
            list = new ArrayList(res);
            pageContext.setAttribute("release",list);
        } else {
            list = null;
        }
        if (list == null || list.size() == 0) {
...
%>
        ...
        <logic:iterate id="compResult" name="release"  type="com.integrosys.base.techinfra.diff.CompareResult"  >
        <%
            IReceiptRelease releaseObj = (IReceiptRelease)compResult.getObj();
        %>
        <tr class="<%=count%2==0?"odd":"even"%>">
          <td class="<bean:write name="compResult" property="key" />" style="text-align:center" width="7%"><%=count + 1%></td>
          ...
          <td style="text-align:center" width="13%"><a href="javascript:viewRelease(<%=releaseObj.getRefID()%>)">View</a></td>
        </tr>
<%
            count++;
%>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo">
        ...
        <tr class="even">
          <td width="100%" style="padding-top:5px;padding-bottom:5px">
            <table width="98%" border="0" cellpadding="0" cellspacing="0" class="tblInfo"  id="divfloat" name="divfloat">
              <tbody>
              <tr class="odd">
