...
<%
	...
	ICollateralLimitMap actualObj = (ICollateralLimitMap) request.getAttribute("actualColLimitMap");
	ICollateralLimitMap stageObj = (ICollateralLimitMap) request.getAttribute("stageColLimitMap");
%>
...
<table class="tblFormSection" width="98%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:10px">
	...
	<tbody>
		...
		<td>
			...
			<tbody>
				<tr class="<%=style[count++%2] %>">
				<td class="<%=CompareOBUtil.compOB(stageObj,actualObj, "limitID")?"fieldname":"fieldnamediff"%>" width="20%"><bean:message key="label.pledge.facility.no"/> </td>
		...
		<td>
			...
			<tbody>
				<tr class="<%=style[count++%2] %>">
				<%
					boolean unChanged = CompareOBUtil.compOB(stageObj, actualObj, "pledgeAmountPercentage");
					Amount actualAmt = (actualObj != null ? actualObj.getPledgeAmount() : null);
					Amount stageAmt = (stageObj != null ? stageObj.getPledgeAmount() : null);
					unChanged = unChanged && UIUtil.isAmountUnChanged(stageAmt, actualAmt);			
				%>
				<td class="<%=unChanged ? "fieldname" : "fieldnamediff" %>" width="15%"><bean:message key="label.pledge.amount"/> </td>
				<td width="85%">
				...
				<tr class="<%=style[count++%2] %>">
				<%
					unChanged = CompareOBUtil.compOB(stageObj, actualObj, "drawAmountPercentage");					
					actualAmt = (actualObj != null ? actualObj.getDrawAmount() : null);
					stageAmt = (stageObj != null ? stageObj.getDrawAmount() : null);
					unChanged = unChanged && UIUtil.isAmountUnChanged(stageAmt, actualAmt);															
				%>
				<td class="<%=unChanged ? "fieldname" : "fieldnamediff" %>"><bean:message key="label.pledge.amount.draw"/></td>
				<td>
