            ...
            <tr>
              <td> <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo" id="pledgorInput">
				...
				<%
                    ...
                    ICollateralLimitMap[] oldLimitMap = oldCollateral.getCurrentCollateralLimits();
                    ICollateralLimitMap[] newLimitMap = null;
                    if (isProcess) {
	                    newLimitMap = newCollateral.getCurrentCollateralLimits();
                    }
				...
				%>
                 ...
                 %>               
		     	 ...
		     	 %>
                    ...
                    <tr class="<%=classtype%>">
                      <td width="5%" class="<%=isProcess?(CompareOBUtil.compOB(newLimitMap[counter],oldLimitMap[counter],"priorityRanking")?"index":"indexdiff"):"index"%>"><%=iCounter + 1%></td>
                      <td width="20%">
                        <%
                      if(isMB){
                      out.println(limitMap[counter].getSCILimitID());
                      }
                      else
                      {
	                  out.println(limitMap[counter].getSCICoBorrowerLimitID());
	                  }       
                      %>
                      </td>
                      <%
	                    ...
	                    obLimit=(ILimit)limitAmountMap.get(""+limitMap[counter].getSCICoBorrowerLimitID());
                      	...
                      	if (limitMap[counter].getPriorityRanking()== ICMSConstant.INT_INVALID_VALUE ) {
							priorityRanking="-";				        
		        		} else {
							priorityRanking = new Integer(limitMap[counter].getPriorityRanking()).toString();
		       		 	}
                      	%>
                      <td width="28%"><integro:currency amount="<%=amount%>"/>&nbsp;</td>
                      <td width="27%"><integro:empty-if-null value="<%=priorityRanking%>"/></td>
                    </tr>
