<%@ page import="com.integrosys.cms.ui.collateral.marketablesec.PortItemForm,
                 com.integrosys.cms.ui.collateral.marketablesec.marksecotherlistedforeign.EquityTypeList,
...
<body>
<input type="hidden" name="spanISINCODE" id="spanISINCODE"/>
    ...
    <tr>
      <td> <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo">
          <tbody>
            ...
            <tr class="even">
              <td width="20%" class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "registeredName")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.reg.owner"/></td>
            ...
            <tr class="even">
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "stockExchange")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.stock.exchange.country"/></td>
			  <td><integro:empty-if-null value="<%=CountryList.getInstance().getCountryName(aForm.getCountryStockExchange())%>"/>&nbsp;</td>
              <td class="<%=isProcess?(CompareOBUtil.compOB(stagingOB,originOB, "stockExchangeCountry")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.local.stock.exchange"/></td>
			  <td >
              <%
                  if (aForm.getLocalStockExchange().equals(ICMSConstant.TRUE_VALUE) || Boolean.valueOf(aForm.getLocalStockExchange()).booleanValue()) {
              %>
              Yes
              <% } else { %>
              No
