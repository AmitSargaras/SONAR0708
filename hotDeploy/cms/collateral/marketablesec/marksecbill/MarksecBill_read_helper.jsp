<%@ page import="com.integrosys.base.techinfra.diff.CompareOBUtil,
                 ...
                 com.integrosys.cms.app.collateral.bus.type.marketable.subtype.govtlocal.IGovtLocal,
...
* $Header: /home/cms2/cvsroot/cms2/public_html/collateral/marketablesec/marksecbill/MarksecBill_read_helper.jsp,v 1.45 2006/07/27 01:59:53 jzhan Exp $
*
* Purpose: Helper page to display data for read and process
* Description: Type - Marketable Securities, Subtype - Govt Local - T Bills
*
* @author $Author: jzhan $<br>
* @version $Revision: 1.45 $
* Date: $Date: 2006/07/27 01:59:53 $
* Tag: $Name:  $
*/
%>

<%
    IGovtLocal oldCollateral = (IGovtLocal) itrxValue.getCollateral();
    IGovtLocal newCollateral = (IGovtLocal) itrxValue.getStagingCollateral();
    String formName = "MarksecBillForm";
    String prefix = "Mark";
    String subtype = "MarksecBill";
...
%>
                     <tr class="<%=classtype%>">
                     <td colspan="5">There is no security detail.</td>
                     </tr>
           ...
           %>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=marketEquity.getUnitPrice().getCurrencyCode()%>"/>&nbsp;<%=marketEquity.getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecbill_read&indexID=<%=marketEquity.getRefID()%>&subtype=MarksecBill&from_event=<%=from_event%>">View</a>
                      </td>
                    </tr>
...
%>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=equity[counter].getUnitPriceCcyCode()%>"/>&nbsp;<%=equity[counter].getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecbill_read&indexID=<%=equity[counter].getRefID()%>&subtype=MarksecBill&from_event=<%=from_event%>"><bean:message key="label.view"/></a>
                      </td>
