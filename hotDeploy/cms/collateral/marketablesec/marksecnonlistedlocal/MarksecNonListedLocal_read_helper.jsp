<%@ page import="com.integrosys.base.techinfra.diff.CompareOBUtil,
                 ...
                 com.integrosys.cms.app.collateral.bus.type.marketable.subtype.nonlistedlocal.INonListedLocal,
                 java.util.List,
                 java.util.ArrayList,
...
* $Header: /home/cms2/cvsroot/cms2/public_html/collateral/marketablesec/marksecnonlistedlocal/MarksecNonListedLocal_read_helper.jsp,v 1.47 2006/09/22 12:28:46 nkumar Exp $
*
* Purpose: Helper page to display data for read and process
* Description: Type - Marketable Securities, Subtype - non listed local
*
* @author $Author: nkumar $<br>
* @version $Revision: 1.47 $
* Date: $Date: 2006/09/22 12:28:46 $
* Tag: $Name:  $
*/
%>

<%
    INonListedLocal oldCollateral = (INonListedLocal) itrxValue.getCollateral();
    INonListedLocal newCollateral = (INonListedLocal) itrxValue.getStagingCollateral();
    String prefix = "Mark";
    String subtype = "MarksecNonListedLocal";
		...
		<logic:present name="equity" >
           ...
           %>
                    <tr class="<%=classtype%>">
                      ...
                      <td width="20%">
                      <% if (marketEquity.getUnitPrice() != null && marketEquity.getUnitPriceCcyCode() != null &&
                              marketEquity.getUnitPriceCcyCode().length() > 0) { %>
                      <integro:empty-if-null value="<%=marketEquity.getUnitPriceCcyCode()%>"/>&nbsp;<%=marketEquity.getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecnonlistedlocal_read&indexID=<%=marketEquity.getRefID()%>&subtype=MarksecNonListedLocal&from_event=<%=from_event%>">View</a>
                      </td>
                    </tr>
...
%>
                    <tr class="<%=classtype%>">
                      ...
                      <td width="20%">
                      <% if (equity[counter].getUnitPrice() != null && equity[counter].getUnitPriceCcyCode() != null &&
                              equity[counter].getUnitPriceCcyCode().length() > 0) { %>
                      <integro:empty-if-null value="<%=equity[counter].getUnitPriceCcyCode()%>"/>&nbsp;<%=equity[counter].getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecnonlistedlocal_read&indexID=<%=equity[counter].getRefID()%>&subtype=MarksecNonListedLocal&from_event=<%=from_event%>">View</a>
                      </td>
