...
function submitForm() {
	...
	document.forms[0].action='AssetInsuranceCollateral.do?event=maker_update_insdeferred_list&collateralID=<%=itrxValue.getReferenceID()%>&calculatedDP=<%=dp%>&fundedShare=<%=fundedShare%>&dueDate=<%=dueDate%>&insuranceCode=<%=insuranceObj.getInsuranceCode()%>';
