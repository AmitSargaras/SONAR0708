                   ...
                   <td width="20%">
                        <% if (cheque[counter].getChequeAmount() != null ) { 
                        	obj.setChequeAmt( String.valueOf(cheque[counter].getChequeAmount().getAmount()));
                          	BigDecimal bdreq = new BigDecimal(cheque[counter].getChequeAmount().getAmount()).setScale(0, BigDecimal.ROUND_HALF_UP);
                     ...
                     <td> <select name="menu1" onchange="MM_jumpMenu('self',this,0)" style="width:110">
                                  ...
                                  if(forward_event!=null && !forward_event.equals("")){
                                	  ...
                                	  %>  
                                        <option value='AssetPostDatedChqsCollateral.do?event=maker_view_list&indexID=<%=counter%>&branchCode=<%=cheque[counter].getBranchCode()%>&itemType=CHEQUE&trxID=<%= trxID %>&pdcEvent=<%=forward_event %>&collateralID=<%=collateralID %>'>View</option>
                                     <%}
                                	  else if(forward_event.equals("track")){  %>
                                      <option value='AssetPostDatedChqsCollateral.do?event=maker_view_list&indexID=<%=counter%>&branchCode=<%=cheque[counter].getBranchCode()%>&itemType=CHEQUE&trxID=<%= trxID %>&pdcEvent=<%="close_totrack" %>&collateralID=<%=collateralID %>'>View</option>
