...
* $Header: /home/cms2/cvsroot/cms2/public_html/collateral/guarantees/gtebankdiff/GteBankDiff_read_helper.jsp,v 1.48 2006/07/27 01:48:07 jzhan Exp $
*
...
* Date: $Date: 2006/07/27 01:48:07 $
* Tag: $Name:  $
*/
%>

<%
    IBankDifferentCurrency oldCollateral = (IBankDifferentCurrency) itrxValue.getCollateral();
    IBankDifferentCurrency newCollateral = (IBankDifferentCurrency) itrxValue.getStagingCollateral();
    String prefix = "Gte";
    String subtype = "GteBankDiff";
...
<%@ include file="/collateral/common/common_legal_enforceability_read.jsp" %>    
    ...
    <tr>
      <td colspan="2"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo">
          <tbody>
            <tr class="even">
              <td width="20%" class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"description")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.desc.guarantees"/></td>
            ...
            <tr class="even">
              <td class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"referenceNo")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.ref.num.guarantee"/></td>
            ...
            <td class="<%=isProcess?((CompareOBUtil.compOB(newCollateral.getGuaranteeAmount(),((oldCollateral==null)?null:oldCollateral.getGuaranteeAmount()),"amount"))?"fieldname":"fieldnamediff"):"fieldname"%>">
              ...
              <td><bean:write name="GteBankDiffForm" property="amtGuarantee"/>&nbsp;</td>
            </tr>
            <tr class="odd">
            ...
            <td class="<%=isProcess?(CompareOBUtil.compOB(newLimit,oldLimit,"chargeType")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.charge.type"/>&nbsp;<font color="#0000FF">* </font></td>
            <td><% if (!isLimitNull) { %>
            ...
            <td width="20%" class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"isBankCountryRiskApproval")?"fieldname":"fieldnamediff"):"fieldname"%>">
            ...
            <td width="20%"  class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"isExchangeCtrlObtained")?"fieldname":"fieldnamediff"):"fieldname"%>">
              <bean:message key="label.security.exchange.ctr.app"/>&nbsp;</td>
              <td width="30%"><integro:empty-if-null value="<%=ExchangeControlList.getInstance().getExchangeControlItem(iCol.getIsExchangeCtrlObtained())%>"/>&nbsp;</td>
            </tr>
            <tr class="even">
              <td class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"issuingBank")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.issuing.bank"/></td>
              <td><integro:empty-if-null value="<%=iCol.getIssuingBank()%>" />&nbsp;</td>
              <td class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"issuingBankCountry")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.issuing.bank.country"/></td>
              <td><integro:empty-if-null value="<%=CountryList.getInstance().getCountryName(iCol.getIssuingBankCountry())%>"/>&nbsp;</td>
            </tr>
            <tr class="odd">
              <td class="<%=isProcess?(CompareOBUtil.compOB(newCollateral,oldCollateral,"beneficiaryName")?"fieldname":"fieldnamediff"):"fieldname"%>"><bean:message key="label.security.beneficiary.name"/></td>
