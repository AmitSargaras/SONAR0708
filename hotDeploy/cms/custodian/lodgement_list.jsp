<%
     ...
     * @since $Date: 2006/11/16 08:54:14 $
...
<%@ page import="com.integrosys.base.businfra.search.SearchResult,
 ...
 <%     SearchResult sr = (SearchResult)session.getAttribute("com.integrosys.cms.ui.custodian.CustodianAction.docList");
        ...
        String checkListStatus = "";
           ...
           DefaultLogger.debug(this,custodianDocList);
        ...
        if (custodianDocList!=null)
        {
            ...
            for (int i=0; i<custodianDocList.size(); i++)
            {
                ...
                //DefaultLogger.debug("lodgement_list.jsp","docItemRef="+custSearchResult.getDocItemRef()+", cat="+custSearchResult.getCategory()+", subCat="+custSearchResult.getSubCategory());
                if (category.equals("S"))
                {
                    ...
                    if (collateral!=null)
                    {
                        ...
                        String combinedSecType = securityID + "/" + secType + "/" + secSubType; //concatenates securityID, type and sub-type to form unique string
                        ...
                        if (!uniqueSecTypeVect.contains(uniqueSecDetailVect))
                        {
                         ...
                         //   DefaultLogger.debug("lodgement_list.jsp","unique securityID/secType/secSubType="+combinedSecType);
                            //uniqueSecID.add(securityID);
                        }
                        //DefaultLogger.debug("lodgement_list.jsp","unique securityID/secType/secSubType="+combinedSecType+", docItemRef="+custSearchResult.getDocItemRef());
                    }
                ...
                else if (subCategory!=null)
                {
                    ...
                    else if (subCategory.equals("CO_BORROWER"))
                    {
                        ...
                        //DefaultLogger.debug("lodgement_list.jsp","no. of unique co-borrowers="+uniqueCoBorrowerIDVect.size());
                    }
                    else if (subCategory.equals("PLEDGOR"))
                    {
                        ...
                        //DefaultLogger.debug("lodgement_list.jsp","no. of unique pledgors="+uniquePledgorIDVect.size());
                    }
...
function submitPage() {
    document.forms[0].memoType.value="lodgement";
...
<html:form action='custodian.do?event=print_lodgement_memo'>
<!-- hidden field indicates that this is a lodgement (instead of withdrawal) memo -->
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10px">
  <thead>
    <tr>
      <td><h3>Print Lodgement Memo</h3></td>
    </tr>
    <tr>
                ...
                <table  width="100%" border="0" cellpadding="0" cellspacing="0" class="tblinfo" style="margin-top:0">
                  <thead>
                    <tr>
                      ...
                      <td>Doc Date</td>
                      <td>Expiry Date</td>
                      <td>Print Lodgement <br> <input type="checkbox" name="print" id="print" onClick="checkedPledgorAll()"
...
/*
                            ...
                            if (checkListItem!=null)
                            {
                                ...
                                //DefaultLogger.debug("lodgement_list.jsp","checkListItem.getParentItemID()="+checkListItem.getParentItemID());
                                //DefaultLogger.debug("lodgement_list.jsp","checkListItem.getItem().getItemID()="+checkListItem.getItem().getItemID());
                            }
*/
        %>
                            <tr class=<%=rowClass%>>
...
/*
                                ...
                                if (checkListItem!=null)
                                {
                                    ...
                                    docDate = checkListItem.getDocDate();
                                    expiryDate = checkListItem.getExpiryDate();
                                    //ICheckListTrxValue checkListTrxValue = checkListItem.
                                    //DefaultLogger.debug("lodgement_list.jsp","checkListItem.getParentItemID()="+checkListItem.getParentItemID());
                                    //DefaultLogger.debug("lodgement_list.jsp","checkListItem.getItem().getItemID()="+checkListItem.getItem().getItemID());
                                }
*/
...
<%              }
                ...
                if (pledgorOB!=null)
                {
                    legalID =Long.parseLong( pledgorOB.getLegalID());    //edit
...
%>
        <tr>
            <td colspan=9 class="odd">
                <B>There are no C/C Documents for Lodgement </B>
            </td>
        </tr>
<%  } else {
        sno=0;
...
%>
                ...
                <td align="center">
                    ...
                    <tbody>
                        ...
                        <tr class="even">
							...
							<td><%=(customerCategory==null || customerCategory.equals("")) ? "-" : customerCategory%></td>
                        </tr>
						<tr class="even">
    ...
    <%
                for (int i=0; i<securitiesVect.size(); i++)
                {
                    ...
                    {
                        ICustodianDocItemSearchResult item = itemArr[ind];
                        //DefaultLogger.debug("lodgement_list.jsp","security docItemRef="+item.getDocItemRef()+", type="+collateralType.getTypeName()+", subType="+collateralSubType.getSubTypeName());
...
%>
            <tr>
                <td colspan=6 class="odd">
                    <B>There are no Security Documents for Lodgement</B>
                </td>
            </tr>
<%      }
%>
      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInput">
          ...
          <tbody>
            <tr class="odd">
              ...
              <td><input name="authzName1" type="text" size="30" value="<%=form.getAuthzName1()%>" /><html:errors property="authzName1Error" /></td>
              <td class="fieldname">Signing Number&nbsp;<font color="#0000FF">* </font></td>
              <td><input name="signNum1" type="text" size="30" value="<%=form.getSignNum1()%>"  /><html:errors property="signNum1Error" /></td>
            </tr>
          </tbody>
        </table></td>
    </tr>
      ...
      <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInput">
          ...
          <tbody>
            <tr class="even">
              ...
              <td><input name="authzName2" type="text" size="30" value="<%=form.getAuthzName2()%>" /><html:errors property="authzName2Error" />
...
%>
    MM_openBrWindow('/cms/custodian/memo.jsp?event=l&a1=<%=form.getAuthzName1()%>&a2=<%=form.getAuthzName2()%>&n1=<%=form.getSignNum1()%>&n2=<%=form.getSignNum2()%>&rea=<%=form.getReason()%>','','scrollbars=yes,toolbar=yes,menubar=yes,resizable=yes,width=760,height=400,left=5,top=5');
<%}%>
//-->
</script>