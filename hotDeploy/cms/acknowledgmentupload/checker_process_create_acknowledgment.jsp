...
<%
	...
	IFileUploadTrxValue itrxValue = (IFileUploadTrxValue) session.getAttribute("com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentUploadAction.IFileUploadTrxValue");
	List acknowledgmentList = (List) session.getAttribute("com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentUploadAction.acknowledgmentList");
	
	if (acknowledgmentList != null) {
		pageContext.setAttribute("acknowledgmentList", acknowledgmentList);
	}
	...
	DefaultLogger.debug(this,"acknowledgmentList size : "+ acknowledgmentList.size());
	
	int sno = 0;
	String heading = "CERSAI Acknowledgment Upload";
	String approve_event = "checker_approve_create_acknowledgment";
...
<%@page import="com.integrosys.cms.app.common.util.CommonUtil"%><html:form
	action='Acknowledgmentupload.do?'>
	<head>
...
function downloadImage(){ //v3.0
		var aLocation = "Acknowledgmentupload.do?event=downloadReport&system=";
...
function approvePage() {
	...
	//	if (x == 'Y') {
		if (x == true) {
		    document.forms[0].action="Acknowledgmentupload.do?event="+'<%=approve_event%>';
...
function rejectPage() {
	var x =document.getElementById("confirm").checked;
	if (x == true) {
    document.forms[0].action="Acknowledgmentupload.do?event=checker_reject_create_acknowledgment";
    document.forms[0].submit();
	}else{
        alert('Please confirm.!!!');
    }
