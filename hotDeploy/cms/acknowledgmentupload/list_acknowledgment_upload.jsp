...
IFileUploadTrxValue trxValueOut=(IFileUploadTrxValue)session.getAttribute("com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentUploadAction.trxValueOut");
	...
	Set errDetLog=null;
	List totalUploadedList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentUploadAction.totalUploadedList");
	...
	if(errorList!=null)
		...
		List<AcknowledgmentError> finalList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentUploadAction.finalList");
...
<%@page import="com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentError"%><head>
...
function downloadImage(){ //v3.0
		var aLocation = "Acknowledgmentupload.do?event=downloadReport&system="+'<%=system%>';
...
function goBack(){
	document.forms[0].action = "Acknowledgmentupload.do?event=excelupload";		
	document.forms[0].submit(); 
}
...
<thead>
   <%if(finalList.size()>=1){ %>
<h3>CERSAI Acknowledgment Upload Error</h3>
<%}else{ %>
<h3>CERSAI Acknowledgment Upload</h3>
<%} %>
<br></br>
</thead>
<tbody>
...
<table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                     ...
                     <logic:iterate id="ob"  offset="<%=String.valueOf(startIndex)%>"  indexId="counter"  name="finalList" type="com.integrosys.cms.ui.acknowledgmentupload.AcknowledgmentError" scope="page" length="100">
                     <%
					String rowClass = "";
					int i =0;
					if (rowCount % 2 != 0) {
