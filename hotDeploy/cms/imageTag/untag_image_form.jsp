...
//session.setAttribute("com.integrosys.cms.ui.imageTag.ImageTagAction.custLimitProfileID",custLimitProfileID);
request.setAttribute("custLimitProfileID",custLimitProfileID);
...
function submitPage() {
	...
	var bankName=document.forms[0].bank.value;
	...
	alert("Please select Bank");
		...
		alert("Please select document for untagging.");
		}else{
		document.forms[0].action="ImageTagMap.do?event=prepare_untag_image_list&customerName=<%=customerName%>";
...
function refreshDropwdowns(dropdown)
{
	...
	if(docTypeCode=='FACILITY_DOC'){
		...
		document.getElementById('exchangeOfInfoFields').style.display = 'none';
		...
		dep = 'docDesc';
		url ='/cms/ImageTag.do?event=refresh_cheklist&custLimitProfileID=<%=custLimitProfileID%>&docTypeCode='+docTypeCode; 
	}else if(docTypeCode == 'EXCHANGE_OF_INFO'	){
			document.getElementById('collateralFields').style.display = 'none';
			document.getElementById('facilityFields').style.display = 'none';
			document.getElementById('docDescFields').style.display = '';
			document.getElementById('exchangeOfInfoFields').style.display = '';
			dep = 'docDesc';
			url ='/cms/ImageTag.do?event=refresh_cheklist&custLimitProfileID=<%=custLimitProfileID%>&docTypeCode='+docTypeCode; 
		}else{
			document.getElementById('collateralFields').style.display = 'none';
...
<tr>
    ...
    <td colspan="2">
        <table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
			...
			<tr class="odd">
			<td colspan="4">
			<table width="100%" id="exchangeOfInfoFields" border="0" cellspacing="0" cellpadding="0" class="tblInfo"
			<%if(!IImageTagConstants.EXCHANGE_OF_INFO.equals(docTye)){%>
				style="display: none;" 
			<%} %>
			>
			<tr>
			...
			<tr class="odd">
			    <td width="30%" class="fieldname">Bank Name<font color="red">*</font></td>
				<td colspan="3" width="70%">
							<html:select name="ImageTagMapForm" property="bank">
			...
			<tr class="odd">
				<td colspan="4">
						...
						<% if(!(IImageTagConstants.CAM_DOC.equals(docTye)||IImageTagConstants.FACILITY_DOC.equals(docTye)||IImageTagConstants.SECURITY_DOC.equals(docTye) || IImageTagConstants.EXCHANGE_OF_INFO.equals(docTye))){%>
								style="display: none;" 
						<%} %>
							>
							<tr>
...
if (custLimitProfileID != null&& !"0".equals(custLimitProfileID)) {
%>
<a href="javascript:submitPage();" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image3311','','img/next2.gif',1)" >
 	<img src="img/next1.gif" name="Image3311" border="0" id="Image3311" />
</a>
<%}%>
