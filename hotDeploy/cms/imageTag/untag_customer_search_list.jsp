...
<%               
 			 ...
 			 %>
                  ...
                  <a href="ImageTagMap.do?event=create_prepare_untag&customerName=<%=OB.getCustomerName()%>&legalName=<%=OB.getLegalReference()%>&custId=<%=OB.getLegalReference()%>&customerID=<%=subprofileID%>&custLimitProfileID=<%=instrID%>&startIndex=<%=aForm.getStartIndex()%>">Untag </a>           
                    </td>
                 </tr>
	            </logic:iterate>
               </logic:present>
...
<td height="25">
			<table  id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td valign="middle" >
                         <integro:pageindex pageIndex='<%=new PageIndex(aForm.getStartIndex(),records_per_page,aForm.getNumItems())%>' url='<%="ImageTag.do?event=untag_customer_search_list&startIndex="%>' submitFlag='<%=new Boolean(false)%>'/>
                        </td>
                      </tr>
               </table>
</td>
