...
||ICMSConstant.TEAM_TYPE_CPU_MAKER_I == Long.parseLong(teamTypeMemID)||ICMSConstant.TEAM_TYPE_CPU_MAKER_I_WFH == Long.parseLong(teamTypeMemID)
||ICMSConstant.TEAM_TYPE_CPU_MAKER_II == Long.parseLong(teamTypeMemID)||ICMSConstant.TEAM_TYPE_CPU_MAKER_II_WFH == Long.parseLong(teamTypeMemID);
...
HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.caseCreationUpdate.MaintainCaseCreationAction.selectedArrayMap");
...
System.out.println("caseCreationUpdate/viewImage.jsp =>Event=>"+event);
String id=request.getParameter("idType");
String limitProfileID=(String)request.getAttribute("limitProfileID1");
...
if(imageTagMapForm!=null){
	...
	int rowIdx = 0;
	int sno = 0;
	 //System.out.println("value of limitProfileID"+limitProfileID);
...
insertMenu('examples');
function cancelPage() {
	document.forms[0].action="caseCreationUpdate.do?event=maker_view_caseCreationUpdate&id=<%=caseId%>&limitProfileID=<%=limitProfileID%>";
    document.forms[0].submit();
}
function closePage() {
	document.forms[0].action="caseCreationUpdate.do?event=maker_view_caseCreationUpdate_branch&id=<%=caseId%>&limitProfileID=<%=limitProfileID%>";
    document.forms[0].submit();
}
function returnPage() {
	document.forms[0].action="caseCreationUpdate.do?event=maker_view_caseCreationUpdate&id=<%=caseId%>&limitProfileID=<%=limitProfileID%>";
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	var events = '<%=event%>';
	if(events == 'view_image_page_menu'){
		document.forms[0].action="caseCreationUpdate.do?event=retrieveImageGallaryCaseCreationMenu&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	}
	else{
		document.forms[0].action="caseCreationUpdate.do?event=retrieveImageGallaryCaseCreation&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	}
...
function getImageForDisplaySecurityDoc(imagePath,imageName,status) {
	   /*var dep = 'imagePath';
	   var url = '/cms/caseCreationUpdate.do?event=retrieveImage&pID='+imagePath+'&imageName='+imageName+'&status='+status;
	   sendLoadImagePathReq(dep, url);*/
	document.getElementById("imageFrame").src= '/cms/caseCreationUpdate.do?event=retrieveImage&pID='+imagePath+'&imageName='+imageName+'&status='+status;
	}
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "caseCreationUpdate.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
...
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("caseCreationUpdate.do?event=print_image&custId=<%=custId%>&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
...
<p align="center">
 <%   if (!"view_image_page_menu".equalsIgnoreCase(event)) { %>
