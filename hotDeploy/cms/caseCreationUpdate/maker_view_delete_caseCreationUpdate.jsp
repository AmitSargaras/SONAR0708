...
if(user.getTeamTypeMembership().getMembershipID()==ICMSConstant.CPU_MAKER||user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_CPU_MAKER_I ||user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_SSC_MAKER||user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_CPU_MAKER_I){
	isCPUT=true;
}
if(user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_BRANCH_CHECKER || user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_BRANCH_VIEW||user.getTeamTypeMembership().getMembershipID()==ICMSConstant.TEAM_TYPE_SSC_CHECKER){
...
if(user.getTeamTypeMembership().getMembershipID()==ICMSConstant.CPU_MAKER_CHECKER){
	System.out.println("teamTypemembershipId  :"+teamTypemembershipId);
	if(teamTypemembershipId==ICMSConstant.CPU_MAKER||teamTypemembershipId==ICMSConstant.TEAM_TYPE_CPU_MAKER_I){
...
System.out.println("caseCreationUpdateObj.getId():"+caseCreationUpdateTrx.getCaseCreation().getId());
...
function cancelPage(){
	/* var docbaecodearr = [];
	docbaecodearr = document.getElementsByName('updatedDocBarcode');
	var x = docbaecodearr[1];
	var y = document.getElementById('documentBarCodes2').value;
	alert(y);
	alert(x); */
    document.forms[0].action="caseCreationUpdate.do?event=maker_list_caseCreationUpdate&id=<%=caseCreationUpdateObj.getId()%>&startIndex=<%=startIndex2%>";
    document.forms[0].submit();
    newShowSilkScreen();
}
function goView(type, checkListItemID,caseId,limitProfileId){
    ...
    document.forms[0].action="caseCreationUpdate.do?event=view_image_page&idType="+checkListItemID+"&type="+type+"&caseId="+caseId+"&limitPtofileId="+limitProfileId;
    document.forms[0].submit();
}
...
<logic:present name="globalDocChkList">
   <logic:iterate id="OB" name="globalDocChkList"  type="com.integrosys.cms.app.caseCreation.bus.ICaseCreation" scope="page" length="<%=String.valueOf(resultList.size())%>" offset="<%=String.valueOf(startIndex2)%>" >
   <%
...
<td class="index"><%=counter1+startIndex2%></td>
		...
		%>
		<%-- <td id="51<%=cnt%>"><input type="text"  value=""  id="documentDescription" readOnly/></td> --%>
 ...
 <%-- <td><integro:empty-if-null value=""/></td> --%>
 ...
 <% }%>
      ...
      <%if((isCPUT||isCPUTMakerchecker) && isActive){
    	   if(isDocMaintained){
				...
				<table width="100%" border="0" align="center" cellpadding="0"
					...
					<tbody>
						<tr>
							...
							<td class="odd"><textarea name="currRemarks" id="123" rows="4" 
...
<% if((isBRANCH||isCPUT||isCPUTMakerchecker) && isActive){%>
