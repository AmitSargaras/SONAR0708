...
<%
	int rowCount=0;
IFileUploadTrxValue trxValueOut=(IFileUploadTrxValue)session.getAttribute("com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsUploadAction.trxValueOut");
	...
	Set errDetLog=null;
	List totalUploadedList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsUploadAction.totalUploadedList");
	if(totalUploadedList!=null)
		...
		List errorList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsUploadAction.errorList");
	if(errorList!=null)
		pageContext.setAttribute("errorList",errorList);
	else
		pageContext.setAttribute("errorList",new ArrayList());
		
		List<ReleaselinedetailsError> finalList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsUploadAction.finalList");
...
<%@page import="com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsError"%><head>
...
function downloadImage(){ //v3.0
		var aLocation = "Releaselinedetailsupload.do?event=downloadReport&system="+'<%=system%>';
...
function goBack(){
	document.forms[0].action = "Releaselinedetailsupload.do?event=excelupload";		
	document.forms[0].submit(); 
}
...
<html:form action="Releaselinedetailsupload.do" >
...
<thead>
   <%if(finalList.size()>=1){ %>
<h3>Release Line Details Upload Error</h3>
<%}else{ %>
<h3>Release Line Details Upload</h3>
<%} %>
<br></br>
</thead>
<tbody>
...
<table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                     ...
                     <logic:iterate id="ob"  offset="<%=String.valueOf(startIndex)%>"  indexId="counter"  name="finalList" type="com.integrosys.cms.ui.releaselinedetailsupload.ReleaselinedetailsError" scope="page" length="100">
                     <%
					String rowClass = "";
					int i =0;
					if (rowCount % 2 != 0) {
