...
<%@ page
	...
	com.integrosys.cms.app.feed.trx.bond.OBBondFeedGroupTrxValue,
	com.integrosys.cms.app.feed.bus.bond.OBBondFeedEntry,
	com.integrosys.cms.app.feed.bus.bond.IBondFeedEntry"%>
<%@ page import="java.text.DateFormat,
                 java.text.SimpleDateFormat" %>
<%
DateFormat df = new SimpleDateFormat("dd-MMM-yyyy");
	BondItemForm bondItemForm = (BondItemForm) request.getAttribute("bondItemForm");

	IBondFeedGroupTrxValue bondFeedGroupTrxValue = (IBondFeedGroupTrxValue) session.getAttribute("com.integrosys.cms.ui.feed.bond.BondAction.bondFeedGroupTrxValue");
    System.out.println("bondFeedGroupTrxValue----"+session.getAttribute("bondFeedGroupTrxValue"));

	SearchResult sr = (SearchResult) request
			.getAttribute("bondFeedEntryList");

	if (sr != null) {
		List bondFeedEntryList = new ArrayList(sr.getResultList());
		pageContext.setAttribute("bondFeedEntryList",
				bondFeedEntryList);
	}
...
function cancelPage(){
    document.forms[0].action="feed_bond_item.do?event=maker_list_bondRateItem";
    
    document.forms[0].submit();
}
function approvePage() {
	
    document.forms[0].action="feed_bond_item.do?event=<%=submitEvent%>";
    document.forms[0].submit();
}
function rejectPage() {
    document.forms[0].action="feed_bond_item.do?event=<%=rejectEvent%>";
    document.forms[0].submit();
}
function cancelPage(){
								...
								<tr>
									<td colspan="4">
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tbody>
												<logic:present name="bondFeedEntryList">
													<logic:iterate id="ob" name="bondFeedEntryList"
													offset="<%=String.valueOf(startIndex)%>"
                               						length="<%=String.valueOf(10)%>"
														type="com.integrosys.cms.app.feed.bus.bond.OBBondFeedEntry"
														scope="page">
														<%
															String rowClass = "";
																			sno++;
														...
														<tr class="<%=rowClass%>">
															...
															<td><integro:empty-if-null value="<%=ob.getBondCode()%>"/>&nbsp;
															</td>
															<td><integro:empty-if-null value="<%=ob.getName()%>"/>&nbsp;
															</td>
															<td><%=ob.getIsinCode() %>&nbsp;
															</td>
															<td><%=ob.getUnitPrice() %>&nbsp;
															</td>
															<td style="text-align:center">&nbsp;<%=ob.getIssueDate()==null?"-":df.format(ob.getIssueDate()) %></td>
															</td>
															<td style="text-align:center">&nbsp;<%=ob.getMaturityDate()==null?"-":df.format(ob.getMaturityDate()) %></td>
															</td>
															<td><integro:empty-if-null value="<%=ob.getRating()%>"/>&nbsp;
															</td>
															<td><%=ob.getCouponRate()%>&nbsp;
															</td>
												...
												<logic:notPresent name="bondFeedEntryList">
													<tr class="odd">
														<td colspan="10">
															<bean:message key="label.global.not.found" />
														</td>
														...
														<table width="100%" border="0" align="center"
															...
															<tbody>
																...
																<tr>
																	...
																	<td class="even"><%=bondFeedGroupTrxValue.getUserInfo()%>&nbsp;
																	</td>
																</tr>
																<tr class="odd">
																	<td class="fieldname">
																	...
																	<td><%=bondFeedGroupTrxValue.getRemarks() != null ? bondFeedGroupTrxValue.getRemarks() : ""%>&nbsp;
																	</td>
																</tr>
															</tbody>
														</table>
															...
															<tr>
																...
																<%
																	String pageIndexUrl = null;
																			...
																			pageIndexUrl = "feed_bond_item.do?event=maker_rejected_delete_read&TrxId="+transId+"&startIndex=";
																%>
