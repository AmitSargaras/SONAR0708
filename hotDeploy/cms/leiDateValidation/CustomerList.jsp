...
<%
    ...
    LeiDateValidationForm aForm;
    
    aForm = (LeiDateValidationForm) request.getAttribute("LeiDateValidationForm");
    
    if (aForm == null) {
    	aForm = (LeiDateValidationForm) request.getAttribute("CustodianSearchForm");
    }
    ...
    SearchResult sresult=(SearchResult)session.getAttribute("com.integrosys.cms.ui.leiDateValidation.LeiDateValidationAction.customerList");
...
<%@page import="com.integrosys.cms.ui.leiDateValidation.LeiDateValidationForm"%><html>
<head>
...
<!--
    ...
    function goPageIndex(ind) {
        document.forms["LeiDateValidationForm"].startIndex.value=ind;
        document.forms["LeiDateValidationForm"].submit();
    }

    function goAlphaIndex(ind) {
        document.forms["LeiDateValidationForm"].customerName.value=ind;
		if (ind == '')
		{
			document.forms["LeiDateValidationForm"].all.value = 'Y';
		}        
        document.forms[0].indicator.value='*';
        document.forms["LeiDateValidationForm"].startIndex.value=0;
        document.forms["LeiDateValidationForm"].submit();
    }
    ...
    function submitForm(){
		...
		if(isadded){
			formObj = document.forms["LeiDateValidationForm"];
...
<!--                  <input type="checkbox" name="itemID" value="<%=sno%>" />-->
 ...
 <!-- End Content -->
   ...
   <!-- General Purpose Bar -->
        <% String alphaURL="/leiDateValidation.do?event=list&startIndex=";%>
<tr>
<td height="25">
			<table  id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td valign="middle" align="right"  >
                         ...
                         <integro:pageindex pageIndex='<%=new PageIndex(aForm.getStartIndex(),records_per_page,aForm.getNumItems())%>' url='<%="leiDateValidation.do?event=list&fromPage=pagination&startIndex="%>' submitFlag='<%=new Boolean(false)%>'/>
                        </td>
                      </tr>
               </table>
</td>
