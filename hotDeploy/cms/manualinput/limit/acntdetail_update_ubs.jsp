...
function udfTab1(name){
	
	 document.forms[0].editTab.value =name;
	showDetail('edit_udf')
}
...
function showDetail(name){
	...
	if(event.equals("prepare_create_ubs") || fromPage.equals("prepare_create_ubs") 
			...
			||event.equals("create_udf") || fromPage.equals("create_udf") || fromPage.equals("add_coBorrower_create") || event.equals("add_coBorrower_create")
		...
		if(name=="edit_released_line_details")
...
<html:form action='MIAcntDetail.do?' >
  <html:hidden name="LmtDetailForm" property="facCoBorrowerLiabIds" styleId="facCoBorrowerIds" value="<%=facCoBorrowerLiabIds%>"/>
	...
	<input type="hidden" name="editTab"  id="editTab" value=""/>
		...
		<tr>
			...
			<table width="95%" border="0" align="center" cellpadding="0"
				...
				<thead>
					...
					<tr align="left">
						<td width="25%" colspan="1"><input onclick="javascript : showDetail('edit_released_line_details')"
							name="AddNew12" type="button" id="AddNew12" value="Released Line Details"
							class="btnNormal" />&nbsp;&nbsp;  
						<input onclick="javascript :  udfTab1('edit_udf')"
							name="editudf1" type="button" id="editudf1" value="USER DEFINED FIELDS(UDF)"
							class="btnNormal" />&nbsp;&nbsp; 
							<input onclick="javascript : udfTab1('edit_udf_2')"
				    ...
				    <tr>
     				<td>
						...
						if(event.equals("edit_udf") || fromPage.equals("edit_udf") || event.equals("create_udf") || fromPage.equals("create_udf")){ %>		
						    <jsp:include page="edit_udf.jsp"></jsp:include>
						<%} else { 
							System.out.println("??????????????????????"+event);
						%>			
					    <jsp:include page="edit_released_line_details.jsp"></jsp:include>
			...
			<%if(event.equals("create_udf") || fromPage.equals("create_udf")) {
				String editTab = request.getParameter("editTab"); 	%>	

					<%if("edit_udf_2".equals(editTab)){%>
					 <img src="img/prev1.gif" style="margin-left: 50%;"  name="Image4411" border="0" id="Image4411" 
									   onclick="javascript : udfTab1('edit_udf')"
									   onmouseover="MM_swapImage('Image4411','','img/prev2.gif',1)"
									    onmouseout="MM_swapImgRestore()"/>&nbsp; 
					<%}else {%>
                       <img src="img/prev1.gif" style="margin-left: 50%;" name="Image4411" border="0" id="Image4411" 
									 onclick="javascript : showDetail('edit_released_line_details')"
									 onmouseover="MM_swapImage('Image4411','','img/prev2.gif',1)"
									  onmouseout="MM_swapImgRestore()"/>&nbsp; 
						<%} %>				             
			<%}else if(event.equals("edit_udf") || fromPage.equals("edit_udf")){
				String editTab = request.getParameter("editTab"); 	%>	
