	   ...
	   else if(document.getElementById("leadValue"+ind).checked)
		{
		   ...
		   document.getElementById("actionDelete"+ind).value="";
	   		...
	   		if( type == "Delete"){					
	    		...
	    		document.forms[0].event.value = "delete_banking_method_in_resubmit";
    ...
    if(nonFundedSharePercent == "")
	 {
      ...
      var url = '/cms/ManualInputCustomer.do?event=calculate_financial_detail_in_resubmit&totalNonFundedLimit='+totalNonFundedLimit+'&nonFundedSharePercent='+nonFundedSharePercent+'&memoExposure='+memoExposure+'&totalFundedLimit='+totalFundedLimit+'&fundedSharePercent='+fundedSharePercent;
		...
		//if(null !=exceptionalCase && ""!=exceptionalCase){
	    	...
	    	if((null != ifscCode && ""!=ifscCode) ||(null !=bankName && ""!=bankName) || (null !=bankBranchName && ""!=bankBranchName)){
					...
					if ( /[^A-Za-z\s]/.test(bankBranchName)) {
	      			...
	      			var name1="display_bank_list_in_resubmit_ifsc_code";  
...
function getAction(id,amt,ind) {
	...
	var type = document.getElementById("actionType"+ind).value;
	 ...
	 if(type =="Edit"){
    	...
    	document.forms[0].event.value = "edit_subline_party_in_resubmit";
    		
    	document.forms[0].submit();
	}
	else if( type == "Delete"){					
		document.forms[0].action = "ManualInputCustomer.do?partyId=" +id +"&index="+ind+"&amount="+amt;		
    	document.forms[0].event.value = "delete_subline_party_in_resubmit";
    	document.forms[0].submit();
	}	    
}
...
<!--<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Start Banking Method >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>-->							
					...
					<tr class="odd" id="exceptional1">
						<td align="right" width="100%">
							<input type="button"  class="btnNormal" onclick="javascript:getBankList('display_bank_list_in_resubmit')"
								...
								<tbody>
									<logic:present name="branchList">
										...
										<tr class="<%=rowClass%>">
											...
											<td>
											<%if(!(ob.getRevisedEmailIds()==null || ob.getRevisedEmailIds().isEmpty())){ %><center>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<html:errors property="multBankNonFundBasedHdfcBankPerError"/>
								...
								<tr>
									...
									<input type="text" name="multBankNonFundBasedLeadBankPer" id="multBankNonFundBasedLeadBankPer1" property='multBankNonFundBasedLeadBankPer' value="<%=value.getMultBankNonFundBasedLeadBankPer()%>"></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tbody>
								...
								<tr>
									...
									<td style="background-color: #E5E4E2">
									<input type="text" name="consBankNonFundBasedLeadBankPer" id="consBankNonFundBasedLeadBankPer1" property='consBankNonFundBasedLeadBankPer'  value="<%=value.getConsBankNonFundBasedLeadBankPer()%>"></td>
