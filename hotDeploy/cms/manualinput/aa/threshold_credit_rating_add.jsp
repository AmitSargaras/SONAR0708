...
*
* Describe this JSP.
* Purpose: For Maker to add new threshold rating detail
* Description: add new threshold rating detail value by Maker
*
...
<%
    ...
    if(event.equals("prepare_update_threshold_new") || event.equals("refresh_prepare_update_threshold_new")){
      eventReturn = "prepare_update_threshold";
    }else if(event.equals("prepare_add_threshold_new") || event.equals("refresh_prepare_add_threshold_new")){
      eventReturn = "prepare_add_threshold";
    }

    String ratingType = aForm.getCreditRatingType();
    if(event.equals("prepare_update_threshold_new_confirm")){
      event = "prepare_update_threshold_new";
      eventReturn = "prepare_update_threshold";
    ...
    }else if(event.equals("prepare_add_threshold_new_confirm")){
      event = "prepare_add_threshold_new";
      eventReturn = "prepare_add_threshold";
      if(! (ratingType.equals(null) || ratingType.equals("")) ){
        RatingList rating = RatingList.getInstance (ratingType);
        Collection ratingListID = rating.getRatingListID ();
...
function submitPage() {
    if('<%=event.substring(0,7)%>' == 'refresh'){
      document.forms[0].action = "ThresholdRating.do?event=<%=event.substring(8,event.length())%>_confirm&indexChange=<%=ind%>&preEvent=<%=preEvent%>&TrxId=<%=trxId%>&customerID=<%=customerID%>";
    }else if('<%=event%>' == 'prepare_update_threshold_new_confirm' || '<%=event%>' == 'prepare_add_threshold_new_confirm'){
      document.forms[0].action = "ThresholdRating.do?event=<%=event%>&indexChange=<%=ind%>&preEvent=<%=preEvent%>&TrxId=<%=trxId%>&customerID=<%=customerID%>";
    }else{
...
function changeCreditRating(ratingType) {
    if('<%=event.substring(0,7)%>' == 'refresh'){
      ...
      document.forms[0].action = "ThresholdRating.do?event=refresh_<%=event%>&indexChange=<%=ind%>&preEvent=<%=preEvent%>&TrxId=<%=trxId%>&customerID=<%=customerID%>";
    }else{
      document.forms[0].action = "ThresholdRating.do?event=refresh_<%=event%>&indexChange=<%=ind%>&preEvent=<%=preEvent%>&TrxId=<%=trxId%>&customerID=<%=customerID%>";
    }
    document.forms[0].submit();
