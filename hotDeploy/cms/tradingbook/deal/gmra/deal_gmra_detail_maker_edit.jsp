...
*
* Describe this JSP.
* Purpose: For Maker to edit the GMRA Deal detail
* Description: edit the GMRA Deal detail value by Maker
*
...
<%
    ...
    System.out.println("request.getParameter(preEvent) = " + request.getParameter("preEvent"));
    if(request.getParameter("preEvent") != null && (event.equals("refresh_maker_edit_dealdetails_reject") || event.equals("refresh_edit_gmra_dealdetails"))){
      preEvent = request.getParameter("preEvent");
      event = preEvent;
    }
    ...
    if( obj!=null ) {
        ...
        if( event.equalsIgnoreCase("maker_edit_dealdetails_reject") ) {
           parameters = obTrxValue.getStagingGMRADeal();
        } else {
           parameters = obTrxValue.getGMRADeal();
        }

        if( parameters!=null ) {
            DefaultLogger.debug(this, "@@@Debug:::2 FROM JSP parameters = "+parameters);
        }
   }else {
   ...
   HashMap hm = new HashMap();
    ...
    if("maker_edit_reject_confirm".equals(event) || "maker_update_dealdetails_confirm".equals(event)){
...
function submitPage() {
    if( "<%=event%>" == "maker_edit_dealdetails_reject" ){
      document.forms[0].action = "GMRADeal.do?event=maker_edit_reject_confirm";
    } else {
      document.forms[0].action = "GMRADeal.do?event=maker_update_dealdetails_confirm";
    }
      document.forms[0].submit();
}
...
function changeMbbBranch() {
    if( "<%=event%>" == "maker_edit_dealdetails_reject" ){
      document.forms[0].action = "GMRADeal.do?event=refresh_maker_edit_dealdetails_reject&preEvent=<%=event%>&TrxId=<%=trxId%>";
    }else{
      document.forms[0].action = "GMRADeal.do?event=refresh_edit_gmra_dealdetails&preEvent=<%=event%>";
    }
    document.forms[0].submit();
}
...
<table class="tblFormSection" width="98%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:10px">
  <thead>
    <tr>
      <td><h3><bean:message key="label.gmra.deal.edit.title"/></h3></td>
    </tr>
    <tr>
      <td><hr/></td>
    </tr>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblInfo">
          <tbody>
            ...
            <tr class="odd">
              ...
              <td colspan="3"><html:text property="secDesc" maxlength="100" size="80" /> <html:errors property="security" />&nbsp;</td>
            </tr>
