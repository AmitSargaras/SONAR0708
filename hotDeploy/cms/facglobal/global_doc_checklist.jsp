...
String facDocCode=(String)request.getAttribute("facDocCode");
String facDocDesc=(String)request.getAttribute("facDocDesc");
String facDocTenureCount=(String)request.getAttribute("facDocTenureCount");
String facDocTenureType=(String)request.getAttribute("facDocTenureType");
...
if(null==facDocTenureCount||"null".equalsIgnoreCase(facDocTenureCount))
	  facDocTenureCount="";
List tenureTypeList = (List)session.getAttribute("com.integrosys.cms.ui.facglobal.FacilityGlobalAction.tenureTypeList");
pageContext.setAttribute("tenureTypeList",tenureTypeList);

SearchResult searchResult = (SearchResult) session.getAttribute("com.integrosys.cms.ui.facglobal.FacilityGlobalAction.searchResult");
int counter = 0;
int noofItems = 0;
int index = 0;
int sno = 0;
...
// System.out.println("value of records_per_page"+records_per_page);
   ...
   String sview = (String) session.getAttribute("com.integrosys.cms.ui.facglobal.FacilityGlobalAction.sview");
...
function getRecList(){
	...
	document.forms[0].action="FacilityGlobal.do?event=global_list&go=Y";
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10px">
 ...
 <THEAD><TR><TD colSpan=4 width="100%">Please enter search criteria</TD></TR></THEAD>
                        ...
                        <td >
                        <html:text property="facDocCode" value="<%= facDocCode %>" styleId='facDocCode'/>
                        <html:errors property="itemCodeError" /></td>
                       <td class="fieldname" >Document Description</td>
                        <td >
                        <html:text value="<%= facDocDesc %>" property="facDocDesc" styleId='facDocDesc'/>
                        <html:errors property="itemDescError" />
                        </td></tr>
                        <tr><td class="fieldname" >Document Tenure</td><td>
                        <html:text maxlength="3" size="10" value="<%= facDocTenureCount %>" property="facDocTenureCount" styleId='facDocTenureCount'/>
                        <html:errors property="tenureError" />
	 <html:select  styleId="facDocTenureType"  property="facDocTenureType" >
    ...
    <tr>
      <td> <h3>Global Facility Document Template</h3></td>
      <%if(!viewOnly){%>
				...
				<tr >
				    <td height="25" colspan="2">
				        <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
				            <tr>
				            <%
				            String url="FacilityGlobal.do?event=paginate&type=F&go=N&startIndex=";
				            %>
