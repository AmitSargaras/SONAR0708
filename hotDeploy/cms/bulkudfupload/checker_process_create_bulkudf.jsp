...
<%
	...
	List bulkudflist = (List) session.getAttribute("com.integrosys.cms.ui.bulkudfupdateupload.BulkUDFUploadAction.bulkudflist");
	...
	DefaultLogger.debug(this,"bulkudflist size : "+ bulkudflist.size());
	
	int sno = 0;
	String heading = "Bulk UDF Update Upload";
	String approve_event = "checker_approve_create_bulkudf";
...
function downloadImage(){ //v3.0
		var aLocation = "Bulkudfupload.do?event=downloadReport&system=";
...
function approvePage() {
	...
	//	if (x == 'Y') {
		if (x == true) {
		    document.forms[0].action="Bulkudfupload.do?event="+'<%=approve_event%>';
...
function rejectPage() {
    document.forms[0].action="Bulkudfupload.do?event=checker_reject_create_bulkudf";
    document.forms[0].submit();
}
function cancelPage(){
