...
<%
	int rowCount=0;
IFileUploadTrxValue trxValueOut=(IFileUploadTrxValue)session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.trxValueOut");
	...
	Set errDetLog=null;
	List totalUploadedList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.totalUploadedList");
	if(totalUploadedList!=null)
		...
		List errorList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.errorList");
	if(errorList!=null)
		pageContext.setAttribute("errorList",errorList);
	else
		pageContext.setAttribute("errorList",new ArrayList());
		
		List<PartyCamError> finalList=(ArrayList)session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.finalList");
	
	/*	for(int i=0; i <finalList.size(); i++){
			PartyCamError partyCamError=finalList.get(i);
			System.out.println("partyCamError:"+partyCamError.getFileRowNo()+" "+partyCamError.getColumnName()+" "+partyCamError.getErrorMessage());
		} */
...
<%@page import="com.integrosys.cms.ui.partycamupload.PartyCamError"%><head>
...
function downloadImage(){ //v3.0
		var aLocation = "Partycamupload.do?event=downloadReport&system="+'<%=system%>';
...
function goBack(){
	document.forms[0].action = "Partycamupload.do?event=csvupload";		
	document.forms[0].submit(); 
}
...
<html:form action="Partycamupload.do" >
...
<thead>
   <%if(finalList.size()>=1){ %>
<h3>Party/CAM File Upload Error</h3>
<%}else{ %>
<h3>Party/CAM File Upload</h3>
<%} %>
<br></br>
</thead>
<tbody>
...
<table width="95%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                     ...
                     <logic:iterate id="ob"  offset="<%=String.valueOf(startIndex)%>"  indexId="counter"  name="finalList" type="com.integrosys.cms.ui.partycamupload.PartyCamError" scope="page" length="100">
                     <%
					String rowClass = "";
					int i =0;
					if (rowCount % 2 != 0) {
...
<%}else if ("NOT_CSV".equals(fileType)){ %>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10px">
