...
<%
	...
	IFileUploadTrxValue itrxValue = (IFileUploadTrxValue) session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.IFileUploadTrxValue");
	List partyCamList = (List) session.getAttribute("com.integrosys.cms.ui.partycamupload.PartyCamUploadAction.partyCamList");
	
	if (partyCamList != null) {
		pageContext.setAttribute("partyCamList", partyCamList);
	}
	...
	DefaultLogger.debug(this,"partyCamList size : "+ partyCamList.size());
	
	int sno = 0;
	String heading = "Party/CAM File Upload";
	String approve_event = "checker_approve_create_partycam";
...
function downloadImage(){ //v3.0
		var aLocation = "Partycamupload.do?event=downloadReport&system=";
...
function approvePage() {
	...
	//	if (x == 'Y') {
		if (x == true) {
		    document.forms[0].action="Partycamupload.do?event="+'<%=approve_event%>';
...
function rejectPage() {
    document.forms[0].action="Partycamupload.do?event=checker_reject_create_partycam";
    document.forms[0].submit();
}
function cancelPage(){
