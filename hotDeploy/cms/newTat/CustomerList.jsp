...
<html:form action="NewTat.do?event=list_tag_page">
...
<%               
				 ...
				 IContact[] contact = OB.getCustomer().getCMSLegalEntity().getRegisteredAddress();
		...
		//					System.out.println("address is "+address.getRegion());
 			 ...
 			 %>
                  ...
                  <td>
                  	<integro:common-code-single categoryCode="<%=CategoryCodeConstant.HDFC_SEGMENT%>"  descWithCode="false" display="true" entryCode="<%=OB.getCustomer().getCustomerSegment() %>"  />				
                  </td>
                  <td>
                  <integro:empty-if-null value="<%=regionMap.get(regionId) %>"/>
                  </td>
                  <td>
                  <%=OB.getCustomer().getStatus() %> 
                  </td>
                  <td>
                
                  <a href="NewTat.do?event=prepare_submit_documents&customerName=<%=OB.getCustomerName()%>&legalName=<%=OB.getLegalReference()%>&custId=<%=OB.getLegalReference()%>&customerID=<%=subprofileID%>&limitProfileID=<%=instrID%>&custLimitProfileID=<%=instrID%>&startIndex=<%=aForm.getStartIndex()%>">
                  Submit Documents </a>
