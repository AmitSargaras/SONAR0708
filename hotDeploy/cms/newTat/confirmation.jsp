...
<%@page import="com.integrosys.cms.app.newTat.bus.INewTat"%><html><!-- InstanceBegin template="Templates/ContentWinLayout.dwt" codeOutsideHTMLIsLocked="false" -->
<head>
...
document.forms[0].submit();
 ...
 function myFunction()
 {
 	...
 	document.forms[0].action="NewTat.do?event=list_ok&startIndexInner="+<%=startIndexInner%>;
     document.forms[0].submit();
  ...
  <tbody>
    <tr>
      <td><table width="100%" border="0" cellpadding="10" cellspacing="0" class="tblInfo">
          ...
          <tbody>
          <tr>
                        <%if(event.equals("confirm_submit_documents")){ %>
                          <td  height="111" class="odd" style="text-align:center">Document Submitted  Successfully.<BR>
                          Case ID is <B><%=newTatObj.getCaseId()%></B>
                          </td>
                          <%}else if(event.equals("deferral_approve")){ %>
                           <td  height="111" class="odd" style="text-align:center">Deferral Approved  Successfully.<BR>
                              Case ID is <B><%=newTatObj.getCaseId()%></B>
                           </td>
                          <%}else if(event.equals("deferral_clearance")){ %>
                           <td  height="111" class="odd" style="text-align:center">Documents Re-Submitted  Successfully.<BR>
                              Case ID is <B><%=newTatObj.getCaseId()%></B>
                           </td>
                          <%}else if(newTatObj.getType()!=null && newTatObj.getType().trim().equals("FTNR")&& newTatObj.getStatus().equals("DOCUMENT_RECEIVED") ){
                        	 %>
                           <td  height="111" class="odd" style="text-align:center">Document Re-Received Successfully.<BR>
                              Case ID is <B><%=newTatObj.getCaseId()%></B>
                           </td>
                          <%}else if(newTatObj.getStatus().equals("DOCUMENT_RECEIVED")){ %>
                           <td  height="111" class="odd" style="text-align:center"">Document Received Successfully.<BR>
                              Case ID is <B><%=newTatObj.getCaseId()%></B>
                           </td>
                          <%} else if(newTatObj.getStatus().equals("CLOSED")){ %>
                           <td  height="111" class="odd" style="text-align:center">Document closed for ( <%=newTatObj.getLspShortName() %> ) Successfully.<BR>
                           Case ID is <B><%=newTatObj.getCaseId()%></B>
</td>
                          <%} else if(newTatObj.getType()!=null && newTatObj.getType().trim().equals("FTNR")&& newTatObj.getStatus().equals("DOCUMENT_SCANNED") ){
                         	 %>
                             <td  height="111" class="odd" style="text-align:center">Document Re-Scanned Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                            <%}else if(newTatObj.getStatus().equals("DOCUMENT_SCANNED")){ %>
                             <td  height="111" class="odd" style="text-align:center">Document Scanned Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                          <%}else if(newTatObj.getStatus().equals("CLIMS_UPDATED")){ %>
                             <td  height="111" class="odd" style="text-align:center">Document Updated in CLIMS Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                          <%}else if(newTatObj.getStatus().equals("DEFERRAL_RAISED")){ %>
                             <td  height="111" class="odd" style="text-align:center">Document Raised Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                          <%} else if(newTatObj.getType()!=null && newTatObj.getType().trim().equals("FTNR")&& newTatObj.getStatus().equals("DEFERRAL_RAISED")){ %>
                             <td  height="111" class="odd" style="text-align:center"">Document Re-Raised Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                          <%}  else if(newTatObj.getStatus().equals("LIMIT_RELEASED")){ %>
                             <td  height="111" class="odd" style="text-align:center"">Limits set Successfully.<BR>
                                Case ID is <B><%=newTatObj.getCaseId()%></B>
                             </td>
                          <%} %>
                        </tr>
           <tr>
              <td height="51" class="odd" style="text-align:center"> <a href="#" onmouseout="MM_swapImgRestore()"
				onmouseover="MM_swapImage('Image3311','','img/ok2.gif',1)"
				onClick=" myFunction()"><img src="img/ok1.gif"
