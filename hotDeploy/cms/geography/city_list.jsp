...
<script language="JavaScript" type="text/JavaScript">
    ...
    function addCity(){
		...
		document.forms[0].event.value	=	"prepare_create_city";
    ...
    function removeCity(id,name)
 	{    
     	var ans = window.confirm("Are You Sure to Delete the City " + name);
     	if( ans == true )
        {
     		var ans1 = window.confirm("All the Related STATES AND its CITIES will be deleted");
         	if( ans1 == true )
            {
		 	 	document.forms[0].action      	= "City.do?cityId="+id;
		     	document.forms[0].event.value 	= "prepare_remove_city";
		     	document.forms[0].submit();
            }
     	}
    ...
    function uploadCity() {
		document.forms[0].action="City.do?event=maker_prepare_upload_city";
	    document.forms[0].submit();
	}
</script>
		...
		<table width="96%" border="0" align="center" cellpadding="0"
			...
			<tr>
				...
				<td valign="bottom" align="right">
					...
					onclick="javascript:addCity()" />
					<input type="button" name="fileupload" value="Upload" class="btnNormal" onclick="uploadCity()"/>
				</td>
			</tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tbody>
						<logic:greaterThan name="citySize" value="0">
							...
							<logic:iterate id="OB" name="cityList"
								type="com.integrosys.cms.app.geography.city.bus.OBCity"
								...
								<tr class="<%=rowClass%>">
									...
									<td><center><%=OB.getCityCode()%></center></td>
									<%
										long i = OB.getIdCity();
									...
									<td><select name="chooseAction"
										...
										<option
											value="City.do?event=view_city_by_index&cityId=<%=i%>&startIndex=<%=startIndex%>">View</option>
										<%
											if (OB.getStatus().equals("ACTIVE")) {
										%>
										<option value="City.do?event=prepare_maker_edit_city&cityId=<%=i%>&startIndex=<%=startIndex%>">Edit</option>
										<%
											}
										%>
										<%
											...
											status = "Disable";
										%>
										<option value="City.do?event=maker_delete_city_read&cityId=<%=i%>&startIndex=<%=startIndex%>">

										<%
											} else {
												status = "Enable";
										%>
										<option value="City.do?event=maker_prepare_activate_city&cityId=<%=i%>&startIndex=<%=startIndex%>">
										<%
											}
										%> <%=status%></option>
									</select></td>
				            ...
				            <tr>
				                <td valign="middle">
				                    <integro:pageindex pageIndex="<%=new PageIndex(startIndex, 10, listSize)%>"
				                            url="City.do?event=paginate&startIndex="/>
				                </td>
				            </tr>
				        </table>
				    </td>
