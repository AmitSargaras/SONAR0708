...
function submitPage(){
	var url="collateralNewMaster.do?event=checker_list_collateralNewMaster&startIndex=0&go=Y";
			...
			<tr>
				<td valign="top">
					<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
                      ...
                      <tr class="odd">
                      <td class="fieldname">Collateral Code </td><td ><html:text property="newCollateralCodeSearch" value="<%= newCollateralCodeSearch %>" maxlength="10"/> <html:errors property="newCollateralCodeError"/></td>
                      <td class="fieldname">Collateral Description </td><td ><html:text property="newCollateralDescriptionSearch" value="<%= newCollateralDescriptionSearch %>"/> <html:errors property="newCollateralDescriptionError"/></td>
					  </tr>
					  <tr >
                     <td class="fieldname">
					  ...
					  <td colspan="4"><center><input align="center" class=btnNormal style="margin-left:7px;height:30px;WIDTH: 70px;"  type="button" value="Go" onclick='submitPage()'></center></td>
							...
							<tr>
                          <td width="82%"><h3>CollateralNewMaster Master</h3></td>
								...
								<td colspan="4"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
										...
										<tbody>
											<logic:present name="collateralNewMasterList">
												<logic:iterate id="ob" name="collateralNewMasterList"
													type="com.integrosys.cms.app.collateralNewMaster.bus.OBCollateralNewMaster"
                        ...
                        %>
													<tr class="<%=rowClass%>">
														...
														<td><integro:common-code-single
																entryCode="<%=ob.getNewCollateralMainType()%>"
																display="true"
																categoryCode="<%=CategoryCodeConstant.COMMON_CODE_SECURITY_TYPE%>"
																descWithCode="false" />&nbsp;</td>
														<td><integro:common-code-single
																entryCode="<%=ob.getNewCollateralSubType()%>"
																descWithCode="false" display="true"
																refEntryCode="<%=ob.getNewCollateralMainType()%>"
                                  ...
                                  %><input type="checkbox" value=""
														...
														<td><%=ob.getRevaluationFrequencyCount()%>&nbsp; <integro:common-code-single
																entryCode="<%=ob.getRevaluationFrequencyDays()%>"
																display="true"
																categoryCode="<%=CategoryCodeConstant.TIME_FREQ%>"
																descWithCode="false" />&nbsp;</td>


														<td>
															<%if(ob.getStatus().trim().equalsIgnoreCase("ACTIVE")){ %>Enable
															<%}else{ %>Disable<%} %>&nbsp;
														</td>
														...
														<td><select name="menu1"
															onchange="MM_jumpMenu('self',this,0)" style="width: 110">
																...
																<option
																	value='collateralNewMaster.do?event=checker_view_collateralNewMaster&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
																<%}else{ %>
																<option
																	value='collateralNewMaster.do?event=maker_view_collateralNewMaster&branchCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
																<%} %>
														</select></td>
													</tr>
												</logic:iterate>
									...
									<table id="generalPurposeBar" width="100%" border="0"
										...
										<tr>
											<td valign="middle"><integro:pageindex
													...
													url="collateralNewMaster.do?event=checker_list_pagination&startIndex=" />
											</td>
										</tr>
									</table>
								</td>
