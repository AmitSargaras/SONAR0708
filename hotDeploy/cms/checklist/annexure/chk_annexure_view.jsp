...
<%
    try {
        com.integrosys.cms.app.recurrent.trx.IRecurrentCheckListTrxValue checkListTrxVal = (IRecurrentCheckListTrxValue) session.getAttribute("com.integrosys.cms.ui.checklist.annexure.AnnexureAction.checkListTrxVal");
...
function goViewRecurrent(itemRef, subItemRef) {
    url = "Annexure.do?event=chk_view_annexure&itemRef=" + itemRef + "&subItemRef=" + subItemRef;
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10px">
  <thead>
    <tr>
      <td><h3><bean:message key="title.update.rec.annexure.process"/></h3></td>
    </tr>
    <tr>
      <td><hr /></td>
    </tr>
...
%>
              ...
              <td style="text-align:center"><%=df.format(OB.getInitialDueDate())%>&nbsp;</td>
              <td><%=OB.getRecurrentCheckListSubItemList()[0].getStatus()%>&nbsp;</td>
			  <td style="text-align:center"><%=df.format(OB.getRecurrentCheckListSubItemList()[0].getReceivedDate())%>&nbsp;</td>
			  <td>&nbsp;</td>
            </tr>
<%
						}
