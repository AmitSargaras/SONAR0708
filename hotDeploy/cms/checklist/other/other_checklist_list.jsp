...
<%
  ...
  //Uma:To Give Document's authorization right's to Branch User.
   ...
   // long otherCheckListID = Long.parseLong(totherCheckListID);
    ...
    DefaultLogger.debug(this,"Party Status: isActive:"+isActive+" otherChecklistId:"+otherChecklistId);
...
%>
  ...
  <a href="OtherCheckList.do?event=view&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=otherChecklistId %>&&secType=O&&secSubType=O"  ><bean:message key="label.view"/></a>
        <%}else{ %>
         <a href="OtherCheckList.do?event=maintain&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=otherChecklistId %>&&secType=O&&secSubType=O"  ><bean:message key="label.process"/></a>
        <%} %>
   <%}else{ %>
&nbsp;-     
<%} %>
