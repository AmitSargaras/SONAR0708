...
<%
	...
	session.setAttribute("SecurityReceiptForm",form);
    ...
    String imageId = stageOb.getSecImageTagUntagId();
    String imageName = stageOb.getSecImageTagUntagImgName();
    String imageStaus = stageOb.getSecImageTagUntagStatus();
    
    
    
    String secListImages = stageOb.getSecImageTagUntagImgName();
  ...
  if(",".equals(untagImagesNames.substring(untagImagesNames.length() - 1))){
    ...
    stageOb.setSecImageTagUntagId(tagImagesNames);
    form.setSecImageTagUntagImgName(secListImages);
    stageOb.setSecImageTagUntagStatus(untagImagesNames);
    ...
    System.out.println("stageOb.secImageId == "+imageId);
    System.out.println("stageOb.secImageName == "+imageName);
    System.out.println("stageOb.secImageStatus == "+imageStaus);
   /*  form.setSecImageTagUntagId(imageId);
    form.setSecImageTagUntagImgName(imageName);
    form.setSecImageTagUntagStatus(imageStaus); */
...
function tagUntagImage(){
	document.forms["SecurityReceiptForm"].event.value = "<%=SecurityReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_VIEW%>";
...
<!--        </tr>-->
		...
		<%-- <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"secImageTagUntagId")? "fieldname":"fieldnamediff"%>">Image Id</td>
            <td>&nbsp;<bean:write name="SecurityReceiptForm" property="secImageTagUntagId"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"secImageTagUntagImgName")? "fieldname":"fieldnamediff"%>">Image Name</td>
            <td>&nbsp;<bean:write name="SecurityReceiptForm" property="secImageTagUntagImgName"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"secImageTagUntagStatus")? "fieldname":"fieldnamediff"%>">Image Status</td>
            <td>&nbsp;<bean:write name="SecurityReceiptForm" property="secImageTagUntagStatus"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr> --%>
        
         <%-- <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"secImageTagUntagStatus")? "fieldname":"fieldnamediff"%>">Tagged Image Name</td>
            <td>&nbsp;<bean:write name="SecurityReceiptForm" property="secImageTagUntagStatus"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"secImageTagUntagId")? "fieldname":"fieldnamediff"%>">Untagged Image Name</td>
            <td>&nbsp;<bean:write name="SecurityReceiptForm" property="secImageTagUntagId"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr> --%>
        ...
        <tr class=<%=aa%>>
            ...
            <tr class=<%=aa%>>
              <td class="<%=CompareOBUtil.compOB(stageOb,ob,"rocCharge")? "fieldname":"fieldnamediff"%>"><bean:message key="label.roc.charge"/></td>
              <td>&nbsp;<integro:empty-if-null value="<%=CheckListHelper.getLabelByLabelValue(stageOb.getRocCharge(), rocChargeList)%>"/></td>
            </tr>
...
//      if (request.getParameter("event") != null && "view_receipt".equals(request.getParameter("event"))) {
        ...
        %>
             <td width="75" valign="baseline" align="center">
                <a href="SecurityReceipt.do?event=view&checkListID=<%=checkListID%>&limitBkgLoc=<%=limitBkgLoc%>&secType=<%=secType%>&secSubType=<%=secSubType%>&collateralID=<%=collateralID%>&limitProfileID=<%=limitProfileID%>"
