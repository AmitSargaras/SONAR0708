...
//pageContext.setAttribute("FacilityReceiptForm",form);
session.setAttribute("FacilityReceiptForm",form);
Locale locale = (Locale)session.getAttribute(ICommonEventConstant.GLOBAL_SCOPE+"."+com.integrosys.base.uiinfra.common.Constants.GLOBAL_LOCALE_KEY);
    ...
    // cr 131
    String imageId = stageOb.getFacImageTagUntagId();
    String imageName = stageOb.getFacImageTagUntagImgName();
    String imageStaus = stageOb.getFacImageTagUntagStatus();
    
    String facListImages = (String)stageOb.getFacImageTagUntagImgName();

    String tagImagesNames = "";
    
    String untagImagesNames = "";
    
    System.out.println("Complete image records(facListImages) stageOb.getFacImageTagUntagImgName()=="+stageOb.getFacImageTagUntagImgName());
    if(facListImages != null && !"".equals(facListImages)){
    	String[] facListImages1 = facListImages.split(",");
    	System.out.println("facListImages1 after split==>"+facListImages1[0]);
    ArrayList facImageListAdd = new ArrayList();
    for(int i=0;i<facListImages1.length;i=i+3){
    	System.out.println("for loop i values==>"+i);
    	System.out.println("Tagged or untagged status =="+facListImages1[i+1].trim()+"*");
    	System.out.println("Tagged or untagged image name =="+facListImages1[i].trim()+"*");
    	if("Tagged".equals(facListImages1[i+1].trim())){
    		tagImagesNames = tagImagesNames + facListImages1[i].trim()+",";
    		System.out.println("Tagged image names=="+tagImagesNames);
    	}else if("UnTagged".equals(facListImages1[i+1].trim())){
    		untagImagesNames = untagImagesNames + facListImages1[i].trim()+",";
    		System.out.println("UnTagged image names=="+untagImagesNames);
    	}
    }
    }
    System.out.println("tagImagesNames ==>"+tagImagesNames);
    System.out.println("untagImagesNames ==>"+untagImagesNames);
  ...
  if(",".equals(untagImagesNames.substring(untagImagesNames.length() - 1))){
    ...
    System.out.println("tagImagesNames after if check ==>"+tagImagesNames);
    System.out.println("untagImagesNames after if check ==>"+untagImagesNames);
  
    stageOb.setFacImageTagUntagId(tagImagesNames);
   // form.setFacImageTagUntagImgName(imageName);
    stageOb.setFacImageTagUntagStatus(untagImagesNames);
    
    form.setFacImageTagUntagId(tagImagesNames);
   // form.setFacImageTagUntagImgName(imageName);
    form.setFacImageTagUntagStatus(untagImagesNames);
    
    System.out.println("stageOb.facImageId == "+imageId);
    System.out.println("stageOb.facImageName == "+imageName);
    System.out.println("stageOb.facImageStatus == "+imageStaus);
    
    
    System.out.println("stageOb.getFacImageTagUntagId() == "+stageOb.getFacImageTagUntagId());
    System.out.println("stageOb.getFacImageTagUntagImgName() == "+stageOb.getFacImageTagUntagImgName());
    System.out.println("stageOb.getFacImageTagUntagStatus() == "+stageOb.getFacImageTagUntagStatus());
    
    System.out.println("form.getFacImageTagUntagId() == "+form.getFacImageTagUntagId());
    System.out.println("form.getFacImageTagUntagImgName() == "+form.getFacImageTagUntagImgName());
    System.out.println("form.getFacImageTagUntagStatus() == "+form.getFacImageTagUntagStatus());
    
    
    System.out.println("ob.getFacImageTagUntagId() == "+ob.getFacImageTagUntagId());
    System.out.println("ob.getFacImageTagUntagImgName() == "+ob.getFacImageTagUntagImgName());
    System.out.println("ob.getFacImageTagUntagStatus() == "+ob.getFacImageTagUntagStatus());
    
    
   /*  form.setFacImageTagUntagId(imageId);
    form.setFacImageTagUntagImgName(imageName);
    form.setFacImageTagUntagStatus(imageStaus); */
    ...
    System.out.println("form.getItemStatus():"+form.getItemStatus());
...
function tagUntagImage(){
	document.forms["FacilityReceiptForm"].event.value = "<%=FacilityReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_VIEW%>";
    ...
    <%--    <tr class=<%=aa%> >
        ...
        <tr class=<%=aa%> >
            ...
            <tr class="even">
              ...
              <%
              System.out.println("stageOb=>"+stageOb.getDocAmt());
              System.out.println("form.getDocAmt()=>"+form.getDocAmt());
...
<!--        </tr>-->
		...
		<%-- <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"facImageTagUntagId")? "fieldname":"fieldnamediff"%>">Image Id</td>
            <td>&nbsp;<bean:write name="FacilityReceiptForm" property="facImageTagUntagId"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"facImageTagUntagImgName")? "fieldname":"fieldnamediff"%>">Image Name</td>
            <td>&nbsp;<bean:write name="FacilityReceiptForm" property="facImageTagUntagImgName"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"facImageTagUntagStatus")? "fieldname":"fieldnamediff"%>">Image Status</td>
            <td>&nbsp;<bean:write name="FacilityReceiptForm" property="facImageTagUntagStatus"/></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr> --%>

        <%-- <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"facImageTagUntagStatus")? "fieldname":"fieldnamediff"%>">Tagged Image Name</td>
            <td>&nbsp;<bean:write name="FacilityReceiptForm" property="facImageTagUntagStatus" /></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr>
        
        <tr class=<%=aa%> >
            <td class="<%=CompareOBUtil.compOB(stageOb,ob,"facImageTagUntagId")? "fieldname":"fieldnamediff"%>">Untagged Image Name</td> 
            <td>&nbsp;<bean:write name="FacilityReceiptForm" property="facImageTagUntagId" /></td>
             <%bb++;  if(bb%2!=0){ aa = "even"; }else{ aa = "odd"; } %>
        </tr> --%>
...
//      if (request.getParameter("event") != null && "view_receipt".equals(request.getParameter("event"))) {
        ...
        %>
             <td width="75" valign="baseline" align="center">
                <a href="FacilityReceipt.do?event=view&checkListID=<%=checkListID%>&limitBkgLoc=<%=limitBkgLoc%>&secType=<%=secType%>&secSubType=<%=secSubType%>&collateralID=<%=collateralID%>&limitProfileID=<%=limitProfileID%>"
