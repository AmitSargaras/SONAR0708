...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'chk_view_approve_img')
	{
		 document.forms[0].action="LADReceipt.do?event=view_attach_image_process&category="+catg;
		   document.forms[0].event.value="view_attach_image_process";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'chk_view_approve_img'){
			 document.forms[0].action="LADReceipt.do?event=chk_view_approve_img&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
