...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'rejectImageGallaryLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=view_attach_lad_process_image&category="+catg;
		   document.forms[0].event.value="view_attach_lad_process_image";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'rejectImageGallaryLADDoc'){
			 document.forms[0].action="LADReceipt.do?event=rejectImageGallaryLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
