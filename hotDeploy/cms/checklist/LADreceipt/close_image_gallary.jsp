...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'closeImageGallaryLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=view_attach_image_close&category="+catg;
		   document.forms[0].event.value="view_attach_image_close";
		  
		    document.forms[0].submit();
	}
	if('<%=event%>'== 'closeToTrackImage')
	{
		 document.forms[0].action="LADReceipt.do?event=mkr_view_totrack_img&category="+catg;
		   document.forms[0].event.value="mkr_view_totrack_img";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'closeToTrackImage'){
			 document.forms[0].action="LADReceipt.do?event=closeToTrackImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
			if('<%=event%>' == 'closeImageGallaryLADDoc'){
				 document.forms[0].action="LADReceipt.do?event=closeImageGallaryLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
