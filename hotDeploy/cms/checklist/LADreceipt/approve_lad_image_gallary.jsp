...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'chkretrieveImageLADDoc')
	{
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'chkretrieveImageLADDoc'){
			 document.forms[0].action="LADReceipt.do?event=chkretrieveImageLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
