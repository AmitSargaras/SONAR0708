...
HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction.selectedArrayMap");
...
function closePage() {
	document.forms["OtherReceiptForm"].event.value="view_return";
	document.forms["OtherReceiptForm"].submit();
}
function returnPage() {
    document.forms["OtherReceiptForm"].event.value = "view_return_to_track";
    document.forms["OtherReceiptForm"].submit();
}
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "OtherReceipt.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="OtherReceipt.do?event=retrieveImageGallaryOtherDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
...
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("OtherReceipt.do?event=print_image&custId=<%=custId%>&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
