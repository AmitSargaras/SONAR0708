...
<%
    ...
    ArrayList migCheckList = (ArrayList)session.getAttribute("com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction.migCheckList");
   String otherStatus;
   ...
   if(otherCheckList!=null){
	   ...
	   otherStatus="NEW";
	    ...
	    otherCheckListType="Other";
   }
   // long otherCheckListID = Long.parseLong(totherCheckListID);
  //  System.out.println("other checklist id :::::::::::"+otherCheckList );
...
function maintain(checkListID,secType,secSubType,limitBkgLoc,orgCode, collateralID, custCategory, subprofileID, applicationType){
    formObj = document.forms["OtherCheckListForm"];
...
<body>
<html:form action="OtherCheckList.do" >
...
<!--             <td width="9%"><bean:message key="label.document.security.location"/></td>-->
           ...
           <!-- 
              ...
              <td width="7%">Other ID</td>
              <td width="14%" nowrap>Other Type /<br />SubType</td>
   ...
   %>
    <tr class="<%=rowClass%>">
              	...
              	<%
              	if(otherChecklistId=="-999999999"){
              	%>
              	<td valign="top" >-&nbsp;</td>
              	<%}else{ %>
              	<td valign="top" >&nbsp;<%=otherChecklistId %></td>
              	<%} %>
              		<td valign="top" >&nbsp; <% if(otherCheckListType.equals("O") ) {%>
              	Other <%} %>
...
//if(otherCheckList.getTrxStatus()==null){
//	canView=true;
//}
//if(otherCheckList.getTrxStatus()!=null && !(otherCheckList.getTrxStatus().equals("ACTIVE"))){
	//canView=true;
//}
...
%>
   ...
   <a href="OtherReceipt.do?event=view&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=otherChecklistId %>&&secType=O&&secSubType=O"  ><bean:message key="label.view"/></a>
        <%}else{ %>
          <a href="OtherReceipt.do?event=prepare_update&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=otherChecklistId %>&&secType=O&&secSubType=O"  ><bean:message key="label.process"/></a>
        <%} %>
   <%}else{ %>
&nbsp;-     
<%} %>
...
<%}%>         
  ...
  if(migCheckList!=null){
   ...
   <a href="OtherReceipt.do?event=view_mig_image_page&&documentID=<%=DocumentId %>&&documentType=<%=DocumentType %>&&documentVersion=<%=DocumentVersion%>"  >View Images</a>
