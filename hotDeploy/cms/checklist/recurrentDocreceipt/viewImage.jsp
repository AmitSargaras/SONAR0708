...
if(imageTagMapForm!=null){
	...
	HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.checklist.recurrentDocreceipt.RecurrentDocReceiptAction.selectedArrayMap");
...
function downloadImage(imagePath,imageName,status){ //v3.0
	var aLocation = "RecurrentDocReceipt.do?event=downloadImage&imageName="+imageName+"&pID="+imagePath+"&imageName="+imageName+"&status="+status;
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "RecurrentDocReceipt.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
	window.location.href = aLocation ;
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="RecurrentDocReceipt.do?event=retrieveImageGallaryRecurrentDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("RecurrentDocReceipt.do?event=print_image&custId=<%=custId%>&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
