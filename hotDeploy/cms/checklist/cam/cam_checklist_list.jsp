...
<%
    ...
    CheckListSearchResult camCheckList = (CheckListSearchResult)request.getAttribute("camCheckList");
   String camStatus;
   String camLocation;
...
<!--             <td width="9%"><bean:message key="label.document.security.location"/></td>-->
   ...
   %>
    <tr class="<%=rowClass%>">
              	...
              	<td valign="top" >&nbsp;<% if(camChecklistId.equals("-999999999")){ %>
              	-
              	<%}else{ %>
              	<%= camChecklistId%>
              	<%} %>
...
if(camCheckList!=null){
	...
	if(!(camCheckList.getTrxStatus().equals("ACTIVE"))){
...
%>
  ...
  <a href="CAMCheckList.do?event=view&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=camChecklistId %>&&secType=CAM&&secSubType=CAM"  ><bean:message key="label.view"/></a>
        <%}else{ %>
         <a href="CAMCheckList.do?event=maintain&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=camChecklistId %>&&secType=CAM&&secSubType=CAM"  ><bean:message key="label.process"/></a>
        <%} %>
   <%}else{ %>
&nbsp;-     
<%} %>
