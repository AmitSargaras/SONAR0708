...
<%
    ...
    aForm = (DiaryItemForm) request.getAttribute("DiaryItemForm");
    
    if (aForm == null) {
    	aForm = (DiaryItemForm) request.getAttribute("CustodianSearchForm");
    }
    ...
    SearchResult sresult=(SearchResult)session.getAttribute("com.integrosys.cms.ui.diaryitem.DiaryItemAction.customerList");
...
<%@page import="com.integrosys.cms.ui.diaryitem.DiaryItemForm"%><html>
<head>
...
<!--
    ...
    function goAlphaIndex(ind) {
        document.forms["DiaryItemForm"].customerName.value=ind;
		if (ind == '')
		{
			document.forms["DiaryItemForm"].all.value = 'Y';
		}        
    ...
    function submitForm(){
		...
		if(isadded){
			formObj = document.forms["DiaryItemForm"];
...
<!--                  <input type="checkbox" name="itemID" value="<%=sno%>" />-->
 ...
 <!-- End Content -->
   ...
   <!-- General Purpose Bar -->
        <% String alphaURL="//DiaryItem.do?event=list&startIndex=";%>
<tr>
<td height="25">
			<table  id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td valign="middle" align="right"  >
                         ...
                         <integro:pageindex pageIndex='<%=new PageIndex(aForm.getStartIndex(),records_per_page,aForm.getNumItems())%>' url='<%="DiaryItem.do?event=list&fromPage=pagination&startIndex="%>' submitFlag='<%=new Boolean(false)%>'/>
                        </td>
                      </tr>
               </table>
</td>
