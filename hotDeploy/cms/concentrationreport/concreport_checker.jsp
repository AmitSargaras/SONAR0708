...
<%
    ...
    IBusinessParameterGroupTrxValue businessParameterGroupTrxVal = (IBusinessParameterGroupTrxValue)session.getAttribute("com.integrosys.cms.ui.concentrationreport.MaintainConcReportAction.SystemParametersTrxValue");
...
function approvePage() {
    document.forms[0].action="MaintainConcReport.do?event=<%=approveEvent%>";
    document.forms[0].submit();
}
function rejectPage() {
    document.forms[0].action="MaintainConcReport.do?event=<%=rejectEvent%>";
    document.forms[0].submit();
}
function cancelPage(){
    //document.forms[0].action="MaintainConcReport.do?event=<%=cancelEvent%>";
    //document.forms[0].submit();
}
...
<table width="70%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection">
  <thead>
    <tr>
      <td><h3>Process Concentration Reports Parameters</h3></td>
    </tr>
    <tr>
      <td><hr /></td>
    </tr>
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInput">
          ...
          <% 
                  ...
                  String parameterCode[] =new String[parametersArray.length];
                        parameterCode= aForm.getParameterCodes();
              int j=0;
              for (int i = 0; i < stageParametersArray.length; i++) {
                  ...
                  IBusinessParameter nextStageParameter=null;
                  IBusinessParameter nextParameter=null;
                  if(i+1<stageParametersArray.length)
                  {
                     nextStageParameter =stageParametersArray[i+1];
                      nextParameter= parametersArray[i+1];
                  }
                   if(!stageParameter.getParameterCode().endsWith("maxrows")){
              j++;
          ...
          <tr class=<%=(j%2==0)?"odd":"even"%>>
			...
			<%if((event!=null) && !(event.equals("to_track"))){%>
             ...
             <%if(nextStageParameter!=null) {%>
               <%if(nextStageParameter.getParameterCode().endsWith("maxrows")){%>
                <td class='<%=((CompareOBUtil.compOB(stageParameter,parameter,"parameterValue"))&&(CompareOBUtil.compOB(nextParameter,nextStageParameter,"parameterValue")))?"index":"indexdiff"%>'><%=(j)%></td>
              <%} else {%>
              <td class='<%=CompareOBUtil.compOB(stageParameter,parameter,"parameterValue")?"index":"indexdiff"%>'><%=(j)%></td>
              <%}%>
              <%} else {%>
              <td class='<%=CompareOBUtil.compOB(stageParameter,parameter,"parameterValue")?"index":"indexdiff"%>'><%=(j)%></td>
              <%}%>
		...
		<%}else{%>	  
              ...
              <td ><%=stageParameter.getParameterValue()%><%--<%=parameterValue[i]%>--%>&nbsp;</td>
               <%if((i+1<parametersArray.length)) {%>
                        <%if(stageParametersArray[i+1].getParameterCode().endsWith("maxrows")){%>
