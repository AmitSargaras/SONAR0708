...
set FILENAME=CLMACTLNK%MMYYYY%.txt
...
db2 export to %OUTPUT_DIR%\%FILENAME%_DTL.tmp of del modified by nochardel coldel0x2C messages %LOG_PATH% "SELECT (DIGITS(CMS_COLLATERAL_ID) || CHAR(COALESCE(LSX_EXT_SYS_ACCT_NUM,''),50) || DIGITS(DECIMAL(CHARGE_AMOUNT,17,2)) || DIGITS(DECIMAL(COALESCE(SECURITY_RANK,0),2,0)) || CHAR(COALESCE(SI_TSFMT(CREATE_DATE,'ddmmyyyy'),''),8) || DIGITS(DECIMAL(COALESCE(CMS_ACT_SEC_COVERAGE,0),5,2))) AS COL FROM VIEW_SI_DW005_CLMACTLNK"

db2 export to %OUTPUT_DIR%\%FILENAME%_TLR.tmp of del modified by nochardel coldel0x2C messages %LOG_PATH% "select digits(count(*)) from %DB_SCHEMA%.VIEW_SI_DW005_CLMACTLNK"
