...
echo [info] This routine EJB compile all the modules in %INTEGRO_HOME%\config\ejb\weblogic
...
for /f %%i in ( %INTEGRO_HOME%\config\ejb\ejb_list.txt ) do call %INTEGRO_HOME%\bin\weblogic\wls_ejbc.bat %%i %INTEGRO_HOME%\build\dist -noexit
