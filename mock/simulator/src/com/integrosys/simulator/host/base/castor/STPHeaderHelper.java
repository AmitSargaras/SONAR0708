...
/*	public static final String MESSAGE_CONFIG_PREFIX_1 = "integrosys.stp.stphelper.";
	...
	public static com.integrosys.simulator.host.stp.STPHeader prepareHeader(String messageType, String messageId, Map headerMap)throws Exception {
		if (null == messageId) {
	...
	public static String getHeaderValue(String msg, String tag) {
		String invalid = "-999999999";// Long.toString(ICMSConstant.LONG_INVALID_VALUE);
	 ...
	 * Helper method
	 */
	public static String setHeaderValue(String msg, String tag, String value)throws Exception {
		String invalid = "-999999999";// Long.toString(ICMSConstant.LONG_INVALID_VALUE);
        StringBuffer newMsg = new StringBuffer();
		try {
			// AT: For simplified empty tag .
			if (msg.toUpperCase().indexOf("<" + tag.toUpperCase()) < 0) {
				throw new Exception("No such tag found : " + tag);
			}

            if(msg.toUpperCase().indexOf("<" + tag.toUpperCase() + "/>") > 0){     //the tag have no value
                String newTag = "<" + tag.toUpperCase() + "/>";
                int start = msg.toUpperCase().indexOf(newTag);

                 newMsg.append(msg.substring(0, start))
                         .append("<" + tag + ">")
                         .append(value)
                         .append("</" + tag + ">")
                         .append(msg.substring(start + newTag.length()));
            }else{
                String newTag = "<" + tag.toUpperCase() + ">";
                int start = msg.toUpperCase().indexOf(newTag);
                start = start + newTag.length();
                ...
                newMsg.append(msg.substring(0, start)).append(value).append(msg.substring(start + end));   
            }
		}
		catch (Exception e) {
            throw e;
