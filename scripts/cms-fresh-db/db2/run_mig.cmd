...
set DB_SCHEMA=CMS_MIG


set DB_DRIVE=F:

set DB_DATAPATH=%DB_DRIVE%\DB2\CMS_MIG
...
db2 create tablespace cms_small_data pagesize 4k managed by database using (FILE '%DB_DATAPATH%\CMS_SMALL_DATA' 2530000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_small_buf

db2 create bufferpool cms_buf size 4096 pagesize 8k extended storage

db2 create tablespace cms_data pagesize 8k managed by database using (FILE '%DB_DATAPATH%\CMS_DATA' 165000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_buf

db2 create bufferpool cms_medium_buf size 2048 pagesize 16k extended storage

db2 create tablespace cms_medium_data pagesize 16384 managed by database using (FILE '%DB_DATAPATH%\CMS_MEDIUM_DATA' 25000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_medium_buf
...
db2 create tablespace cms_index pagesize 8192 managed by database using (FILE '%DB_DATAPATH%\CMS_INDEX' 960000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_index_buf


db2 create bufferpool cms_batch_buf size 2048 pagesize 32k extended storage

db2 create tablespace cms_batch pagesize 32768 managed by database using (FILE '%DB_DATAPATH%\CMS_BATCH' 5000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_batch_buf
...
db2 create tablespace cms_stp pagesize 16k managed by database using (FILE '%DB_DATAPATH%\CMS_STP' 46000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_stp_buf

db2 create bufferpool cms_stp_idx_buf size 2048 pagesize 8k extended storage

db2 create tablespace cms_stp_index pagesize 8192 managed by database using (FILE '%DB_DATAPATH%\CMS_STP_INDEX' 73000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_stp_idx_buf


db2 create bufferpool cms_audit_buf size 4096 pagesize 4096 extended storage

db2 create tablespace cms_audit_data pagesize 4096 managed by database using (FILE '%DB_DATAPATH%\CMS_AUDIT_DATA' 1000) AUTORESIZE YES INCREASESIZE 100 M MAXSIZE NONE bufferpool cms_audit_buf
...
rem db2 update dbm cfg using maxagents 50

rem for synonym (alias) purpose
rem db2 update dbm cfg using FEDERATED YES IMMEDIATE

db2 update database configuration using stmtheap 16384 immediate 

db2 update database configuration using STAT_HEAP_SZ 16384 immediate

rem db2 UPDATE DB CFG USING APP_CTL_HEAP_SZ 256 
...
rem db2 UPDATE DATABASE CONFIGURATION USING SHEAPTHRES_SHR 131072 IMMEDIATE  

rem db2 UPDATE DATABASE CONFIGURATION USING SORTHEAP 65536 IMMEDIATE  
...
rem db2 -tf cms_create_functions2.sql -l cms_create_functions2.log

db2 -tf cms_create_procedures.sql -l cms_create_procedures.log

rem db2 -td! -vf cms_create_procedures2.sql -l cms_create_procedures2.log
...
db2 -td@ -f cms_collateral_allocation_setup.sql -l cms_collateral_allocation_setup.log
