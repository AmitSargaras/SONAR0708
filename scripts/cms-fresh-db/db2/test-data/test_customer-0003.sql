...
values
(20070101000000 + next value for customer_seq, 'CUST-0003', 'Jupiter Hotel', 'Jupiter Hotel', 
'56', 'C', 'MY', 'CUST-0003-BUS', '5', '720', 'ARBS', 'C');
...
cms_le_main_profile_id, 'MY', '002', 'N'
from sci_le_main_profile where lmp_le_id = 'CUST-0003';
...
select 20070101000000 + next value for limit_profile_seq, lsp_le_id, lsp_id, 'AA-0003', 'PL-717621-00002',
cms_le_sub_profile_id, 'ACTIVE', 'MY', '002', 'ARBS', 'N', 'BANKING', 'Y', 'Y',
'5', '720', 'PL', 'CON'
from sci_le_sub_profile where lsp_le_id = 'CUST-0003';
...
select 20070101000000 + next value for limit_seq, cms_lsp_lmt_profile_id, 'FAC-0005', '27', '60',
'25', 'OUTER', 750000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '240'
from sci_lsp_lmt_profile where llp_le_id = 'CUST-0003';
...
-- 'SIBS-ACT-0001', cms_le_sub_profile_id, cms_sub_orig_country, 'N', 'A'
-- from sci_le_sub_profile where lsp_le_id = 'CUST-0003';
...
-- where lsx_ext_sys_acct_num = 'SIBS-ACT-0001';
...
'--', '--', '--', '-999999999', -999999999, -999999999
from sci_lsp_appr_lmts where lmt_id = 'FAC-0005';
...
where cms_lsp_appr_lmts_id in (select cms_lsp_appr_lmts_id
	from sci_lsp_appr_lmts where lmt_id in ('FAC-0005'));
...
select 20070101000000 + next value for collateral_seq, 'AB102', 'AB102-0001', 'MY', subtype_name,
security_type_name, 'AB102', 'AB', 'MYR', 'ACTIVE', 'MYR', '002', 'ARBS', 'Y'
from cms_security_sub_type where security_sub_type_id = 'AB102';

insert into cms_asset (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB102-0001';	
...
type_name, sci_security_subtype_value, sci_security_type_value, sci_security_currency, status,
sci_orig_security_currency, security_organisation
from cms_security where sci_security_dtl_id = 'AB102-0001';
...
select 20070101000000 + next value for col_limit_map_seq, 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB102-0001'), 'AB102-0001',
...
type_name, sci_security_subtype_value, sci_security_type_value, sci_security_currency, status,
sci_orig_security_currency, security_organisation, source_id, is_migrated_ind)
select 20070101000000 + next value for collateral_seq, 'DC302', 'DC302-0001', 'MY', subtype_name,
security_type_name, 'DC302', 'GT', 'MYR', 'ACTIVE', 'MYR', '002', 'ARBS', 'Y'
from cms_security_sub_type where security_sub_type_id = 'DC302';

insert into cms_document (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'DC302-0001';	
...
type_name, sci_security_subtype_value, sci_security_type_value, sci_security_currency, status,
sci_orig_security_currency, security_organisation
from cms_security where sci_security_dtl_id = 'DC302-0001';
...
create_date)
select 20070101000000 + next value for col_limit_map_seq, 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'DC302-0001'), 'DC302-0001',
	cms_lsp_appr_lmts_id, lmt_id, cms_limit_profile_id, 'MB', current timestamp 
from sci_lsp_appr_lmts where lmt_id = 'FAC-0005';	
...
from cms_security
where sci_security_dtl_id in ('AB102-0001', 'DC302-0001');
