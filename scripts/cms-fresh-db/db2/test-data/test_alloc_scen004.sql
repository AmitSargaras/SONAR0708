...
'CUST-ALLOC-0004', 'Mytel Communication Co.', 'Mytel Communication Co.', 
'56', 'C', 'MY', 'CUST-ALLOC-0004-BUS', 'ARBS');
...
from sci_le_main_profile where lmp_le_id = 'CUST-ALLOC-0004';
...
lsp_le_id, lsp_id, '80000004', 'ALLOC-AA-0004', cms_le_sub_profile_id, 'ACTIVE', 
'MY', '2', 'ARBS', 'N', 'BANKING', 'Y', 'Y', 'CO', 'CON'
from sci_le_sub_profile where lsp_le_id = 'CUST-ALLOC-0004';
...
where llp_le_id = 'CUST-ALLOC-0004';
...
cms_lsp_lmt_profile_id, '700000005', '27', 'TC',
'25', 'OUTER', 180000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 180000000, '210', 1, 'FAC-ALLOC-0005', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0004';
...
where los_lmt_id in ('FAC-ALLOC-0005') ;
...
'--', '--', '--', '-999999999', -999999999, -999999999
from sci_lsp_appr_lmts where los_lmt_id = 'FAC-ALLOC-0005';
...
where cms_lsp_appr_lmts_id in (select cms_lsp_appr_lmts_id
	from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0005'));
...
'AB102', 'AB102-ALLOC-0003', 'AB102-ALLOC-0003', 'MY', subtype_name, security_type_name, 
'AB102', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
50000000, 'MYR', 50000000, 'MYR'
from cms_security_sub_type where security_sub_type_id = 'AB102';

insert into cms_asset (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB102-ALLOC-0003';	
...
sci_orig_security_currency, security_organisation,
cmv, cmv_currency, fsv, fsv_currency
from cms_security where sci_security_dtl_id = 'AB102-ALLOC-0003';
...
cmv, cmv_currency, fsv, fsv_currency)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for collateral_seq), 2)) AS bigint), 
'PT701', 'PT701-ALLOC-0001', 'PT701-ALLOC-0001', 'MY', subtype_name, security_type_name, 
'PT701', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
150000000, 'MYR', 150000000, 'MYR'
from cms_security_sub_type where security_sub_type_id = 'PT701';

insert into cms_property (cms_collateral_id)
select cms_collateral_id from cms_security where sci_security_dtl_id = 'PT701-ALLOC-0001';	
...
sci_orig_security_currency, security_organisation,
cmv, cmv_currency, fsv, fsv_currency
from cms_security where sci_security_dtl_id = 'PT701-ALLOC-0001';
...
'--', '--', '--', '-999999999', -999999999, -999999999
from cms_security where sci_security_dtl_id in ('AB102-ALLOC-0003', 'PT701-ALLOC-0001');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	...
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'AB102-ALLOC-0003';
...
and s.sci_security_dtl_id in ('AB102-ALLOC-0003');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	...
	CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint),
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'PT701-ALLOC-0001';
...
from cms_charge_detail d, cms_security s
where d.cms_collateral_id = s.cms_collateral_id
and s.sci_security_dtl_id in ('PT701-ALLOC-0001');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'AB102-ALLOC-0003'), 'AB102-ALLOC-0003',
...
customer_category,create_date, update_status_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'PT701-ALLOC-0001'), 'PT701-ALLOC-0001',
	cms_lsp_appr_lmts_id, lmt_id, cms_limit_profile_id, 'MB', current timestamp , 'I'
from sci_lsp_appr_lmts where los_lmt_id = 'FAC-ALLOC-0005';	
...
and s.sci_security_dtl_id in ('AB102-ALLOC-0003', 'PT701-ALLOC-0001');
...
'MYR', (current timestamp - 5 days), 'V9999', 50000000, 50000000, s.cms_collateral_id, 'S', 'ARBS'
from cms_security s
...
(valuation_id, valuation_currency, valuation_date, valuer, cmv, fsv, cms_collateral_id, source_type, source_id)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for VALUATION_SEQ), 2)) AS bigint), 
'MYR', (current timestamp - 5 days), 'V9999', 150000000, 150000000, s.cms_collateral_id, 'S', 'ARBS'
from cms_security s
where s.sci_security_dtl_id in ('PT701-ALLOC-0001');
...
from cms_security
where sci_security_dtl_id in ('AB102-ALLOC-0003', 'PT701-ALLOC-0001');
...
'37', 'ARBS', 'SIBS-ACT-0009', 'MY', 'N', 'A', 'L', 1, 180000000, 'I');
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	...
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0009'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0005';
