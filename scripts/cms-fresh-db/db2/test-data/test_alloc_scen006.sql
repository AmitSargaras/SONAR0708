...
'CUST-ALLOC-0006', 'Hello Killy Pte. Ltd.', 'Hello Killy Pte. Ltd.', 
'56', 'C', 'MY', 'CUST-ALLOC-0006-BUS', 'ARBS');
...
from sci_le_main_profile where lmp_le_id = 'CUST-ALLOC-0006';
...
from sci_le_sub_profile where lsp_le_id = 'CUST-ALLOC-0006';
...
where llp_le_id = 'CUST-ALLOC-0006';
...
cms_lsp_lmt_profile_id, '700000007', '27', 'HP',
'25', 'OUTER', 50000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 50000000, '210', 1, 'FAC-ALLOC-0007', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0006';
...
account_type, lmt_bca_ref_num, los_bca_ref_num)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for limit_seq), 2)) AS bigint), 
cms_lsp_lmt_profile_id, '700000008', '27', 'HP',
'25', 'OUTER', 20000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 20000000, '210', 1, 'FAC-ALLOC-0008', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0006';
...
account_type, lmt_bca_ref_num, los_bca_ref_num)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for limit_seq), 2)) AS bigint), 
cms_lsp_lmt_profile_id, '700000009', '27', 'HP',
'25', 'OUTER', 30000000, 'MYR', llp_le_id, llp_lsp_id,
'ACTIVE', cms_orig_country, cms_orig_organisation, 'ARBS',
'26', '210', 30000000, '210', 1, 'FAC-ALLOC-0009', 
'L', llp_bca_ref_num, los_bca_ref_num
from sci_lsp_lmt_profile where llp_le_id = 'CUST-ALLOC-0006';
...
where los_lmt_id in ('FAC-ALLOC-0007','FAC-ALLOC-0008', 'FAC-ALLOC-0009') ;
...
'--', '--', '--', '-999999999', -999999999, -999999999
from sci_lsp_appr_lmts where los_lmt_id in ('FAC-ALLOC-0007','FAC-ALLOC-0008', 'FAC-ALLOC-0009');
...
'PT700', 'PT700-ALLOC-0002', 'PT700-ALLOC-0002', 'MY', subtype_name, security_type_name, 
'PT700', 'PT', 'MYR', 'ACTIVE', 'MYR', '12', 'ARBS', 'Y',
...
select cms_collateral_id from cms_security where sci_security_dtl_id = 'PT700-ALLOC-0002';	
...
sci_orig_security_currency, security_organisation,
cmv, cmv_currency, fsv, fsv_currency
from cms_security where sci_security_dtl_id = 'PT700-ALLOC-0002';
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	...
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'PT700-ALLOC-0002';

insert into cms_charge_detail 
	(charge_detail_id, charge_amount, charge_currency_code, security_rank, charge_type, cms_collateral_id, 
	cms_ref_id, status, prior_charge_type, source_id, is_legal_enforce, caveat_waived_ind)
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint), 
	40000000, 'MYR', 2, 'G', s.cms_collateral_id,
	CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for CMS_CHARGE_DETAIL_SEQ), 2)) AS bigint),
	'ACTIVE', 'O', 'ARBS', 'N', 'Y'
from cms_security s where sci_security_dtl_id = 'PT700-ALLOC-0002';
...
and s.sci_security_dtl_id in ('PT700-ALLOC-0002');
...
select CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for col_limit_map_seq), 2)) AS bigint), 
	(select cms_collateral_id from cms_security where sci_security_dtl_id = 'PT700-ALLOC-0002'), 'PT700-ALLOC-0002',
...
and lmts.los_lmt_id in ('FAC-ALLOC-0007', 'FAC-ALLOC-0008');
...
and s.sci_security_dtl_id in ('PT700-ALLOC-0002')
and lmts.cms_lsp_appr_lmts_id = lsm.cms_lsp_appr_lmts_id
and lmts.los_lmt_id in ('FAC-ALLOC-0007', 'FAC-ALLOC-0009');
...
from cms_security s
where s.sci_security_dtl_id in ('PT700-ALLOC-0002');
...
from cms_security
where sci_security_dtl_id in ('PT700-ALLOC-0002');
...
'37', 'ARBS', 'SIBS-ACT-0011', 'MY', 'N', 'A', 'L', 1, 50000000, 'I');
...
values
( CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for customer_x_ref_seq), 2)) AS bigint), 
'37', 'ARBS', 'SIBS-ACT-0012', 'MY', 'N', 'A', 'L', 1, 20000000, 'I');
...
values
( CAST((TS_FMT(CURRENT_TIMESTAMP, 'yyyymmdd') || SUBSTR(DIGITS(next value for customer_x_ref_seq), 2)) AS bigint), 
'37', 'ARBS', 'SIBS-ACT-0013', 'MY', 'N', 'A', 'L', 1, 30000000, 'I');
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	...
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0011'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0007';
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	(select xref.cms_lsp_sys_xref_id
	from sci_lsp_sys_xref xref
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0012'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0008';
...
lmt.lmt_le_id, lmt.lmt_id, lmt.lmt_lsp_id, lmt.cms_lsp_appr_lmts_id, 'ACTIVE', 
	(select xref.cms_lsp_sys_xref_id
	from sci_lsp_sys_xref xref
	where xref.lsx_ext_sys_acct_num = 'SIBS-ACT-0013'), 'I'
from sci_lsp_appr_lmts lmt
where los_lmt_id = 'FAC-ALLOC-0009';
