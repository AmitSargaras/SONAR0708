drop procedure MIG_VALIDATE_AB101@
drop procedure MIG_VALIDATE_CHARGE_AB101@
drop procedure MIG_VALIDATE_PLEDGOR_AB101@
drop procedure MIG_VALIDATE_AB101_SEC@

create procedure mig_validate_ab101_sec
	language sql
begin
	...
	--------------------------------------------
	set v_update_ind = 'update MIG_AB101 '|| 
		'set VALID_IND = ''N'' '||
		'where VALID_IND = ''Y'' '||
		'and (PLANT_EQUIP_TYPE is null '||
	...
	set v_insert_ind = 'insert into mig_error_log '||
		...
		'(select current_timestamp, ''MIG-''||SOURCE_ID, ''MIG_AB101-''||REGION, SOURCE_SECURITY_ID, '||
			'''QC3d'', ''Mandatory Field(s): ''' ||
			'|| SI_BVL(PLANT_EQUIP_TYPE, '' [PLANT_EQUIP_TYPE] '') '||
			...
			'|| SI_BVL(EQUIPMENT_MF, '' [EQUIPMENT_MF] '') '||
			'|| SI_BVL(EQUIPMENT_RISK_GRADING, '' [EQUIPMENT_RISK_GRADING] '') '||
			'|| SI_BVL(EQUIPMENT_CODE, '' [EQUIPMENT_CODE] '') '||
			'|| SI_BVL(OLV, '' [OLV] '') '||
			'|| ''is/are empty.'' '||
			'FROM MIG_AB101 ' ||
	  		' WHERE valid_ind = ''N'' '||
...
CREATE PROCEDURE MIG_VALIDATE_AB101
	LANGUAGE SQL
BEGIN
	----------------------------------
	-- Validate common security fields
	----------------------------------
	call "MIG_VALIDATE_COMMON_SEC_MANDATORY"('MIG_AB101');
	
	--------------------------------------------------------------
	-- Validate the validity of security subtype given
	---------------------------------------------------------------
	call "MIG_VALIDATE_SECURITY_SUBTYPE"('MIG_AB101');
	
	-------------------------------------------------------------------
	-- Validate the validity of the physical inspection date
	--------------------------------------------------------------------
	call "MIG_VALIDATE_PHY_INSP"('MIG_AB101');
	
	-------------------------------------------------------------------------------
	-- Validate the validity of the security confirmed env risky date
	-------------------------------------------------------------------------------	
	call "MIG_VALIDATE_SEC_ENV"('MIG_AB101');	
	
	call "MIG_VALIDATE_AB101_SEC";
END
@

create procedure MIG_VALIDATE_CHARGE_AB101
	LANGUAGE SQL
BEGIN
	...
	----------------------------------------------------------------------------------
	call "MIG_VALIDATE_CHARGE_SECURITY"('MIG_CHARGE_AB101', 'MIG_AB101');

	--------------------------------------
	-- Validate for valid limit id
	--------------------------------------
	call "MIG_VALIDATE_CHARGE_LIMIT"('MIG_CHARGE_AB101');
	
	------------------------------------------------------------------------------------------------------------
	-- validate for valid charge 
	-- invalid the records for same charge if one of the record is failed from validation
	------------------------------------------------------------------------------------------------------------
	call "MIG_VALIDATE_RELATED_CHARGE_RANK_LINKAGE"('MIG_CHARGE_AB101');
	
	call "MIG_INVALID_CHARGE_RANK"('MIG_CHARGE_AB101');
	
END 
@

create procedure MIG_VALIDATE_PLEDGOR_AB101
	language sql
begin
	call "MIG_VALIDATE_PLEDGOR"('MIG_PLEDGOR_AB101');
end
@
