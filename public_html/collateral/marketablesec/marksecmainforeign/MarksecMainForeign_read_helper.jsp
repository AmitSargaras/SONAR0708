...
* $Header: /home/cms2/cvsroot/cms2/public_html/collateral/marketablesec/marksecmainforeign/MarksecMainForeign_read_helper.jsp,v 1.44 2006/07/27 02:05:06 jzhan Exp $
*
...
* Date: $Date: 2006/07/27 02:05:06 $
* Tag: $Name:  $
*/
%>

<%
    IMainIndexForeign oldCollateral = (IMainIndexForeign) itrxValue.getCollateral();
    IMainIndexForeign newCollateral = (IMainIndexForeign) itrxValue.getStagingCollateral();
           ...
           %>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=marketEquity.getUnitPrice().getCurrencyCode()%>"/>&nbsp;<%=marketEquity.getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecmainforeign_read&indexID=<%=marketEquity.getRefID()%>&subtype=MarksecMainForeign&from_event=<%=from_event%>">View</a>
                      </td>
                    </tr>
...
%>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=equity[counter].getUnitPriceCcyCode()%>"/>&nbsp;<%=equity[counter].getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecmainforeign_read&indexID=<%=equity[counter].getRefID()%>&subtype=MarksecMainForeign&from_event=<%=from_event%>">View</a>
                      </td>
