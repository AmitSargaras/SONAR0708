...
* $Header: /home/cms2/cvsroot/cms2/public_html/collateral/marketablesec/marksecgovtforeigndiff/MarksecGovtForeignDiff_read_helper.jsp,v 1.43 2006/07/27 02:03:34 jzhan Exp $
*
...
* Date: $Date: 2006/07/27 02:03:34 $
* Tag: $Name:  $
*/
%>

<%
    IGovtForeignDiffCurrency oldCollateral = (IGovtForeignDiffCurrency) itrxValue.getCollateral();
    IGovtForeignDiffCurrency newCollateral = (IGovtForeignDiffCurrency) itrxValue.getStagingCollateral();
           ...
           %>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=marketEquity.getUnitPrice().getCurrencyCode()%>"/>&nbsp;<%=marketEquity.getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecgovtforeigndiff_read&indexID=<%=marketEquity.getRefID()%>&subtype=MarksecGovtForeignDiff&from_event=<%=from_event%>"><bean:message key="label.view"/></a>
                      </td>
                    </tr>
...
%>
                    <tr class="<%=classtype%>">
                      ...
                      <integro:empty-if-null value="<%=equity[counter].getUnitPriceCcyCode()%>"/>&nbsp;<%=equity[counter].getUnitPrice().getAmount()%>
                       <% } %>
                       &nbsp;</td>
                      <td width="11%"><a href="MarketPortItem.do?event=marksecgovtforeigndiff_read&indexID=<%=equity[counter].getRefID()%>&subtype=MarksecGovtForeignDiff&from_event=<%=from_event%>">View</a>
                      </td>
