            ...
            <td colspan="2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInput">
                   ...
                   <logic:present name="lienList">
										...
										<logic:iterate id="ob" name="lienList"
											type="com.integrosys.cms.app.collateral.bus.type.cash.OBLien"
											scope="page">
											<%
												String rowClass = "";
														no++;
											...
											<tr class="<%=rowClass%>">
												...
												<%							                    
							                    String lienNostr ="";
												if(ob.getLienNumber() != null && !(ob.getLienNumber().equals("")))
							                    {
													lienNostr = ob.getLienNumber().toString();
							                    }

												if(rem != null)
												{
													rem = rem.trim();
													rem = rem.replaceAll("\'","&appos;");
													rem = rem.replaceAll("\"","&quot;");
													rem = rem.replaceAll("\\n|\\r"," ");
												}
							                    %> 
												...
												<center><select id="actionType<%=no%>"
													onchange="javascript:submitPage1(9,'<%=lienNostr%>','<%=dft.format(ob.getLienAmount())%>','<%=no%>','<%=srNoStr%>','<%=rem%>','<%= ob.getBaselSerial() %>','<%=ob.getFacilityName() %>','<%=ob.getFacilityId() %>','<%=ob.getLcnNo() %>')">
