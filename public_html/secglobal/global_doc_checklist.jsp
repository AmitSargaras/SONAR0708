...
String secDocCode=(String)request.getAttribute("secDocCode");
String secDocDesc=(String)request.getAttribute("secDocDesc");
String secDocTenureCount=(String)request.getAttribute("secDocTenureCount");
String secDocTenureType=(String)request.getAttribute("secDocTenureType");
if(null==secDocCode||"null".equalsIgnoreCase(secDocCode))
	  secDocCode="";
if(null==secDocDesc||"null".equalsIgnoreCase(secDocDesc))
	  secDocDesc="";
if(null==secDocTenureCount||"null".equalsIgnoreCase(secDocTenureCount))
	  secDocTenureCount="";
List tenureTypeList = (List)session.getAttribute("com.integrosys.cms.ui.secglobal.SecurityGlobalAction.tenureTypeList");
pageContext.setAttribute("tenureTypeList",tenureTypeList);

SearchResult searchResult = (SearchResult) session.getAttribute("com.integrosys.cms.ui.secglobal.SecurityGlobalAction.searchResult");
int counter = 0;
int noofItems = 0;
int index = 0;
int sno = 0;
...
// System.out.println("value of records_per_page"+records_per_page);
   ...
   String sview = (String) session.getAttribute("com.integrosys.cms.ui.secglobal.SecurityGlobalAction.sview");
...
function getRecList(){
	var secDocCode=document.getElementById('secDocCode').value;
	var secDocDesc=document.getElementById('secDocDesc').value;
	var secDocTenureCount=document.getElementById('secDocTenureCount').value;
	var secDocTenureType=document.getElementById('secDocTenureType').value;
	document.forms[0].action="SecurityGlobal.do?event=global_list&go=Y";
...
function MM_jumpMenu(selObj,docTrxID,startIndex){ //v3.0
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblFormSection" style="margin-top:10px">
 ...
 <THEAD><TR><TD colSpan=4 width="100%">Please enter search criteria</TD></TR></THEAD>
                        ...
                        <td >
                        <html:text property="secDocCode" value="<%= secDocCode %>" styleId='secDocCode'/>
                        <html:errors property="itemCodeError" /></td>
                       <td class="fieldname" >Document Description</td>
                        <td >
                        <html:text value="<%= secDocDesc %>" property="secDocDesc" styleId='secDocDesc'/>
                        <html:errors property="itemDescError" />
                        </td></tr>
                        <tr><td class="fieldname" >Document Tenure</td><td>
                        <html:text maxlength="3" size="10" value="<%= secDocTenureCount %>" property="secDocTenureCount" styleId='secDocTenureCount'/>
                        <html:errors property="tenureError" />
	 <html:select  styleId="secDocTenureType"  property="secDocTenureType" >
												<html:option value=""> Please Select </html:option>
                        <%
                         if(null!=secDocTenureType||!"".equalsIgnoreCase(secDocTenureType)){
                         for(int i=0;i<tenureTypeList.size();i++){
                         LabelValueBean labelValueBean = (LabelValueBean)tenureTypeList.get(i);
                         if(labelValueBean.getValue().equals(secDocTenureType)){ %>
    ...
    <tr>
      <td> <h3>Global Collateral Document Template</h3></td>
      <%if(!viewOnly){%>
...
<logic:present name="globalDocChkList">
   ...
   %>
        ...
        <tr class="<%=rowClass%>">
         ...
         if(OB.getStatus().trim().equalsIgnoreCase("ENABLE")){ %>Enable
              <%}else{ %>Disable<%} %>&nbsp;</td>
				...
				<tr >
				    <td height="25" colspan="2">
				        <table id="generalPurposeBar" width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
				            <tr>
				            <%
				            String url="SecurityGlobal.do?event=paginate&type=S&go=N&startIndex=";
				            %>
