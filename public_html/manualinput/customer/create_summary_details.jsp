...
<%@page import="com.integrosys.cms.app.generalparam.bus.IGeneralParamEntry"%><%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean"%>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html"%>
<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic"%>
<%@ taglib uri="/WEB-INF/IntegroTag.tld" prefix="integro"%>
<%@ page
	import="com.integrosys.base.techinfra.logger.DefaultLogger,com.integrosys.base.businfra.search.SearchResult,java.math.BigDecimal,com.integrosys.base.businfra.currency.Amount,
...
String systemDate= df.format(new Date());
    ...
    String customerCifId =(String) request.getAttribute("customerCifId");

   
    if(customerCifId == null || customerCifId == "")
    {
    	 customerCifId =(String) session
    			.getAttribute("com.integrosys.cms.ui.manualinput.customer.ManualInputCustomerAction.cifId");
    }
    pageContext.setAttribute("cifId", customerCifId);
    System.out.println("sumaary cifId-------->"+customerCifId);
	...
	System.out.println("relationshipMgr requestscope " +relationshipMgr);
	System.out.println("relationshipMgr pagecontext " +pageContext.getAttribute("relationshipMgr"));
	System.out.println("relationshipMgr session scope " +session.getAttribute("relationshipMgr"));
...
<%  String party= "";
	...
	//DefaultLogger.debug(this,"*************cif id is*********************** "
	//		+ customerCifId);
...
function getvendorAction(name,id,ind) {
	...
	var type = document.getElementById("action"+ind).value;
	 ...
	 if(type =="Edit"){
		...
		document.forms[0].event.value = "edit_vendor";
    	document.forms[0].submit();
	}
	else if( type == "Delete"){					
		document.forms[0].action = "ManualInputCustomer.do?vendorName=" +name +"&vendorId="+id+"&index="+ind;		
    	document.forms[0].event.value = "delete_vendor";
    	document.forms[0].submit();
	}	    
}
function addDetail(){
...
function refreshGroup(obj) {
	document.getElementById("partyGroupName").value = obj.value;
}
...
function checkboxForForm6061(){	
	var check = document.getElementById("form6061").checked;
	if(check == true)
	{		
		document.getElementById("form6061Checked").value= "Y";
		document.getElementById("form6061").value ="Y";
	}
	else{
		document.getElementById("form6061Checked").value= "N";
		document.getElementById("form6061").value="N";
	}
...
function validateLEICode(){
    ...
    <%if("Y".equalsIgnoreCase(leiParamValue)){%>
		...
		xmlhttp.onreadystatechange=function()
		{
	 	...
	 	xmlhttp.open("GET","/cms/ManualInputCustomer.do?event=validate_lei_code_with_CCIL&leiCode="+lei+"&customerCifId="+'<%=customerCifId%>',true);
		xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		xmlhttp.send();  
...
function validatePANNo(){
	...
	if(pan!=null && pan!=""){
        ...
        <%if("Y".equalsIgnoreCase(paramValue)){%>
			...
			var cif = '<%=customerCifId%>';
			xmlhttp.open("GET","/cms/ManualInputCustomer.do?event=validate_pan_no_with_NSDL&panNo="+pan+"&customerCifId="+cif+"&dateOfIncorporation="+dateOfIncorporation+"&partyNameAsPerPan="+partyNameAsPerPanEncoded,true);
			xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xmlhttp.send();
...
function populateRMData(){
	...
	<%System.out.println(relationshipMgr +" --------------------=================================  "+rmRegion);%>
...
<table width="100%" height="100%" cellspacing="0" cellpadding="0"
				...
				<tr id="customerdetails">
					...
					<table width="100%" height="100%" cellspacing="0" cellpadding="0"
								...
								<tbody>
									<tr class="odd">
										...
										<input type="text" size="30" style="background:#B4CFEC"  disabled="true"/>
										 <%-- <html:hidden property="cifId" value="<%=customerCifId%>" ></html:hidden>&nbsp;  --%>
									<input type="hidden" name="cifId" id="cifId" value="<%=customerCifId%>" /></td> 
										...
										<td width="30%"><html:text property="customerNameShort" styleId="customerNameShort"
											size="40" maxlength="100"  onchange="javascript : addDetail();changeNameAsPerPan()"></html:text>&nbsp;
									...
									<tr class="even">
										...
										<td width="30%">
										<%if( value.getStatus()==null || value.getStatus()==""){
											%>
						                  	<input type="radio" name="status" id="status" checked="checked" value="ACTIVE" >Open</input>
									...
									<tr class="odd">
										...
										<td width="30%"><html:text
											property="relationshipStartDate" size="30" maxlength="20" readonly="true"></html:text><img
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0"
								...
								<tbody>
									<tr class="odd">
										...
										<td width="30%"><textarea id="name"  readonly="readonly" style="width: 300px; height: 30px;"></textarea>&nbsp;
											</td>
									...
									<tr class="even">
										...
										<td width="30%">	
									<input type="text" id="branch" readonly="readonly"></input>&nbsp;
										</td>
										<td class="fieldname" width="20%">Group</td>
										<td width="30%">
										<%BigDecimal str = new BigDecimal("0.0"); %>
										<html:select property="partyGroupName" styleId="partyGroupName"
											size="1" style="width:250px" onchange="refreshGroup(this);" >
											<option value="">Please Select</option>
											<logic:present name="partyGroupList">
												<logic:iterate id="ob" name="partyGroupList"
													...
													%>
													<option 
														value="<%=ob.getId()%>"><%=ob.getPartyName()%></option>
													<%
													//System.out.println("1---"+ob.getGroupExpLimit());
														} else {
													%>
													<%
														if (value.getPartyGroupName().equals(
																				String.valueOf(ob.getId()))) {
													%>
													<option 
														value="<%=ob.getId()%>" selected="selected"><%=ob.getPartyName()%></option>
													<%
													str = ob.getGroupExpLimit().getAmountAsBigDecimal();
													//System.out.println("2---"+ob.getGroupExpLimit());
														} else {
													%>
													<option 
														value="<%=ob.getId()%>"><%=ob.getPartyName()%></option>
													<%
													//System.out.println("3-----"+ob.getGroupExpLimit());
														}
													%>
													<%
														}
									...
									<tr class="odd">
										...
										<td width="30%"><html:text
											name="ManualInputCustomerInfoForm" styleId="relationshipMgr" property="relationshipMgr" style="width:250px" readonly="true"/>
																				</td>
									</tr>
									<tr class="even">
									...
									<!-- code start:Uma Khot 18/08/2015 Phase 3 CR:Rename Region to RM Region-->
										...
										<td width="30%"><html:text
											...
											onchange="javascript:refreshRelationshipMgrId(this)" style="width:250px" readonly="true"/>
										
										<br><html:errors property="rmRegionError"/>
										</td>
										<td class="fieldname">Segment</td>
										<td>									
										<input type="text" id="segment" readonly="readonly" size="40">	
										</input>&nbsp;								
										</td>
									</tr>
									<tr class="odd">
										...
										<td><html:select property="industryName" style="width:250px" onchange="javascript:refreshRbiIndustryCode(this)" 
											style="width:250px"> 
									...
									<tr class="even">
										...
										<td><html:text property="partyNameAsPerPan" styleId="partyNameAsPerPan" onchange="javascript : makeUpperCasePan()" size="40" maxlength="85"></html:text>
									...
									<tr class="even">
										...
										<td>
											<% if(value.getForm6061Checked()==null || value.getForm6061Checked().equals("")){%>
												<input type="checkbox"  name="form6061Checked" id="form6061" value="" onclick="javascript:checkboxForForm6061()"></input>
												<html:hidden property="form6061Checked" styleId="form6061Checked"/>
												<% }else if(value.getForm6061Checked().equals("Y")){%>
												<input type="checkbox" name="form6061Checked" id="form6061" value="Y" checked="checked" onclick="javascript:checkboxForForm6061()"></input>
												<html:hidden property="form6061Checked" styleId="form6061Checked"/>
												<%} else if(value.getForm6061Checked().equals("N")){%>
												<input type="checkbox" name="form6061Checked" id="form6061" value="N" onclick="javascript:checkboxForForm6061()"></input>
												<html:hidden property="form6061Checked" styleId="form6061Checked"/>
											<%} %>
										</td>
									...
									<tr class="even">
										...
										<td><html:text property="leiCode" size="30"
											...
											<%} %>
											<br>
											<% try{ if( 'Y'== value.getIsLeiValidated()){ %>
									...
									<tr class="even">
										...
										<td><html:text property="postcode" size="10"
											...
											<br><html:errors property="postcodeLengthError"/>
									...
									<tr class="odd">
										...
										<td>
										<html:text property="stdCodeTelNo" size="5"
										...
										<td>
										<html:text property="stdCodeTelex" size="5"
											maxlength="5"></html:text>
										<html:text property="telex" size="20" maxlength="15"></html:text>&nbsp;
									...
									<tr class="odd">
										...
										<td><html:text property="scfStatus" size="30"
															maxlength="50" disabled="true" value="NA"></html:text>&nbsp;
										<br></td>
						...
						<tr id="addVendorDetails" style="display: none;">
							...
							<input onclick="javascript : addSystem('add_vendor_name')"
					...
					<tr id="addVendorDetails2" style="display: none;">
						...
						<tr id="addSystemDetails" style="display: none;">
							...
							<input onclick="javascript : addSystem('add_other_system')"
								name="AddNew12" type="button" id="AddNew12"
						...
						<tr id="addSystemDetails2" style="display: none;">
							<td>
								<html:errors property="systemListEmptyError"/>
	...
	<html:hidden name="ManualInputCustomerInfoForm"
		property="customerNameShort" styleId="customerNameShort" />
...
<html:hidden name="ManualInputCustomerInfoForm" property="scfStatus" />	
	...
	<html:hidden name = "ManualInputCustomerInfoForm" property="isLeiValidated" styleId="isLeiValidated" />
		...
		<html:hidden name = "ManualInputCustomerInfoForm" property="panValGenParamFlag" styleId="panValGenParamFlag" value='<%=paramValue%>' />
