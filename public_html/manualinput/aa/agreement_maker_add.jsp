...
*
* Describe this JSP.
* Purpose: For Maker add new trading agreement
* Description: add new trading agreement value by Maker
*
...
<%
    ...
    InterestRateTypeList interestRateList = InterestRateTypeList.getInstance ();
    Collection interestRateTypeListID = interestRateList.getInterestRateTypeListID ();
    Collection interestRateTypeListValue = interestRateList.getInterestRateTypeListValue ();
    ...
    if("maker_add_agreement_confirm".equals(event)){
...
function goToPage() {
    document.forms[0].action = "TradingAgreement.do?event=prepare_add_threshold&preEvent=<%=preEvent%>&indexChange=<%=indexChange%>&customerID=<%=customerID%>";
    document.forms[0].submit();
}

function submitPage() {
    document.forms[0].action = "TradingAgreement.do?event=maker_add_agreement_confirm&preEvent=<%=preEvent%>&indexChange=<%=ind%>&customerID=<%=customerID%>";
    document.forms[0].submit();
}

function returnPage() {
    document.forms[0].action = "TradingAgreement.do?event=cancel&preEvent=<%=preEvent%>&indexChange=<%=ind%>&customerID=<%=customerID%>";
    document.forms[0].submit();
}

function changeCountRating() {
    document.forms[0].action = "TradingAgreement.do?event=refresh_maker_add_agreement&preEvent=<%=preEvent%>&indexChange=<%=indexChange%>&customerID=<%=customerID%>";
    document.forms[0].submit();
}

function changeMaybankRating() {
    document.forms[0].action = "TradingAgreement.do?event=refresh_maker_add_agreement&preEvent=<%=preEvent%>&indexChange=<%=indexChange%>&customerID=<%=customerID%>";
    document.forms[0].submit();
}
    ...
    <tr>
      <td width="100%" valign="bottom">
        <table width="100%"  border="0" align="right" cellpadding="0" cellspacing="0">
          <tr>
            <td width="60%"><h3><bean:message key="label.agreement.title.add"/></h3></td>
            ...
            <tr class="odd">
              ...
              <td>
                <%if(aForm.getCounterPartyThresholdAmt()!= null && !aForm.getCounterPartyThresholdAmt().equals("") ){
                    //Amount countThresholdAmt = new Amount(Double.parseDouble( aForm.getCounterPartyThresholdAmt().substring(4,aForm.getCounterPartyThresholdAmt().length()) ),
                                                /*aForm.getCounterPartyThresholdAmt().substring(0,3) );*/%>
