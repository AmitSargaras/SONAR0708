...
<%
	...
	request.setAttribute("goodsList", goodsList);
	 ...
	 Collection incoTermList = (Collection) session.getAttribute("com.integrosys.cms.ui.manualinput.limit.LmtDetailAction.incoTermList");
		request.setAttribute("incoTermList", incoTermList);
...
<head>
  ...
  function checkboxCountry(){
		if(document.getElementById('countryRestriction').checked==false){
			var listSize='<%=restCountryList.size()%>';
			if(listSize==0){
...
function checkboxDrawer(){
		if(document.getElementById('drawerRestriction').checked==false){
			var listSize='<%=restDrawerList.size()%>';
			if(listSize==0){
...
function checkboxDrawee(){
		if(document.getElementById('draweeRestriction').checked==false){
			var listSize='<%=restDraweeList.size()%>';
			if(listSize==0){
...
function checkboxBene(){
		if(document.getElementById('beneRestriction').checked==false){
			var listSize='<%=restBeneList.size()%>';
			if(listSize==0){
...
function checkboxCurrency(){
		if(document.getElementById('currencyRestriction').checked==false){
			var listSize='<%=restCurrencyList.size()%>';
			if(listSize==0){
				...
				document.forms[0].currencyRestriction.value="";
			...
			document.forms[0].currencyRestriction.value="Y";
...
function checkboxBank(){
	if(document.getElementById('bankRestriction').checked==false){
		var listSize='<%=restBankList.size()%>';
		if(listSize==0){
...
function checkboxGoods(){
	if(document.getElementById('goodsRestriction').checked==false){
		var listSize='<%=covenantGoodsRestrictionList==null? Collections.emptyList():covenantGoodsRestrictionList.size()%>';
		if(listSize==0){
...
function enableCheckBox(){
	...
	//document.getElementById('currencyRestriction').disabled = false;
	//It made disable for some time as per requirement :: 03112022
	document.getElementById('currencyRestriction').disabled = true;
...
function deleteRestCountry(assetId,index) {
	document.forms[0].action = "MILimitCovenant.do?countryName=" +assetId +"&index="+index;		
...
function deleteRestDrawer(Name,Amount,custId,custName,index) {
	document.forms[0].action = "MILimitCovenant.do?drawerName=" +Name +"&drawerAmount="+Amount+"&drawerCustId="+custId+"&drawerCustName="+custName+"&index="+index;		
	document.forms[0].event.value = "remove_drawer_rest";
	document.forms[0].submit();
}

function deleteRestDrawee(Name,Amount,custId,custName,index) {
	document.forms[0].action = "MILimitCovenant.do?draweeName=" +Name +"&draweeAmount="+Amount+"&draweeCustId="+custId+"&draweeCustName="+custName+"&index="+index;		
	document.forms[0].event.value = "remove_drawee_rest";
	document.forms[0].submit();
}

function deleteRestBene(Name,Amount,custId,custName,index) {
	document.forms[0].action = "MILimitCovenant.do?beneName=" +Name +"&beneAmount="+Amount+"&beneCustId="+custId+"&beneCustName="+custName+"&index="+index;		
...
function addRestCurrency(){
	...
	document.forms[0].action="MILimitCovenant.do?currencyRestrictionCode=" +currencyRestrictionCode+"&currencyRestrictionAmount="+currencyRestrictionAmount;
...
function addRestDrawer(){
	...
	var index =  '<%=index%>';
	document.forms[0].action="MILimitCovenant.do?index="+index;
...
function refreshIncoDesc() {
	var incoTerm = document.getElementById('incoTerm').value;
      ...
      var url = '/cms/MILimitCovenant.do?event=refresh_inco_desc&incoTerm='+incoTerm;
...
function refreshChildGoodsMaster(dropdown, name){
	 ...
	 var url = '/cms/MILimitCovenant.do?event=refresh_goods_master&goodsType='+goodsType+'&goodsCode='+curSel;
	 if(curSel!=""){
  ...
  function deleteCovenantGoods(){
	  ...
	  var selectedGoods  = selectedGoodsArr.join(',');
		...
		document.forms[0].action="MILimitCovenant.do?goodsCode="+selectedGoods;
...
function addCovenantGoods(){
	...
	//alert("MILimitCovenant.do?goodsParentCode="+goodsParentCode+"&goodsChildCode="+goodsChildCode+"&goodsSubChildCode="+goodsSubChildCode);
	document.forms[0].action="MILimitCovenant.do?goodsParentCode="+goodsParentCode+"&goodsChildCode="+goodsChildCode+"&goodsSubChildCode="+goodsSubChildCode;
...
<html:form action="MILimitCovenant.do">
	...
	<input type="hidden" name="eventForEdit" value="<%=request.getAttribute("eventForEdit")%>"/>
	...
	%>
	<%
		System.out.println("eventForEdit "+request.getAttribute("eventForEdit"));
	%>
...
<tr class="even" >	  
       <td class="fieldname"></td>	  
	    <td colspan="3"> 
	    	  <html:checkbox  onclick="javascript:checkboxCurrency()" property="currencyRestriction" disabled="disabled" styleId="currencyRestriction" value ="Y"/>
	     	 <html:hidden property="currencyRestriction" styleId="currencyRestrictionHidden"/>
...
<tr class="odd">	  
       ...
       %>
         ...
         <input type="text" id ="drawerName" name="drawerName" value="<%=formObj.getDrawerName()%>" disabled = "disabled"/>
        <%
        ...
        %>
         <input type="text" id ="drawerName" name="drawerName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="drawerName" name="drawerName" value="<%=formObj.getDrawerName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="drawerName" name="drawerName"/>
...
<tr class="even" >	
        ...
        %>
         <input type="text" id ="drawerAmount" name="drawerAmount" onblur="javascript:formatAmountAsCommaSeparated(this);"/>
          <%
       	     ...
       	     %>
       	     <input type="text" id ="drawerAmount" name="drawerAmount" value="<%=UIUtil.formatWithCommaAndDecimal(formObj.getDrawerAmount())%>" onblur="javascript:formatAmountAsCommaSeparated(this);"/>
       	      <%
       	      	}} else{
          	     %>
	   ...
	   <td colspan="5">
	   <input class="btnNormal" type="button" name="btnedit_search_Drawer" id="btnedit_search_Drawer" value="Search" disabled="true">				
	   </td>
 </tr>
<tr class="odd" >	
       <td class="fieldname"></td>
...
<tr class="even" >	
       ...
       %>
         ...
         <input type="text" id ="drawerCustName" name="drawerCustName" value="<%=formObj.getDrawerCustName()%>" disabled = "disabled"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        ...
        %>
         <input type="text" id ="drawerCustName" name="drawerCustName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="drawerCustName" name="drawerCustName" value="<%=formObj.getDrawerCustName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="drawerCustName" name="drawerCustName"/>
									...
									<logic:present name="restDrawerList">
										<logic:iterate id="ob" name="restDrawerList"
											...
											<tr class="<%=rowClass%>">
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getDrawerName()%>"/>&nbsp;</center>
												</td>
												<td>
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getDrawerCustName()%>"/>&nbsp;</center>
												</td>
												 <td>
...
<tr class="odd" >		  
       ...
       %>
         <input type="text" id ="draweeName" name="draweeName" value="<%=formObj.getDraweeName()%>" disabled = "disabled"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        ...
        %>
         <input type="text" id ="draweeName" name="draweeName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="draweeName" name="draweeName" value="<%=formObj.getDraweeName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="draweeName" name="draweeName"/>
...
<tr class="even" >	
       ...
       %>
        ...
        %>
         <input type="text" id ="draweeAmount" name="draweeAmount" onblur="javascript:formatAmountAsCommaSeparated(this);" />
          <%
          	}else{
       	     %>
       	     <input type="text" id ="draweeAmount" name="draweeAmount" onblur="javascript:formatAmountAsCommaSeparated(this);" value="<%=UIUtil.formatWithCommaAndDecimal(formObj.getDraweeAmount())%>"/>
	   ...
	   <td colspan="5">
	   <input class="btnNormal" type="button" name="btnedit_search_Drawee" id="btnedit_search_Drawee" value="Search" disabled = "true">				
	   </td>
 </tr>
<tr class="odd" >	
       <td class="fieldname"></td>
	   ...
	   <td>
	    <%
       	if(formObj.getDraweeRestriction().equalsIgnoreCase("Y")){ 
           	   if(eventForEdit!=null && eventForEdit.equals("prepare_edit_drawee_rest")){
       %>
         <input type="text" id ="draweeCustName" name="draweeCustName" value="<%=formObj.getDraweeCustName()%>" disabled = "disabled"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        ...
        %>
         <input type="text" id ="draweeCustName" name="draweeCustName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="draweeCustName" name="draweeCustName" value="<%=formObj.getDraweeCustName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="draweeCustName" name="draweeCustName"/>
									...
									<logic:present name="restDraweeList">
										<logic:iterate id="ob" name="restDraweeList"
											...
											<tr class="<%=rowClass%>">
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getDraweeName()%>"/>&nbsp;</center>
												</td>
												<td>
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getDraweeCustName()%>"/>&nbsp;</center>
												</td>
												 <td>
...
<tr class="odd" >		  
       ...
       %>
         ...
         <input type="text" id ="beneficiaryName" name="beneficiaryName" value="<%=formObj.getBeneficiaryName()%>" disabled = "disabled"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        ...
        %>
         <input type="text" id ="beneficiaryName" name="beneficiaryName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="beneficiaryName" name="beneficiaryName" value="<%=formObj.getBeneficiaryName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="beneficiaryName" name="beneficiaryName"/>
...
<tr class="even" >	
       ...
       %>
         ...
         <input type="text" id ="beneficiaryAmount" name="beneficiaryAmount" value="<%=UIUtil.formatWithCommaAndDecimal(formObj.getBeneficiaryAmount())%>" onblur="javascript:formatAmountAsCommaSeparated(this);"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        		if(exceptionMap.containsKey("beneficiaryAmount")){
        %>
         <input type="text" id ="beneficiaryAmount" name="beneficiaryAmount" onblur="javascript:formatAmountAsCommaSeparated(this);"/>
          <%
          	}else{
       	     %>
...
<tr class="even" >	
       ...
       %>
         ...
         <input type="text" id ="beneficiaryCustName" name="beneficiaryCustName" value="<%=formObj.getBeneficiaryCustName()%>" disabled = "disabled"/>
        <%
        	}else if (exceptionMap!=null && !exceptionMap.isEmpty()){
        ...
        %>
         <input type="text" id ="beneficiaryCustName" name="beneficiaryCustName" />
          <%
       	     ...
       	     %>
       	     <input type="text" id ="beneficiaryCustName" name="beneficiaryCustName" value="<%=formObj.getBeneficiaryCustName()%>"/>
       	      <%
       	      	}} else{
          	     %>
          	     <input type="text" id ="beneficiaryCustName" name="beneficiaryCustName"/>
									...
									<logic:present name="restBeneList">
										<logic:iterate id="ob" name="restBeneList"
											...
											<tr class="<%=rowClass%>">
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getBeneName()%>"/>&nbsp;</center>
												</td>
												<td>
												...
												<td>
												<center><integro:empty-if-null value="<%=ob.getBeneCustName()%>"/>&nbsp;</center>
												</td>
												 <td>
...
<tr class="odd" >	
       ...
       <%
       	if(formObj.getRunningAccountRestriction().equalsIgnoreCase("Y")){
       %>
        <html:select property="incoTerm" styleId="incoTerm"  value="incoTerm" disabled = "false" onchange ="refreshIncoDesc();" value="<%=formObj.getIncoTerm()%>">
                    <html:option value="">Please Select</html:option>
								<%
		...
		<%
 		...
 		%>	
		<html:select property="incoTerm" styleId="incoTerm"  value="incoTerm" disabled = "true" onchange ="refreshIncoDesc();" value="<%=formObj.getIncoTerm()%>">
                    <html:option value="">Please Select</html:option>
								<%
									if (incoTermList == null|| incoTermList.size() == 0) {
															}
...
<!-- New changes for Goods Restriction Start-->
<!--	-->
<tr class="even" >	
	   <td class="fieldname"></td>
	    <td  colspan="6">
 ...
 <%-- <%if(formObj.getGoodsRestriction().equalsIgnoreCase("Y")){ %> --%>
							<table border="0" cellpadding="0" cellspacing="0" class="tblinfo"
								...
								<tbody>
									...
									<logic:notPresent name="<%=ILmtCovenantConstants.SESSION_COVENANT_GOODS_RESTRICTION_LIST%>">
						<tr class="odd">
...
<!-- New changes for Goods Restriction End-->
...
<tr class="even" >	  
	   ...
	   <td colspan="6">
	   <%if(formObj.getCurrencyRestriction().equalsIgnoreCase("Y")){ %>
                    ...
                    <html:option value="">Please Select</html:option>
								<%
									if (currencyList == null
											...
											while (iter2.hasNext()) {
												LabelValueBean labValBean1 = (LabelValueBean) iter2.next();
												String currentLabel1 = labValBean1.getLabel();
												String currentValue1 = labValBean1.getValue();
								%>
								<html:option value="<%=currentLabel1%>"><%=currentLabel1%>
								</html:option>
			...
			<input type="text" id = "currencyRestrictionAmount1" name ="currencyRestrictionAmount" onblur="javascript:formatAmountAsCommaSeparated(this);" />
			&nbsp;&nbsp;
		 ...
		 <html:hidden property="currencyRestrictionCode"/>
		  <html:hidden property="currencyRestrictionAmount"/>
		  <html:errors property="currencyRestrictionAmount" />
	   </td>
</tr>	
<tr class="even">
<td class="fieldname"></td>
	    ...
	    <%if(formObj.getBankRestriction().equalsIgnoreCase("Y")){ %>
                    ...
                    <html:option value="">Please Select</html:option>
								<%
									if (bankList == null
											...
											while (iter1.hasNext()) {
												LabelValueBean labValBean2 = (LabelValueBean) iter1.next();
												String currentLabel2 = labValBean2.getLabel();
												String currentValue2 = labValBean2.getValue();
								%>
								<html:option value="<%=currentLabel2%>"><%=currentLabel2%>
								</html:option>
		  ...
		  <%} %>	
			...
			<html:hidden property="bankRestrictionAmount"/>
			 <html:errors property="bankRestrictionAmount" />
