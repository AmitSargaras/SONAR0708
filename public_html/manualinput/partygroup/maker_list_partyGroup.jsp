...
<%
	SearchResult searchResult =(SearchResult) session.getAttribute("com.integrosys.cms.ui.manualinput.partygroup.MIPartyGroupAction.partyGroupList");
	...
	if (searchResult != null) {
		...
		pageContext.setAttribute("partyGroupSize", Integer.toString(listSize));
		pageContext.setAttribute("partyGroupList", resultList);
	}else
...
function createPartyGroup() {
 	...
 	document.forms[0].action = "MIPartyGroup.do?event=maker_prepare_create_party_group&startIndex=<%=startIndex%>";
	...
	function searchParty()
	{
        ...
        {
	        document.forms[0].action            = "MIPartyGroup.do";
	        document.forms[0].event.value       = "maker_list_party_group";
	        document.forms[0].submit();
        }
        else
        {
	        if(searchType !='' && searchText!='')
		    {	        	
		        document.forms[0].action            = "MIPartyGroup.do?type="+searchType +"&"+ "text="+searchText;		        
		        document.forms[0].event.value       = "maker_list_party_group";		              
				document.forms[0].submit();
	        }	       
	    }      
			...
			<table width="95%" border="0" align="center" cellpadding="0"
				...
				<thead>
                   ...
                   <td><label>Search :&nbsp;</label>
	           		...
	           		&nbsp;<img alt="Search" src="./img/search.gif" onclick="javascript:searchParty()">
                   </td>
                   <td>&nbsp;</td>
                   <td  valign="bottom" align="right">						
						<input type="button" name="Submit" value="Add New" class="btnNormal" onclick="createPartyGroup()" />
				   </td>    
               </tr>
               <tr>
                   <td>&nbsp;</td>
					...
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<thead>
								<tr>
									<td width="5%">S/N</td>
									<td width="32%">Party Group Code</td>
									<td width="37%">Party Group Name</td>
									<td width="32%">Grp Exp Limit</td>
								...
								<logic:present name="partyGroupList">
									<logic:iterate id="ob" name="partyGroupList"
										...
										<tr class="<%=rowClass%>">
											...
											<td><center><%=ob.getPartyCode()%>&nbsp;</center></td>
											<td><center><%=ob.getPartyName()%>&nbsp;</center></td>
											<!-- <td><center><%=ob.getGroupExpLimit().getAmountAsBigDecimal()%> <%=ob.getGroupExpLimit().getCurrencyCode()%>&nbsp;</center></td> -->
											<td><center><%=UIUtil.formatWithCommaAndDecimal(String.valueOf(ob.getGroupExpLimit().getAmountAsBigDecimal()))%> <%=ob.getGroupExpLimit().getCurrencyCode()%>&nbsp;</center></td>
											...
											<td><select name="menu1"
												...
												<%
													if (event.equals("checker_list_partyGroup")) {
												%>
												<option
													value='MIPartyGroup.do?event=checker_view_party_group&partyCode=<%=ob.getId()%>'>View</option>
												<%
													} else {
												%>
												<option
													value='MIPartyGroup.do?event=maker_view_party_group&partyCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>View</option>
												...
												%>
												<option
													value='MIPartyGroup.do?event=maker_edit_party_group_read&partyCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Edit</option>
												<option
													value='MIPartyGroup.do?event=maker_disable_party_group&partyCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Disable</option>
												<%
													} else {
												%>
												<option
													value='MIPartyGroup.do?event=maker_enable_party_group&partyCode=<%=ob.getId()%>&startIndex=<%=startIndex%>'>Enable</option>
												<%
													}
												%>
												<%
...
<td height="25">
 ...
 <%
                            String pageIndexUrl=null;
                                ...
                                pageIndexUrl = "MIPartyGroup.do?event=list_pagination&startIndex=";
