...
DateFormat df = new SimpleDateFormat("dd-MMM-yyyy");
	MaintainHolidayForm holidayForm = (MaintainHolidayForm) request
			.getAttribute("MaintainHolidayForm");

	IHolidayTrxValue holidayTrx = (IHolidayTrxValue) session
			.getAttribute("com.integrosys.cms.ui.holiday.MaintainHolidayAction.IHolidayTrxValue");
	...
	if (sr != null) {
		...
		pageContext.setAttribute("holidayList",
...
<%@page
	import="com.integrosys.cms.app.systemBankBranch.bus.ISystemBankBranch"%><html>
...
function approvePage() {
    ...
    document.forms[0].action="holiday.do?event=<%=submitEvent%>";
    document.forms[0].submit();
}
function rejectPage() {
    document.forms[0].action="holiday.do?event=<%=rejectEvent%>";
    document.forms[0].submit();
}
function cancelPage(){
													...
													offset="<%=String.valueOf(startIndex)%>"
                               						length="<%=String.valueOf(10)%>"
														...
														<tr class="<%=rowClass%>">
															...
															<td><%=ob.getDescription()%>&nbsp;
															</td>
															<td><%=ob.getStartDate()==null?"-":df.format(ob.getStartDate()) %>&nbsp;
															</td>
															<td><%=ob.getEndDate()==null?"-":df.format(ob.getEndDate()) %>&nbsp;
															</td>
															<%String isRecurrent = ob.getIsRecurrent();
																if(isRecurrent!=null && isRecurrent.trim().equalsIgnoreCase("on"))
																{
																isRecurrent = "checked";
																}else{
																isRecurrent = "";
															 	...
															 	%>
															 	<td><input type=checkbox name="isRecurrent" disabled="disabled" <%=isRecurrent%>>&nbsp;
														    	</td>														    	
															<td><%=ob.getLastUpdateDate()==null?"-":df.format(ob.getLastUpdateDate()) %>&nbsp;
															</td>
															<td><%=ob.getLastUpdateBy() %>&nbsp;
															</td>
														</tr>
													</logic:iterate>
													<%
														...
														<table width="100%" border="0" align="center"
															...
															<tbody>
																...
																<tr>
																	...
																	<td class="even"><%=holidayTrx.getUserInfo()%>&nbsp;
																	</td>
																</tr>
																<tr class="odd">
																	<td class="fieldname">
																	...
																	<td><%=holidayTrx.getRemarks() != null ? holidayTrx.getRemarks() : ""%>&nbsp;
																	</td>
																</tr>
															</tbody>
														</table>
