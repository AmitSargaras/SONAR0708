	...
	if (curDropDown != null && curDoc != null) {
		...
		if (toAdd == null) {	//fix for other browsers since IE can retrieve those under Name as Id
      	...
      	if (toAdd != null && optionList != null) {
      		...
      		if(optionList[0].getAttribute("fundedShareLimit") == 'null'){
      			document.getElementById("fundedShareLimit").value='';
      		}else{
      		document.getElementById("fundedShareLimit").value=optionList[0].getAttribute("fundedShareLimit");
      		}
      		
      		if(optionList[0].getAttribute("nonFundedShareLimit") == 'null'){
      			document.getElementById("nonFundedShareLimit").value='';
      		}else{
      		document.getElementById("nonFundedShareLimit").value=optionList[0].getAttribute("nonFundedShareLimit");
      		}
      		
      		if(optionList[0].getAttribute("totalSanctionedLimit") == 'null'){
      			document.getElementById("totalSanctionedLimit").value='';
      		}else{
      		document.getElementById("totalSanctionedLimit").value=optionList[0].getAttribute("totalSanctionedLimit");
      		}
		}	
	}	
}
