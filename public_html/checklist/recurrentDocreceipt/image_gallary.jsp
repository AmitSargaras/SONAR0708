...
List tageedImageList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.recurrentDocreceipt.RecurrentDocReceiptAction.tagDetailList");
List migCheckListImages = (List) session.getAttribute("com.integrosys.cms.ui.checklist.recurrentDocreceipt.RecurrentDocReceiptAction.migCheckListImages");
...
if (searchResult != null) {
	...
	if(null!=event && event.equals("retrieveMigImageGallaryRecurrentDoc"))
	{
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryRecurrentDoc')
	{
		 document.forms[0].action="RecurrentDocReceipt.do?event=view_image_page&category="+catg;
		   document.forms[0].event.value="view_image_page";
		  
		    document.forms[0].submit();
	}
	else if('<%=event%>'== 'retrieveMigImageGallaryRecurrentDoc')
	{
		 document.forms[0].action="RecurrentDocReceipt.do?event=view_mig_image_page&category="+catg;
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallaryRecurrentDoc'){
			 document.forms[0].action="RecurrentDocReceipt.do?event=retrieveImageGallaryRecurrentDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
			else if('<%=event%>' == 'retrieveMigImageGallaryRecurrentDoc'){
				 document.forms[0].action="RecurrentDocReceipt.do?event=retrieveMigImageGallaryRecurrentDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
