...
<%
    ...
    Map checkListIDMap = (HashMap)session.getAttribute("com.integrosys.cms.ui.checklist.facility.FacilityCheckListAction.checkListIDMap");
    //System.out.print(checkListIDMap.size());
    ILimit limit = null ;
    ...
    if(checkListTrxVal.getCheckList() !=null){
    	...
    	String colleteral_id = String.valueOf(((OBCollateralCheckListOwner)checkListTrxVal.getStagingCheckList().getCheckListOwner()).getCollateralID());
	    ILimit[] limitList= limitProfile.getLimits();
	    for(int ab=0;ab<limitList.length;ab++){
	    	if(colleteral_id.equals(String.valueOf(limitList[ab].getLimitID()))){
	    		limit=limitList[ab];
	    	}
	    }
    }
      ...
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
          ...
          <tbody>
            <tr class="even">
              ...
              <td>
	              <%if(checkListTrxVal.getCheckList()!=null && checkList.getCheckListID()!=-999999999 ){ %>
	              <integro:empty-if-null value="<%= String.valueOf(checkListTrxVal.getCheckList().getCheckListID())%>"/>
	              <%}else{ %>
	              -
	              <%} %>
              </td>
