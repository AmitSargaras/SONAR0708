...
<%
    ...
    CheckListSearchResult LADCheckList = (CheckListSearchResult)request.getAttribute("LADCheckList");
   String LADStatus;
   String LADLocation;
   String LADChecklistId;
   String LADCheckListType;
   if(LADCheckList!=null){
	    LADStatus=LADCheckList.getCheckListStatus();
	    LADLocation=LADCheckList.getCheckListLocation().getCountryCode();
	    LADChecklistId=String.valueOf(LADCheckList.getCheckListID());
	    LADCheckListType=LADCheckList.getCheckListType();
   }else{
	   LADStatus="NEW";
	    LADLocation="IN";
	    LADChecklistId="-999999999";
	    LADCheckListType="LAD";
   }
   // long LADCheckListID = Long.parseLong(tLADCheckListID);
  //  System.out.println("LAD checklist id :::::::::::"+LADCheckList );
...
function maintain(checkListID,secType,secSubType,limitBkgLoc,orgCode, collateralID, custCategory, subprofileID, applicationType){
    formObj = document.forms["LADCheckListForm"];
...
<!--             <td width="9%"><bean:message key="label.document.security.location"/></td>-->
           ...
           <!-- 
              ...
              <td width="7%">LAD ID</td>
              <td width="14%" nowrap>LAD Type /<br />SubType</td>
   ...
   %>
    <tr class="<%=rowClass%>">
              	...
              	<%
              	if(LADChecklistId=="-999999999"){
              	%>
              	<td valign="top" >-&nbsp;</td>
              	<%}else{ %>
              	<td valign="top" >&nbsp;<%=LADChecklistId %></td>
              	<%} %>
              		<td valign="top" >&nbsp; <% if(LADCheckListType.equals("LAD") ) {%>
              	LAD <%} %>

</td>
<!--              	<td valign="top" >&nbsp;<%=LADLocation%></td>-->
              	<td valign="top" >&nbsp;<%=LADStatus%></td>	
              	<td valign="top" >&nbsp;
...
//if(LADCheckList.getTrxStatus()==null){
//	canView=true;
//}
//if(LADCheckList.getTrxStatus()!=null && !(LADCheckList.getTrxStatus().equals("ACTIVE"))){
	//canView=true;
//}
...
%>
   ...
   <a href="LADReceipt.do?event=view&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=LADChecklistId %>&&secType=LAD&&secSubType=LAD"  ><bean:message key="label.view"/></a>
        <%}else{ %>
          <a href="LADReceipt.do?event=prepare_update&&limitBkgLoc=IN&&orgCode=000&&checkListID=<%=LADChecklistId %>&&secType=LAD&&secSubType=LAD"  ><bean:message key="label.process"/></a>
        <%} %>
   <%}else{ %>
&nbsp;-     
<%} %>
