...
function showListing() {
	//alert('1');
	var catg= '<%=category%>';
	if('<%=event%>'== 'mkr_view_draft_close')
	{
		 document.forms[0].action="LADReceipt.do?event=view_close_image_draft&category="+catg;
		   document.forms[0].event.value="view_close_image_draft";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'mkr_view_draft_close'){
				 document.forms[0].action="LADReceipt.do?event=mkr_view_draft_close&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
