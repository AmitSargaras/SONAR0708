...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'makerProcessImageLADDoc')
	{
		 document.forms[0].action="LADReceipt.do?event=view_process_image_page&category="+catg;
		   document.forms[0].event.value="view_process_image_page";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'makerProcessImageLADDoc'){
			 document.forms[0].action="LADReceipt.do?event=makerProcessImageLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
