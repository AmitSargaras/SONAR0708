...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="LADReceipt.do?event=retrieveImageGallaryLADDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
function printPage() {
...
<input type="hidden" name="id" value="<%=docIdLAD %>"/>
...
<p align="center">
 <%   if (canView) { %>
                <a href="LADReceipt.do?event=prepare_view_lad&operation=LADREC&id=<%=docIdLAD + urlForView%>" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image1','','img/return2.gif',1)"><img src="img/return1.gif" name="Image1"  border="0" id="Image1" /></a>
             <% } else { %>
               <a href="LADReceipt.do?event=prepare_view_lad&operation=LADREC&id=<%=docIdLAD + urlForView%>" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image2','','img/cancel2.gif',1)">
