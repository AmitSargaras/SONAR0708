...
function closePage() {
	document.forms[0].action="LADReceipt.do?event=return_draft_close_view_lad";
	document.forms[0].submit();
}
function returnPage() {
	document.forms[0].action="LADReceipt.do?event=return_draft_process_view__lad";
	document.forms[0].submit();
}
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="LADReceipt.do?event=mkr_view_draft_close&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}

function getImageGallaryLADDoc(imagePath,imageName,status,count) {
//  var dep = 'imagePath';
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  //sendLoadImagePathReq(dep, url);
  var catg= document.forms[0].category.value;
  ...
  //alert("getImageGallary");
	...
	//alert("category is "+catg);
	//alert("count is "+count);
	document.forms[0].action="LADReceipt.do?event=mkr_view_draft_process&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
																	...
																	<tr class=<%=rowClass%>>
																		...
																		<%
																			String subs = obImageUploadAdd.getImgFileName();
																						//subs = subs.substring();
																		%>
<%if("view_close_image_draft".equals(event)){ %>
																		<td width="25%">
																			&nbsp;
...
<%} %>	
																		...
																		<%}else if("view_process_image_draft".equals(event)){ %>
																		
																		<td width="25%">
																			&nbsp;
																		<%if(obImageUploadAdd.getStatus()==3){ %>	
																		<a href="#"
onclick="javascript:getImageGallaryLADDoc('<%=obImageUploadAdd.getImageFilePath().trim()%>','<%=obImageUploadAdd.getImgFileName().trim()%>','<%=obImageUploadAdd.getStatus()%>','<%=counter1-1%>')"><%=obImageUploadAdd.getImgFileName()%>
</a>
<%}else{ %>
<%=obImageUploadAdd.getImgFileName()%>
<%} %>	
...
<p align="center">
 <%   if ("view_process_image_draft".equals(event)) { %>
