...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveLADDocImage')
	{
		 document.forms[0].action="LADReceipt.do?event=view_image_page&category="+catg;
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveLADDocImage'){
				 document.forms[0].action="LADReceipt.do?event=retrieveLADDocImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
		}
