...
List tagDetailList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.tagDetailList");
List migCheckListImages = (List) session.getAttribute("com.integrosys.cms.ui.checklist.camreceipt.CAMReceiptAction.migCheckListImages");
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryCAMDoc')
	{
		 document.forms[0].action="CAMReceipt.do?event=view_image_page&category="+catg;
	...
	{
		 document.forms[0].action="CAMReceipt.do?event=view_mig_image_page&category="+catg;
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallaryCAMDoc'){
			 document.forms[0].action="CAMReceipt.do?event=retrieveImageGallaryCAMDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
			else if('<%=event%>' == 'retrieveMigImageGallaryCAMDoc'){
				 document.forms[0].action="CAMReceipt.do?event=retrieveMigImageGallaryCAMDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
