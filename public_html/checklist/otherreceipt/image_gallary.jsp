...
List tageedImageList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction.tagDetailList");
List migCheckListImages = (List) session.getAttribute("com.integrosys.cms.ui.checklist.otherreceipt.OtherReceiptAction.migCheckListImages");
...
if (searchResult != null) {
	...
	if(null!=event && event.equals("retrieveMigImageGallaryOtherDoc"))
	{
		resultList =  migCheckListImages;
		
		pageContext.setAttribute("resultList", migCheckListImages);
	}
	else{
		resultList =  tageedImageList;
	
		pageContext.setAttribute("resultList", tageedImageList);
	}
}
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryOtherDoc')
	{
		 document.forms[0].action="OtherReceipt.do?event=view_image_page&category="+catg;
		   document.forms[0].event.value="view_image_page";
		  
		    document.forms[0].submit();
	}
	else if('<%=event%>'== 'retrieveMigImageGallaryOtherDoc')
	{
		 document.forms[0].action="OtherReceipt.do?event=view_mig_image_page&category="+catg;
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallaryOtherDoc'){
			 document.forms[0].action="OtherReceipt.do?event=retrieveImageGallaryOtherDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
			else if('<%=event%>' == 'retrieveMigImageGallaryOtherDoc'){
				 document.forms[0].action="OtherReceipt.do?event=retrieveMigImageGallaryOtherDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
					document.forms[0].submit();
				}
...
<%
	...
	if(null!=event && event.equals("retrieveMigImageGallaryOtherDoc")) 
	{
		...
		System.out.println("object is"+migCheckListImages.get(Integer.parseInt(request.getParameter("count"))));
		if(Integer.parseInt(cnt)!=(migCheckListImages.size())){
			  strArray = (String[]) migCheckListImages.get(Integer.parseInt(request.getParameter("count")));
		}else{
			obj = (OBImageUploadAdd) migCheckListImages.get((Integer.parseInt(request.getParameter("count")))-1);
		}
		
	}
	else{
		
		if(Integer.parseInt(cnt)!=(tageedImageList.size())){
		obj = (OBImageUploadAdd) tageedImageList.get(Integer.parseInt(request.getParameter("count")));
		}else{
			obj = (OBImageUploadAdd) tageedImageList.get((Integer.parseInt(request.getParameter("count")))-1);
		}
	...
	<b style="font-size: x-small">Image Name :</b>&nbsp;<font size="2"><%=strArray[4] %></font></td> &nbsp;&nbsp;&nbsp;&nbsp;
	 <td><b style="font-size: x-small">Category:</b>&nbsp;
		...
		<td><b style="font-size: x-small">Document Name:</b>&nbsp;<font size="2">
		<%-- <integro:empty-if-null value="<%=obj.getDocumentName() %>"/></font>	 --%>	
		<integro:empty-if-null value="<%=documentName %>"/></font>											
...
<%if(null!=event && event.equals("retrieveMigImageGallaryOtherDoc")) { %>
<a href="javascript:getImageGallary1('<%=request.getParameter("pID").trim()%>','<%=request.getParameter("imageName").trim()%>','<%=request.getParameter("status")%>','<%=migCheckListImages.size()-1%>','<%=request.getParameter("category")%>');" >
	<img src="img/last.jpg"  width="30" height="17" border="0"  align="middle"/>
</a>
<%}else{ %>
<a href="javascript:getImageGallary1('<%=request.getParameter("pID").trim()%>','<%=request.getParameter("imageName").trim()%>','<%=request.getParameter("status")%>','<%=tageedImageList.size()-1%>','<%=request.getParameter("category")%>');" >
	<img src="img/last.jpg"  width="30" height="17" border="0"  align="middle"/>
</a>
<%} %>
&nbsp;&nbsp;&nbsp;
