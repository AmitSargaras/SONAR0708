...
function tagUntagImage(){
	document.forms["OtherReceiptForm"].event.value = "<%=OtherReceiptAction.EVENT_PREPARE_TAG_UNTAG_IMAGE_VIEW%>";
...
<html:form action="OtherReceipt.do">
...
//      if (request.getParameter("event") != null && "view_receipt".equals(request.getParameter("event"))) {
        ...
        %>
             <td width="75" valign="baseline" align="center">
                <a href="OtherReceipt.do?event=view&checkListID=<%=checkListID%>&limitBkgLoc=<%=limitBkgLoc%>&secType=<%=secType%>&secSubType=<%=secSubType%>&collateralID=<%=collateralID%>&limitProfileID=<%=limitProfileID%>"
