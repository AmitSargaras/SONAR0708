...
function closePage() {
	document.forms["OtherReceiptForm"].event.value="list";
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="OtherReceipt.do?event=retrieveMigImageGallaryOtherDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg;
	document.forms[0].submit();
}
...
function getImageForDisplayOtherDoc(imagePath,imageName,status) {
	   /*var dep = 'imagePath';
	   var url = '/cms/OtherReceipt.do?event=retrieveImage&pID='+imagePath+'&imageName='+imageName+'&status='+status;
	   sendLoadImagePathReq(dep, url);*/
	document.getElementById("imageFrame").src= '/cms/OtherReceipt.do?event=retrieveImage&pID='+imagePath+'&imageName='+imageName+'&status='+status;
	}
</script>
</head>
<body> 
