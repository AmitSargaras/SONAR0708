...
List tageedImageList  = (List) session.getAttribute("com.integrosys.cms.ui.checklist.secreceipt.SecurityReceiptAction.tagDetailList");
...
String custLimitProfileID=request.getParameter("custLimitProfileID");
	...
	request.setAttribute("tagDetailList",list1);
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallarySecurityDoc')
	{
		 document.forms[0].action="SecurityReceipt.do?event=view_image_page&category="+catg;
  ...
  //sendLoadImagePathReq(dep, url);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallarySecurityDoc'){
			 document.forms[0].action="SecurityReceipt.do?event=retrieveImageGallarySecurityDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
