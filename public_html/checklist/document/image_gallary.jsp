...
List tagDetailList = (List) session.getAttribute("com.integrosys.cms.ui.checklist.document.DocumentCheckListAction.tagDetailList");
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryChecklistDoc')
	{
		 document.forms[0].action="DocumentCheckList.do?event=view_checklist_image&category="+catg;
		   document.forms[0].event.value="view_checklist_image";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallaryChecklistDoc'){
			 document.forms[0].action="DocumentCheckList.do?event=retrieveImageGallaryChecklistDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
