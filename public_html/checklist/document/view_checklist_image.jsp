...
<%
	...
	List list1 = null;
	String docIdLAD=(String)request.getAttribute("docIdLAD");
%>
...
HashMap selectedArrayMap = (HashMap) session.getAttribute("com.integrosys.cms.ui.checklist.document.DocumentCheckListAction.selectedArrayMap");
...
function returnPage() {
    document.forms["FacilityReceiptForm"].event.value = "view";
...
function downloadImageZip() {
	...
	if(s_value!=''){
	    ...
	    //document.forms[0].submit();
	    var aLocation = "DocumentCheckList.do?event=downloadImage&custId=<%=custId%>&imageId="+s_value+"&unCheckId="+n_value;
	window.location.href = aLocation ;
...
//  var url = '/cms/ImageTagMap.do?event=retrieveImage&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath;
  ...
  //alert("getImageGallary");
	...
	document.forms[0].action="DocumentCheckList.do?event=retrieveImageGallaryChecklistDoc&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+catg+"&imageId="+s_value+"&unCheckId="+n_value;
	document.forms[0].submit();
}
function printPage() {
	...
	if(s_value!=''){
	   ...
	   MM_openBrWindow1("DocumentCheckList.do?event=print_image&custId=<%=custId%>&customerName="+customerName+"&category="+catg+"&custLimitProfileID="+<%=custLimitProfileID%>+"&imageId="+s_value+"&unCheckId="+n_value,'_blank','scrollbars=no,toolbar=no,menubar=no,resizable=no,width=0,height=0,left=100,top=100')
	  // window.focus(); 
		} else{
