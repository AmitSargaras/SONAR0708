...
<script type="text/javascript">
	function sbmForm(nextTab) {
		document.forms[0].event.value = "<%=(lastEvent==null)?FacilityMainAction.EVENT_NAVIGATE_VIEW_WO_FRAME:FacilityMainAction.EVENT_NAVIGATE_WO_FRAME%>";
		document.forms[0].nextTab.value = nextTab;
		document.forms[0].submit();
	}
	function save_whole_obj() {
		enableAll();
		document.forms[0].event.value="<%=(lastEvent==null)?FacilityMainAction.EVENT_SAVE_WHOLE_OBJ:FacilityMainAction.EVENT_SAVE_WHOLE_OBJ_WO_FRAME%>";
		document.forms[0].submit();
	}
	function submit_whole_obj() {
		document.forms[0].event.value="<%=(lastEvent==null)?FacilityMainAction.EVENT_SUBMIT:FacilityMainAction.EVENT_SUBMIT_WO_FRAME%>";
		document.forms[0].submit();
	}
...
<%
	...
	if (currentTab != null) {
		if (FacilityMainAction.TAB_MASTER.equals(currentTab)
				|| FacilityMainAction.TAB_MASTER_VIEW_WO_FRAME.equals(currentTab)) {
			styleTabFacilityMaster = "tabCurrent";
		}
		else if (FacilityMainAction.TAB_BNM_CODES_VIEW_WO_FRAME.equals(currentTab)) {
			styleTabBNMCodes = "tabCurrent";
		}
		else if (FacilityMainAction.TAB_OFFICER.equals(currentTab)
				|| FacilityMainAction.TAB_OFFICER_VIEW_WO_FRAME.equals(currentTab)) {
			styleTabOfficer = "tabCurrent";
		}
		else if (FacilityMainAction.TAB_RELATIONSHIP.equals(currentTab)
				|| FacilityMainAction.TAB_RELATIONSHIP_VIEW_WO_FRAME.equals(currentTab)) {
			styleTabRelationship = "tabCurrent";
		}
		else if (FacilityMainAction.TAB_INSURANCE_VIEW_WO_FRAME.equals(currentTab)) {
			styleTabInsurance = "tabCurrent";
		}
		else if (FacilityMainAction.TAB_ISLAMIC_MASTER_VIEW_WO_FRAME.equals(currentTab)) {
		...
		<table border="0" cellpadding="0px" cellspacing="0px"
			...
			<tr style="text-align: left">
				...
				<td class="<%=styleTabFacilityMaster %>"><a
					href="javascript:sbmForm('<%=FacilityMainAction.TAB_MASTER_VIEW_WO_FRAME %>');"><bean:message
					key="label.facility.master" /></a></td>
				<td class="<%=styleTabBNMCodes %>"><a
					href="javascript:sbmForm('<%=FacilityMainAction.TAB_BNM_CODES_VIEW_WO_FRAME %>');"><bean:message
					key="label.facility.bnm.codes" /></a></td>
				<td class="<%=styleTabOfficer %>"><a
					href="javascript:sbmForm('<%=FacilityMainAction.TAB_OFFICER_VIEW_WO_FRAME %>');"><bean:message
					key="label.facility.officer" /></a></td>
				<td class="<%=styleTabRelationship %>"><a
					href="javascript:sbmForm('<%=FacilityMainAction.TAB_RELATIONSHIP_VIEW_WO_FRAME %>');"><bean:message
					key="label.facility.relationship" /></a></td>
				<% if (StringUtils.equals(allowInsurance, "Y")) {%>
                <td class="<%=styleTabInsurance %>"><a
					href="javascript:sbmForm('<%=FacilityMainAction.TAB_INSURANCE_VIEW_WO_FRAME %>');"><bean:message
                ...
                <% if (applicationLawType.equals(ICMSConstant.AA_LAW_TYPE_ISLAM)) { %>
                    ...
                    <td class="<%=styleTabFacIslamicMaster %>"><a
                        href="javascript:sbmForm('<%=FacilityMainAction.TAB_ISLAMIC_MASTER_VIEW_WO_FRAME %>');"><bean:message
		...
		<table border="0" width="10%" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<td valign="baseline" style="text-align: right"><a
					href="ToDo.do?event=prepare&isMenu=Y" onmouseout="MM_swapImgRestore()"
