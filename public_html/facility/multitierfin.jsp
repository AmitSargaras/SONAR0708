...
<%@page	import="com.integrosys.cms.ui.limit.facility.multitierfin.MultiTierFinForm"%>
...
<%
	...
	String limitProfileLawType = (limitProfile == null) ? null : limitProfile.getApplicationLawType();
	...
	List compareResultsList = (List) session
			.getAttribute("com.integrosys.cms.ui.limit.facility.main.FacilityMainAction.compareResultsMultiTierFinList");
...
<table width="100%" height="100%" cellspacing="0" cellpadding="0"
	border="0">
	<tr>
		<td valign="top"><html:form action="FacilityMultiTierFin.do">
			...
			<table width="97%" border="0" align="center" cellpadding="0"
				...
				<tbody>
					...
					<tr>
						<%
							if (!FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
										&& !FacilityMainAction.EVENT_NAVIGATE_WO_FRAME_CHECKER.equals(event)
										&& !FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event)
										&& !FacilityMainAction.EVENT_NAVIGATE_VIEW_WO_FRAME.equals(event)
										&& !FacilityMainAction.EVENT_NAVIGATE_VIEW.equals(event)
										&& !FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW.equals(event)
										&& !isProcessHere
										&& ICMSConstant.AA_LAW_TYPE_ISLAM.equals(limitProfileLawType)) {
						%>
						<%
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							<thead>
								<tr>
									...
									<td width="15%"><bean:message key="label.facility.multitierfin.seq.no" /></td>
									<td width="20%"><bean:message
										key="label.facility.multitierfin.tier.term" /></td>
									<td width="25%"><bean:message
										key="label.facility.multitierfin.tier.term.code" /></td>
									<td width="25%"><bean:message
										key="label.facility.interest.rate" /></td>
							...
							<tbody>
								<%
									if (!(
											//FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event) || FacilityMainAction.EVENT_NAVIGATE_WO_FRAME_CHECKER.equals(event)||
												isProcessHere)) {
								%>
								<logic:empty name="multiTierFins">
									<tr class="odd">
										<td colspan="6"><bean:message key="label.global.not.found" /></td>
									</tr>
								</logic:empty>
								<logic:notEmpty name="multiTierFins">
									<logic:iterate id="multiTierFin"
										name="multiTierFins"
										type="com.integrosys.cms.app.limit.bus.IFacilityMultiTierFinancing"
										indexId="indexId">
										<tr class="<%=style[(indexId.intValue())%2] %>">
											...
											%> <integro:empty-if-null
												value="<%=multiTierFin.getTierSeqNo()%>" />
											<%
												}
											%>&nbsp;
											</td>
											...
											%> <integro:empty-if-null
												value="<%=multiTierFin.getTierTerm()%>" />
											<%
												}
											%>
											</td>
											<td>
												...
												if (multiTierFin.getTierTermCode() != null) {
													String tierTermCode = String.valueOf(multiTierFin.getTierTermCode());
												%>
												<integro:common-code
							 ...
							 if (!FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
									...
									&& !FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW.equals(event)) {
											%> <select onchange="MM_jumpMenu('self',this,0)">
												...
												<%
										if (FacilityMainAction.EVENT_LIST_WO_FRAME.equals(event)) {
												if (ICMSConstant.AA_LAW_TYPE_ISLAM.equals(limitProfileLawType)) {
												%>													
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_EDIT_WO_FRAME %>&key=<%=indexId.intValue() + startIdx %>"><bean:message
													key="label.edit" /></option>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_DELETE_WO_FRAME %>&key=<%=indexId.intValue() + startIdx %>"><bean:message
												...
												%>
												<option
													value="FacilityMultiTierFin.do?event=view_detail_only&lastEvent=list_no_frame&key=<%=indexId.intValue() + startIdx %>"><bean:message
													key="label.view" /></option>
												<%
												}
										}
										else {
												...
												%>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_EDIT %>&key=<%=indexId.intValue() + startIdx %>"><bean:message
													key="label.edit" /></option>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_DELETE %>&key=<%=indexId.intValue() + startIdx %>"><bean:message
													key="label.global.delete" /></option>
												<%
												}
												else {
												%>
												<option
													value="FacilityMultiTierFin.do?event=view_detail_only_w_frame&lastEvent=list&key=<%=indexId.intValue() + startIdx %>"><bean:message
													key="label.view" /></option>
												<%
												}
										}
 								...
 								if (FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
 										|| FacilityMainAction.EVENT_NAVIGATE_WO_FRAME_CHECKER.equals(event)) {
 %> 											<a
												href="FacilityMultiTierFin.do?event=view_detail_checker&key=<%=indexId.intValue() + startIdx %>"><bean:message
												key="label.view" /></a> <%
 								}
 								else if (FacilityMainAction.EVENT_NAVIGATE_VIEW_WO_FRAME.equals(event)
 										|| FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event)) {
 %>												<a
												href="FacilityMultiTierFin.do?event=view_detail_only&key=<%=indexId.intValue() + startIdx %>"><bean:message
												key="label.view" /></a> <%
 								}
 								else if (FacilityMainAction.EVENT_NAVIGATE_VIEW.equals(event)
 										|| FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW.equals(event)) {
 %> 											<a
												href="FacilityMultiTierFin.do?event=view_detail_only_w_frame&key=<%=indexId.intValue() + startIdx %>"><bean:message
												key="label.view" /></a> <%
 								}
 						}
 %>
								...
								<logic:notEmpty name="compareResultsList">
									<logic:iterate id="result" name="compareResultsList"
										...
										indexId="indexId">
										<%
											IFacilityMultiTierFinancing multiTierFin = (IFacilityMultiTierFinancing) result
																			.getObj();
										%>
										<tr class="<%=style[(indexId.intValue())%2] %>">
											...
											<%
												if (!FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
																				&& !FacilityMainAction.EVENT_NAVIGATE_WO_FRAME_CHECKER.equals(event)
																				&& !FacilityMainAction.EVENT_NAVIGATE_VIEW_WO_FRAME.equals(event)
																				&& !FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event)&&!isProcessHere) {
											%> <select onchange="MM_jumpMenu('self',this,0)">
												<option value="#"><bean:message
													key="label.please.select" /></option>
												<%
												...
												%>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_EDIT_WO_FRAME %>&key=<%=indexId %>"><bean:message
													key="label.edit" /></option>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_DELETE_WO_FRAME %>&key=<%=indexId %>"><bean:message
													key="label.global.delete" /></option>
												<%
													}
																				else {
												%>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_EDIT %>&key=<%=indexId %>"><bean:message
													key="label.edit" /></option>
												<option
													value="FacilityMultiTierFin.do?event=<%=FacilityMainAction.EVENT_DELETE %>&key=<%=indexId %>"><bean:message
													key="label.global.delete" /></option>
												<%
													}
												%>
 								...
 								if (FacilityMainAction.TAB_MULTI_TIER_FINANCING_CHECKER.equals(event)
 										|| FacilityMainAction.EVENT_NAVIGATE_WO_FRAME_CHECKER.equals(event)) {
 %> <a href="FacilityMultiTierFin.do?event=view_detail_checker&key=<%=indexId%>"><bean:message
												key="label.view" /></a> <%
 	}
 								else if (FacilityMainAction.EVENT_NAVIGATE_VIEW_WO_FRAME.equals(event)
 										|| FacilityMainAction.TAB_MULTI_TIER_FINANCING_VIEW_WO_FRAME.equals(event)) {
 %><a href="FacilityMultiTierFin.do?event=view_detail_only&key=<%=indexId%>"><bean:message
												key="label.view" /></a> <%
 	}
 			}else{
 %><a href="FacilityMultiTierFin.do?event=view_detail_checker_process&key=<%=indexId%>"><bean:message
												key="label.view" /></a>
	<% } 
 								}%>
											</td>
                    ...
                    <tr>
                        <td colspan="2"><html:errors property="multiTierFinancingErrors" />&nbsp;</td>
                    </tr>
                </tbody>
			</table>
		</html:form></td>
 ...
 %> <integro:pageindex
					...
					url='<%="FacilityMultiTierFin.do?event=list_no_frame&startIdx="%>'
					submitFlag='<%= new Boolean(false)%>' /> <%
 	}
 	else {
 %> <integro:pageindex
					pageIndex='<%= new PageIndex(startIdx, FacilityMainAction.RECORD_PER_PAGE, recordSize)%>'
					url='<%="FacilityMultiTierFin.do?event=list&startIdx="%>'
					submitFlag='<%= new Boolean(false)%>' /> <%
 	}
 %>
				</td>
