...
<%@ page
	import="com.integrosys.base.techinfra.logger.DefaultLogger,
                 ...
                 java.util.Locale,
                 java.util.Arrays,
                 com.integrosys.cms.ui.collateralNewMaster.MaintainCollateralNewMasterForm,com.integrosys.cms.app.collateralNewMaster.trx.ICollateralNewMasterTrxValue,
                 com.integrosys.cms.app.collateralNewMaster.trx.OBCollateralNewMasterTrxValue,
...
function cancelPage(){
    document.forms[0].action="collateralNewMaster.do?event=maker_list_collateralNewMaster&startIndex="+<%=ind%>+"&go=N&newCollateralDescription=&newCollateralCode=&newCollateralMainType=&newCollateralSubType=";
    document.forms[0].submit();
}

function submitForm() {
 	document.forms[0].action="collateralNewMaster.do?event=maker_delete_collateralNewMaster";
    document.forms[0].submit();
}
...
<td >&nbsp;</td>
	...
	<table width="80%" border="0" align="center" cellpadding="0"
		...
		<thead>
			...
			<tr>
				<td>
				<h3><%if(collateralNewMasterForm.getStatus().trim().equalsIgnoreCase("ACTIVE")){ %>
                                     Delete Collateral New Master
                                      <%}else{ %>
                                   Enable Collateral New Master
                                      <%} %></h3>
				</td>
			</tr>
			<tr>
		...
		<tbody>
			<tr>
					...
					<tbody>
						...
						<tr class="odd">
                                    		...
                                    		categoryCode="<%=CategoryCodeConstant.COMMON_CODE_SECURITY_TYPE%>" descWithCode="false" />
												...
												<td>
												<integro:common-code-single entryCode="<%=collateralNewMasterForm.getNewCollateralSubType()%>" descWithCode="false"  display="true" refEntryCode="<%=collateralNewMasterForm.getNewCollateralMainType()%>"
											...
											<tr class="even">
												...
												<td><integro:empty-if-null value="<%=collateralNewMasterForm.getInsurance() %>"/>
												</td>
												<td class="fieldname">
													Revaluation Frequency
												</td>
												<td><integro:empty-if-null value="<%=collateralNewMasterForm.getRevaluationFrequencyCount() %>"/><integro:empty-if-null value="<%=collateralNewMasterForm.getRevaluationFrequencyDays() %>"/>
												</td>
              ...
              <td ><%if(collateralNewMasterForm.getStatus().trim().equalsIgnoreCase("ENABLE")){ %>Enable
              <%}else{ %>Disable<%} %>&nbsp;</td>
              ...
              <%}else{ %>Movable<%} %>&nbsp;</td>
                 ...
                 <td>
                 	...
                 	<%} %>
							...
							<html:hidden name="MaintainCollateralNewMasterForm"
								property="createBy" />
							<html:hidden name="MaintainCollateralNewMasterForm"
