...
<%
    ...
    ISCCertificate actCert = certTrxVal.getSCCertificate();
    ...
    ISCCertificateItem arrayActual[] = null;
    ISCCertificateItem arrayStaging[] = null;

    if(certTrxVal.getSCCertificate()==null)
    {    	
        arrayActual = null;        
    }
    else
    {
        arrayActual = certTrxVal.getSCCertificate().getSCCItemList();

    }
    
    if(certTrxVal.getStagingSCCertificate()!=null) {
	    arrayStaging = certTrxVal.getStagingSCCertificate().getSCCItemList();
	/* weiling : No need to clone staging for actual because all activated limit fields in maker page are mandatory */
	
	if (arrayStaging != null) {
		DefaultLogger.debug(this, "Array Length staging" + arrayStaging.length);
		if (arrayActual == null) {
			// copy arrayStaging to arrayActual
			arrayActual = new ISCCertificateItem[arrayStaging.length];
			DefaultLogger.debug(this, "copying arrayStaging to arrayActual");
			for (int i=0; i<arrayStaging.length; i++) {
				arrayActual[i] = (ISCCertificateItem) arrayStaging[i].clone();
			}
			// set the activated amount of each sc item to 0
            /*
			for (int i = 0; arrayActual != null && i < arrayActual.length; i++) {
				ISCCertificateItem actualItem = arrayActual[i];
				if (actualItem != null) {
    ...
    List res = CompareOBUtil.compOBArray(arrayStaging,arrayActual);

    ArrayList list = new ArrayList(res);
    pageContext.setAttribute("sccitems",list);
    ...
    ISCCertificateItem arrayActualClean[] = null;
    ISCCertificateItem arrayActualNotClean[] = null;
    ISCCertificateItem arrayStagingClean[] = null;
    ISCCertificateItem arrayStagingNotClean[] = null;

    if(actCert != null){
        arrayActualClean = actCert.getCleanSCCertificateItemList();
        arrayActualNotClean= actCert.getNotCleanSCCertificateItemList();
    }
    if(cert != null) {
        arrayStagingClean= cert.getCleanSCCertificateItemList();
        arrayStagingNotClean = cert.getNotCleanSCCertificateItemList();
    }
    ...
    if(cleanList!=null)
        pageContext.setAttribute("cleansccitems",cleanList);

    if(notCleanList!=null)
        pageContext.setAttribute("notcleansccitems",notCleanList);
...
<!-- InstanceBeginEditable name="menuScript" --> <!-- InstanceEndEditable -->
				   ...
				   String fam = famCode + ", " + famName ; 
                                  ...
                                  <td><%=custDetail.getLegalID()%>/<%= cert.getSCCertRef()%></td>                                </tr>
                                  -->
                              </tbody>
                            </table></td>
                        </tr>
                      ...
                      <tbody>
                        <tr>
                          <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                              ...
                              <tbody>
<logic:present name="cleansccitems"  >
     <logic:iterate id="compResult" name="cleansccitems" type="com.integrosys.base.techinfra.diff.CompareResult"  >
<%
    ISCCertificateItem OB = (ISCCertificateItem)compResult.getObj();
    String rowClass="";
    row++;
    if(row%2!=0){
       rowClass="odd";
	...
	if(OB.getProductDesc()!=null) {
		prodDesc = ((ISCCertificateItem)compResult.getObj()).getProductDesc();
		if(prodDesc !=null)
		{
...
<logic:notPresent name="cleansccitems" >
            <tr class="odd">
                        ...
                        <tbody>
                          <tr>
                            <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblInfo">
                                ...
                                <tbody>
  <logic:present name="sccitems"  >
       <logic:iterate id="compResult" name="sccitems" type="com.integrosys.base.techinfra.diff.CompareResult"  >
  <%--
  <logic:present name="notcleansccitems"  >
       <logic:iterate id="compResult" name="notcleansccitems" type="com.integrosys.base.techinfra.diff.CompareResult"  >--%>
<%
      ISCCertificateItem OB = (ISCCertificateItem)compResult.getObj();
      String rowClass="";
      row++;
              ...
              <% if (!OB.getIsInnerOuterSameBCA()) { %> </a> <% } %>
                ...
                <td width="12%">
                    <%if (OB.getCollateralAllocations() != null) {%>
                        <logic:iterate id="temp0" name="compResult" property="obj.collateralAllocations" ><%=((ICollateralAllocation)temp0).getCollateral().getSCISecurityID() %><br></logic:iterate>
                    <%} else {
                        out.println("-");
                    }%>
                    </td>
                ...
                <td width="12%">
                    <%if (OB.getCollateralAllocations() != null) {%>
                        <logic:iterate id="temp1" name="compResult" property="obj.collateralAllocations" ><bean:write name="temp1" property="collateral.collateralType.typeName" /><br></logic:iterate>
                    <%} else {
                        out.println("-");
                    }%>
                    </td>
  ...
  <logic:notPresent name="notcleansccitems" >
              <tr class="odd">
                <td colspan="9">There is no document</td>
              </tr>
  </logic:notPresent>
                          ...
                          <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tblInfo">
                              <tbody>
                                <tr>
                                  ...
                                  <td width="80%" class="odd"><textarea name="remarks" cols="90" rows="3"><%=remarks%></textarea></td>
                                </tr>
                                <tr>
