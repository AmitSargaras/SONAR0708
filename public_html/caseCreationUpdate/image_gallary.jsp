...
List tageedImageList = (List) session.getAttribute("com.integrosys.cms.ui.caseCreationUpdate.MaintainCaseCreationAction.tagDetailList");
...
String custLimitProfileID=request.getParameter("custLimitProfileID");
	...
	boolean flag = false;
	if("retrieveImageGallaryCaseCreationMenu".equals(event)){
		flag = true;
		event = "retrieveImageGallaryCaseCreation";
	}
...
function showListing() {
	var catg= '<%=category%>';
	if('<%=event%>'== 'retrieveImageGallaryCaseCreation' && <%=flag%> == false)
	{
		 document.forms[0].action="caseCreationUpdate.do?event=view_image_page&category="+catg;
		/*  document.forms[0].action="caseCreationUpdate.do?event=view_image_page_menu&category="+catg; */
		...
		/*  document.forms[0].action="caseCreationUpdate.do?event=view_image_page&category="+catg; */
			  document.forms[0].action="caseCreationUpdate.do?event=view_image_page_menu&category="+catg; 
			   document.forms[0].event.value="view_image_page_menu";
   ...
   // alert("imagePath is "+imagePath);
	...
	if(count<0){
	   alert("This is first Image.");
			...
			if('<%=event%>' == 'retrieveImageGallaryCaseCreation' || '<%=event%>' == 'retrieveImageGallaryCaseCreationMenu'){
			 document.forms[0].action="caseCreationUpdate.do?event=retrieveImageGallaryCaseCreation&imageName="+imageName+"&status="+status+"&count="+count+"&pID="+imagePath+"&category"+categ;
				document.forms[0].submit();
			}
...
<%
	...
	if(obj != null){
		String documentName = "";
		System.out.println("obj.getTypeOfDocument()=>"+obj.getTypeOfDocument());
		if("Facility".equals(obj.getTypeOfDocument())){ 
			System.out.println("obj.getFacilityDocName()=>"+obj.getFacilityDocName());
	...
	%>
		...
		<td><b style="font-size: x-small">Document Name:</b>&nbsp;<font size="2"><integro:empty-if-null value="<%=documentName %>"/></font> 
...
<html:hidden name="idType" property="idType" value="<%=id%>"/>
<html:hidden name="caseId" property="caseId" value="<%=caseId%>"/>
