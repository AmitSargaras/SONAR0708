...
<%
	...
	List autoupdationlmtsList = (List) session.getAttribute("com.integrosys.cms.ui.autoupdationlmtsupload.AutoupdationLmtsUploadAction.autoupdationlmtsList");
	...
	DefaultLogger.debug(this,"autoupdationlmtsList size : "+ autoupdationlmtsList.size());
	
	int sno = 0;
	String heading = "Auto Updation of Limits";
	String approve_event = "checker_approve_create_autoupdationlmts";
...
<%@page import="com.integrosys.cms.app.common.util.CommonUtil"%><html:form
	action='AutoupdationLmtsupload.do?'>
	<head>
...
function downloadImage(){ //v3.0
		var aLocation = "AutoupdationLmtsupload.do?event=downloadReport&system=";
...
function approvePage() {
	...
	//	if (x == 'Y') {
		if (x == true) {
		    document.forms[0].action="AutoupdationLmtsupload.do?event="+'<%=approve_event%>';
...
function rejectPage() {
	var x =document.getElementById("confirm").checked;
	if (x == true) {
    document.forms[0].action="AutoupdationLmtsupload.do?event=checker_reject_create_autoupdationlmts";
    document.forms[0].submit();
	}else{
        alert('Please confirm.!!!');
    }
