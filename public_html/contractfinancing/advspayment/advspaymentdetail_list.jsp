...
function JumpOnSelect(selectObj,advanceIndex,facilityType,moa) {
    ...
    if (selectObj.options[selectIndex].value == "view")
        window.location.href =  "CF_AdvsPayment.do?initAdvEvent=view&event=view&advanceIndex="+advanceIndex+"&facilityType="+facilityType+"&moa="+moa;
    if (selectObj.options[selectIndex].value == "edit")
        window.location.href =  "CF_AdvsPayment.do?initAdvEvent=maker_prepare_update&event=maker_prepare_update&advanceIndex="+advanceIndex+"&facilityType="+facilityType+"&moa="+moa;
    if (selectObj.options[selectIndex].value == "delete")
        window.location.href =  "CF_AdvsPayment.do?initAdvEvent=maker_prepare_delete&event=maker_prepare_delete&advanceIndex="+advanceIndex+"&facilityType="+facilityType+"&moa="+moa;
}
//-->
</script>
<!-- InstanceEndEditable -->
...
for (i=0;i<facilityTypeSummaryLength;i++){
    ...
    <thead>
        <tr class="even" align="center">
            ...
            <td colspan="5" class="fieldname"><bean:message key="label.advancepayment.advance.section"/></td>
            <td class="fieldname"><bean:message key="label.advancepayment.payment.section"/></td>
            <td class="fieldname"><bean:message key="label.advancepayment.distribution.payment.received"/></td>
            <td width="9%" rowspan="2" class="fieldname"><bean:message key="label.advancepayment.total.outstanding.amount"/></td>
            <td width="7%" rowspan="2" class="fieldname"><bean:message key="label.advancepayment.unpaid.os"/></td>
        ...
        <tr class="even" align="center">
            ...
            <td class="fieldname" width="9%"><bean:message key="label.advancepayment.amount"/></td>
            ...
            <td width="13%" class="fieldname"><bean:message key="label.advancepayment.total.amount.credited"/></td>
        </tr>
    </thead>
    <tbody>
        <%
                ...
                if (matchFacilityType) {
                    ...
                    <tr class='<%=(count%2==0?"even":"odd")%>'>
                        ...
                        if(isEdit!=null && isEdit.equals("true")) {
                            ...
                            %>
                            <td><center><a href="CF_AdvsPayment.do?initAdvEvent=view&event=view&advanceIndex=<%=j%>&facilityType=<%=facilityTypeList[i].getFacilityType()%>&moa=<%=facilityTypeList[i].getMoa()%>">View</a></center></td>
                            <%
                        }
                        %>
                    </tr>
        ...
        <tr class="even">
            ...
            <td width="4%" class="total">&nbsp;</td>
            <td width="7%" class="total">&nbsp;</td>
            <td width="6%" class="total" style="text-align:right"><bean:message key="label.contractfinancing.total"/></td>
            <td width="9%" class="total"style="text-align:left">
            <%
            if(!grandAmountError){
            ...
            <td width="13%" class="total" style="text-align:left">
            <%
            if(!grandTotalPaymentDistributedError){
            ...
            <td width="9%" style="text-align:left" class="total">&nbsp;</td>
            <td width="7%" class="total">&nbsp;</td>
