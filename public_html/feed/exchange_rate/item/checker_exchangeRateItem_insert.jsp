...
<%@ page
	...
	com.integrosys.cms.app.feed.trx.forex.OBForexFeedGroupTrxValue,
	com.integrosys.cms.app.feed.bus.forex.OBForexFeedEntry,
	com.integrosys.cms.app.feed.bus.forex.IForexFeedEntry,
	java.text.DateFormat,
    java.text.SimpleDateFormat" %>
<%
DateFormat df = new SimpleDateFormat("dd-MMM-yyyy");
	ExchangeRateItemForm exchangeRateItemForm = (ExchangeRateItemForm) request.getAttribute("exchangeRateItemForm");

	IForexFeedGroupTrxValue forexFeedGroupTrxValue = (IForexFeedGroupTrxValue) session.getAttribute("com.integrosys.cms.ui.feed.exchangerate.ForexAction.forexFeedGroupTrxValue");
    System.out.println("forexFeedGroupTrxValue----"+session.getAttribute("forexFeedGroupTrxValue"));

	SearchResult sr = (SearchResult) request
			.getAttribute("forexFeedEntryList");

	if (sr != null) {
		List forexFeedEntryList = new ArrayList(sr.getResultList());
		pageContext.setAttribute("forexFeedEntryList",
				forexFeedEntryList);
	}
...
function cancelPage(){
    document.forms[0].action="feed_exchange_rate_item.do?event=maker_list_exchangeRateItem";
    
    document.forms[0].submit();
}
function approvePage() {
	
    document.forms[0].action="feed_exchange_rate_item.do?event=<%=submitEvent%>";
    document.forms[0].submit();
}
function rejectPage() {
    document.forms[0].action="feed_exchange_rate_item.do?event=<%=rejectEvent%>";
    document.forms[0].submit();
}
function cancelPage(){
								...
								<tr>
									<td colspan="4">
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tbody>
												<logic:present name="forexFeedEntryList">
													<logic:iterate id="ob" name="forexFeedEntryList"
													offset="<%=String.valueOf(startIndex)%>"
                               						length="<%=String.valueOf(10)%>"
														type="com.integrosys.cms.app.feed.bus.forex.OBForexFeedEntry"
														scope="page">
														<%
															String rowClass = "";
																			sno++;
														...
														<tr class="<%=rowClass%>">
															...
															<td><integro:empty-if-null value="<%=ob.getBuyCurrency()%>"/>&nbsp;
															</td>
															<td><integro:empty-if-null value="<%=ob.getCurrencyIsoCode()%>"/>&nbsp;
															</td>
															<td><%=ob.getCurrencyDescription() %>&nbsp;
															</td>
															<td><%=ob.getBuyRate() %>&nbsp;
															</td>
															<td style="text-align:center">&nbsp;<%=ob.getEffectiveDate()==null?"-":df.format(ob.getEffectiveDate()) %></td>
															</td>
															<td><integro:empty-if-null value="<%=ob.getStatus()%>"/>&nbsp;
															</td>
												...
												<logic:notPresent name="forexFeedEntryList">
													<tr class="odd">
														<td colspan="10">
															<bean:message key="label.global.not.found" />
														</td>
														...
														<table width="100%" border="0" align="center"
															...
															<tbody>
																...
																<tr>
																	...
																	<td class="even"><%=forexFeedGroupTrxValue.getUserInfo()%>&nbsp;
																	</td>
																</tr>
																<tr class="odd">
																	<td class="fieldname">
																	...
																	<td><%=forexFeedGroupTrxValue.getRemarks() != null ? forexFeedGroupTrxValue.getRemarks() : ""%>&nbsp;
																	</td>
																</tr>
															</tbody>
														</table>
