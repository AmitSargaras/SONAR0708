...
<%
    ...
    IMutualFundsFeedGroup stagingFeedGroup = feedGroupTrxValue.getStagingMutualFundsFeedGroup();
    ...
    if (event.equals(MutualFundsListAction.EVENT_READ_CHECKER_APPROVE_REJECT)) {
        event = MutualFundsListAction.EVENT_LIST_CHECKER_APPROVE_REJECT;
    } else if (event.equals(MutualFundsListAction.EVENT_READ_MAKER_CLOSE)) {
        event = MutualFundsListAction.EVENT_LIST_MAKER_CLOSE;
    }

    // Get the record markers.
    int offset = ((Integer) session.getAttribute(
            "com.integrosys.cms.ui.feed.mutualfunds.MutualFundsAction.offset")).intValue();
    int length = ((Integer) session.getAttribute(
            "com.integrosys.cms.ui.feed.mutualfunds.MutualFundsAction.length")).intValue();
    ...
    List compareResultsList = (List) session.getAttribute("com.integrosys.cms.ui.feed.mutualfunds.MutualFundsAction.compareResultsList");
...
function approve() {
document.forms[0].event.value = "<%=MutualFundsListAction.EVENT_APPROVE%>"
document.forms[0].submit();
}
function reject() {
document.forms[0].event.value = "<%=MutualFundsListAction.EVENT_REJECT%>"
document.forms[0].submit();
}
function closeForm() {
...
<thead>
    <tr>
        <td height="34"><h3>Process MutualFunds</h3></td>
    </tr>
    <tr>
        <td colspan="2"><hr/></td>
    </tr>
    ...
    <td colspan="2">
        <table id="tblExchange" width="100%" border="0" cellpadding="0" cellspacing="0" class="tblinfo" style="margin-top:0">
            ...
            <logic:iterate id="result" name="compareResultsList"
                ...
                <%
                    IMutualFundsFeedEntry feedEntry = (IMutualFundsFeedEntry) result.getObj();
                    ...
                    if (result.isAdded()) {
                        ...
                        originalUnitPrice = String.valueOf(((IMutualFundsFeedEntry) result.getOriginal()).getCurrentNAV());
                    } else if (result.isUnmodified()) {
                        // Original value still the same.
                        originalUnitPrice = String.valueOf(feedEntry.getCurrentNAV());
                    ...
                    String updatedPrice = FeedUtils.padFractionDigits(String.valueOf(feedEntry.getCurrentNAV()), 1, 2);
                ...
                <tr class="<%=cssClass%>">
                    ...
                    <td>&nbsp;<integro:empty-if-null value="<%=feedEntry.getSchemeCode()%>"/></td>
                    <td>&nbsp;<integro:empty-if-null value="<%=feedEntry.getSchemeName()%>"/></td>
                    <td>&nbsp;<integro:empty-if-null value="<%=feedEntry.getSchemeType()%>"/></td>
                    <td>&nbsp;<integro:empty-if-null value="<%=UIUtil.formatWithCommaAndDecimal(Double.toString(feedEntry.getCurrentNAV()))%>"/></td>
                    <td>&nbsp;<integro:date object="<%=feedEntry.getStartDate()%>"/></td>
                    <td>&nbsp;<integro:date object="<%=feedEntry.getExpiryDate()%>"/></td>
